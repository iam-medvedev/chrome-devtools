...
async function createUserTimingsDataFromEvents(events) {
...
describeWithEnvironment('UserTimingsHandler', function () {
    ...
    describe('performance timings', function () {
        ...
        describe('performance.measure events parsing', function () {
            it('parses the start and end events and returns a list of blocks', async () => {
                ...
                assert.strictEqual(Trace.Helpers.Trace.extractId(timingsData.performanceMeasures[1]), 'blink.user_timing:0x9072211:first measure');
                assert.strictEqual(timingsData.performanceMeasures[1].name, 'first measure');
                assert.strictEqual(Trace.Helpers.Trace.extractId(timingsData.performanceMeasures[0]), 'blink.user_timing:0x6ece31c8:second measure');
                assert.strictEqual(timingsData.performanceMeasures[0].name, 'second measure');
                assert.strictEqual(Trace.Helpers.Trace.extractId(timingsData.performanceMeasures[2]), 'blink.user_timing:0x10c31982:third measure');
                ...
                for (let i = 0; i < timingsData.performanceMeasures.length; i++) {
                    assert.strictEqual(timingsData.performanceMeasures[i].args.data.beginEvent.ph, "b" /* Trace.Types.Events.Phase.ASYNC_NESTABLE_START */);
                    assert.strictEqual(timingsData.performanceMeasures[i].args.data.endEvent.ph, "e" /* Trace.Types.Events.Phase.ASYNC_NESTABLE_END */);
                }
            });
    ...
    describe('console timings', function () {
        ...
        describe('console.time events parsing', function () {
            it('parses the start and end events and returns a list of blocks', async () => {
                ...
                assert.strictEqual(Trace.Helpers.Trace.extractId(timingsData.consoleTimings[0]), 'blink.console:0x12c00282160:first console time');
                assert.strictEqual(timingsData.consoleTimings[0].name, 'first console time');
                assert.strictEqual(Trace.Helpers.Trace.extractId(timingsData.consoleTimings[1]), 'blink.console:0x12c00282160:second console time');
                ...
                for (let i = 0; i < timingsData.consoleTimings.length; i++) {
                    assert.strictEqual(timingsData.consoleTimings[i].args.data.beginEvent.ph, "b" /* Trace.Types.Events.Phase.ASYNC_NESTABLE_START */);
                    assert.strictEqual(timingsData.consoleTimings[i].args.data.endEvent.ph, "e" /* Trace.Types.Events.Phase.ASYNC_NESTABLE_END */);
                }
            });
            it('sorts the blocks to ensure they are in time order', async function () {
    ...
    describe('parsing user timings events', function () {
        ...
        function createUserTimingsDataExample() {
            const extensionData = [
    ...
    describe('sorting function userTimingComparator', () => {
        function makeFakeSyntheticEvent(ts, dur, name, track) {
            const beginEvent = makeCompleteEvent(name, ts, dur, 'blink.user_timing', 0, 0);
            beginEvent.ph = "b" /* Trace.Types.Events.Phase.ASYNC_NESTABLE_START */;
            beginEvent.args.detail = JSON.stringify({ devtools: { track } });
            const endEvent = makeCompleteEvent(name, ts + dur, 0, 'blink.user_timing', 0, 0);
            endEvent.ph = "e" /* Trace.Types.Events.Phase.ASYNC_NESTABLE_END */;
        ...
        function makeFakeConsoleTimestampEvent(start, end, name, track) {
            return {
    ...
    describe('parsing performance.measures with re-used IDs', () => {
        const { microToMilli, } = Trace.Helpers.Timing;
         ...
         */
        function roundedMilliTimings(events) {
            return events.map(e => {
                return {
