...
let idCounter = 0;
function makeTimingEventWithExtensionData({ name, ts: tsMicro, detail, dur: durMicro }) {
...
export async function createTraceExtensionDataFromTestInput(extensionData) {
...
describe('ExtensionTraceDataHandler', function () {
    ...
    describe('Timeline markers from user timings that use the extension API', function () {
        ...
        it('discards markers whose details are not valid stringified JSON', async () => {
            ...
            assert.isNull(Trace.Handlers.ModelHandlers.ExtensionTraceData.extensionDataInTiming(performanceMarkEvent));
        });
    ...
    describe('Track groups', () => {
        it('builds extension track data for grouped tracks correctly', async function () {
            ...
            const extensionHandlerOutput = await createTraceExtensionDataFromTestInput(extensionDevToolsObjects.map((devtools, i) => ({
                detail: { devtools },
                name: 'A measurement',
