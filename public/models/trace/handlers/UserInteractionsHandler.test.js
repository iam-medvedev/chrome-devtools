...
describe('UserInteractionsHandler', function () {
    ...
    it('detects correct events for a click and keydown interaction', async () => {
        await processTrace(this, 'slow-interaction-keydown.json.gz');
        const data = Trace.Handlers.ModelHandlers.UserInteractions.data();
        const foundInteractions = data.interactionEvents.filter(Trace.Types.Events.isEventTimingStart).filter(e => e.dur > 1);
        // We expect there to be 3 interactions:
        // User clicks on input:
        // 1.pointerdown, 2. click, 3. pointerup.
        // User types into input:
        // 4. keydown, 5. keyup.
        assert.deepEqual(foundInteractions.map(event => event.type), ['pointerdown', 'click', 'pointerup', 'keydown', 'keyup']);
    ...
    describe('collapsing nested interactions', () => {
        const { removeNestedInteractionsAndSetProcessingTime: removeNestedInteractions } = Trace.Handlers.ModelHandlers.UserInteractions;
