...
describeWithEnvironment('TraceProcessor', function () {
    ...
    describe('insights', () => {
            ...
            sinon.stub(Trace.Processor.TraceProcessor, 'getInsightRunners').callsFake(() => {
                return {
                    ...
                    RenderBlocking: {
                        ...
                        // eslint-disable-next-line @typescript-eslint/no-explicit-any
                        i18nString: (() => { }),
                        isRenderBlockingInsight: (_x) => false,
                        generateInsight: () => {
        ...
        it('returns insights for soft navigations', async function () {
            ...
            const insightSets = Array.from(processor.insights.values());
            assert.isTrue(Trace.Types.Events.isSoftNavigationStart(insightSets[1].navigation));
            assert.isTrue(Trace.Types.Events.isSoftNavigationStart(insightSets[2].navigation));
            assert.isTrue(Trace.Types.Events.isSoftNavigationStart(insightSets[3].navigation));
        });
