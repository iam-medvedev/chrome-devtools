...
describeWithEnvironment('DuplicatedJavaScript', function () {
    it('works (external source maps)', async () => {
        ...
        const results = Object.fromEntries([...duplication.entries()].filter(v => v[1].estimatedDuplicateBytes > 1000 * 30).map(([key, data]) => {
        ...
        assert.deepEqual(insight.metricSavings, { FCP: 100, LCP: 100 });
    });
    function assertInlineMapsTestCase(insight) {
        const duplication = insight.duplicationGroupedByNodeModules;
        const results = Object.fromEntries([...duplication.entries()].filter(v => v[1].estimatedDuplicateBytes > 1000 * 25).map(([key, data]) => {
            return [key, data.duplicates.map(v => ({ url: v.script.url, transferSize: v.attributedSize }))];
        }));
        ...
        assert.deepEqual(insight.metricSavings, { FCP: 50, LCP: 50 });
    }
...
//# sourceMappingURL=DuplicatedJavaScript.test.js.map