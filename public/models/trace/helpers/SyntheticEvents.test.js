...
describe('SyntheticEvents', function () {
    ...
    describe('SyntheticBased registration', () => {
        it('stores synthetic based events at the same index as their corresponding raw event in the source array', async function () {
            ...
            const rawEvents = 'traceEvents' in contents ? contents.traceEvents : contents;
            ...
            const allSyntheticEventsInManagerCount = syntheticEventsManager.getSyntheticTraces().reduce((count, event) => event !== undefined ? (count + 1) : 0, 0);
