...
describeWithEnvironment('Timing helpers', () => {
    ...
    describe('timeStampForEventAdjustedByClosestNavigation', () => {
        ...
        it('can adjust a MARK_SOFT_FCP event timestamp relative to its soft navigation start', async () => {
            ...
            const adjustedTime = Trace.Helpers.Timing.timeStampForEventAdjustedByClosestNavigation(softFcpEvent, traceBounds, new Map(), softNavigationsById, new Map());
        ...
        it('can adjust a SoftNavigationStart event timestamp relative to its closest navigation start', async () => {
            const navigationStart = {
                name: "navigationStart" /* Trace.Types.Events.Name.NAVIGATION_START */,
                ph: "R" /* Trace.Types.Events.Phase.MARK */,
                pid: Trace.Types.Events.ProcessID(1),
                tid: Trace.Types.Events.ThreadID(1),
                cat: 'disabled-by-default-devtools.timeline',
            ...
            const adjustedTime = Trace.Helpers.Timing.timeStampForEventAdjustedByClosestNavigation(softNavStart, traceBounds, new Map(), new Map(), navigationsByFrameId);
