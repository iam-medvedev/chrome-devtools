...
describeWithEnvironment('Timing helpers', () => {
    ...
    describe('timeStampForEventAdjustedByClosestNavigation', () => {
        it('can use the navigation ID to adjust the time correctly', async function () {
            ...
            const adjustedTime = TraceModel.Helpers.Timing.timeStampForEventAdjustedByClosestNavigation(lcpEvent, traceData.Meta.traceBounds, traceData.Meta.navigationsByNavigationId, traceData.Meta.navigationsByFrameId);
            const unadjustedTime = TraceModel.Helpers.Timing.microSecondsToMilliseconds(TraceModel.Types.Timing.MicroSeconds(lcpEvent.ts - traceData.Meta.traceBounds.min));
        ...
        it('can use the frame ID to adjust the time correctly', async function () {
            ...
            const unadjustedTime = TraceModel.Helpers.Timing.microSecondsToMilliseconds(TraceModel.Types.Timing.MicroSeconds(dclEvent.ts - traceData.Meta.traceBounds.min));
            assert.strictEqual(unadjustedTime.toFixed(2), String(190.79));
            const adjustedTime = TraceModel.Helpers.Timing.timeStampForEventAdjustedByClosestNavigation(dclEvent, traceData.Meta.traceBounds, traceData.Meta.navigationsByNavigationId, traceData.Meta.navigationsByFrameId);
            // To make the assertion easier to read.
            const timeAsMS = TraceModel.Helpers.Timing.microSecondsToMilliseconds(adjustedTime);
    ...
    describe('expandWindowByPercentOrToOneMillisecond', () => {
        it('can expand trace window by a percentage', async function () {
            const traceWindow = TraceModel.Helpers.Timing.traceWindowFromMicroSeconds(milliToMicro(40), milliToMicro(60));
            const maxTraceWindow = TraceModel.Helpers.Timing.traceWindowFromMicroSeconds(milliToMicro(0), milliToMicro(100));
            const expandedTraceWindow = TraceModel.Helpers.Timing.expandWindowByPercentOrToOneMillisecond(traceWindow, maxTraceWindow, 50);
        ...
        it('if the expanded window is smaller than 1 millisecond, expands it to 1 millisecond ', async function () {
            ...
            const traceWindow = TraceModel.Helpers.Timing.traceWindowFromMicroSeconds(TraceModel.Types.Timing.MicroSeconds(1000), TraceModel.Types.Timing.MicroSeconds(1500));
            const maxTraceWindow = TraceModel.Helpers.Timing.traceWindowFromMicroSeconds(milliToMicro(0), milliToMicro(100));
            const expandedTraceWindow = TraceModel.Helpers.Timing.expandWindowByPercentOrToOneMillisecond(traceWindow, maxTraceWindow, 5);
            ...
            assert.strictEqual((traceWindow.max + traceWindow.min) / 2, (expandedTraceWindow.max + expandedTraceWindow.min) / 2);
        ...
        it('window does not expand past the provided max window', async function () {
            const traceWindow = TraceModel.Helpers.Timing.traceWindowFromMicroSeconds(milliToMicro(5), milliToMicro(55));
            const maxTraceWindow = TraceModel.Helpers.Timing.traceWindowFromMicroSeconds(milliToMicro(0), milliToMicro(100));
            const expandedTraceWindow = TraceModel.Helpers.Timing.expandWindowByPercentOrToOneMillisecond(traceWindow, maxTraceWindow, 50);
