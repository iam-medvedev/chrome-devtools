...
export function calculateWindow(traceBounds, mainThreadEntries) {
    ...
    function findLowUtilizationRegion(startIndex, stopIndex) {
        ...
        const entryAtCut = mainThreadEntries[cutIndex];
    ...
    const rightIndex = findLowUtilizationRegion(mainThreadEntries.length - 1, 0);
    const leftIndex = findLowUtilizationRegion(0, rightIndex);
    const leftTimings = Helpers.Timing.eventTimingsMicroSeconds(mainThreadEntries[leftIndex]);
    const rightTimings = Helpers.Timing.eventTimingsMicroSeconds(mainThreadEntries[rightIndex]);
