...
describeWithEnvironment('FilmStrip', function () {
    ...
    describe('using the old screenshot format', () => {
        it('identifies the frames from a trace', async function () {
            ...
            const filmStrip = Trace.Extras.FilmStrip.fromHandlerData(data);
        ...
        it('can use a custom zero time to filter out screenshots', async function () {
            ...
            const newFilmStrip = Trace.Extras.FilmStrip.fromHandlerData(data, newCustomZeroTime);
    ...
    describe('using the new screenshot format', () => {
        it('identifies the frames from a trace', async function () {
            const { data } = await TraceLoader.traceEngine(this, 'example-dot-com.json.gz');
            const filmStrip = Trace.Extras.FilmStrip.fromHandlerData(data);
    ...
    it('caches the film strip based on the trace data and the zero time', async function () {
        const { data } = await TraceLoader.traceEngine(this, 'web-dev.json.gz');
        const filmStrip1 = Trace.Extras.FilmStrip.fromHandlerData(data);
        const filmStrip2 = Trace.Extras.FilmStrip.fromHandlerData(data);
        // It is from cache so you get back the exact same object.
        assert.strictEqual(filmStrip1, filmStrip2);
        const filmStrip3 = Trace.Extras.FilmStrip.fromHandlerData(data, Trace.Types.Timing.Micro(0));
        const filmStrip4 = Trace.Extras.FilmStrip.fromHandlerData(data, Trace.Types.Timing.Micro(5));
    ...
    it('can return the frame closest to a given timestamp', async function () {
        ...
        assert.strictEqual(Trace.Extras.FilmStrip.frameClosestToTimestamp(filmStrip, timestampNearestFirstFrame), filmStrip.frames.at(0));
        const timestampNearestThirdFrame = Trace.Types.Timing.Micro(frameTimestamps[2] + 10);
        assert.strictEqual(Trace.Extras.FilmStrip.frameClosestToTimestamp(filmStrip, timestampNearestThirdFrame), filmStrip.frames.at(2));
