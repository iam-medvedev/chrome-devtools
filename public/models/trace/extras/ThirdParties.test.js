...
describeWithEnvironment('ThirdParties', function () {
    describe('byTraceBounds', function () {
        it('full trace bounds', async function () {
            ...
            const requests = parsedTrace.NetworkRequests.byTime.filter(request => Trace.Helpers.Timing.eventIsInBounds(request, parsedTrace.Meta.traceBounds));
            const thirdPartySummary = Trace.Extras.ThirdParties.summarizeThirdParties(parsedTrace, parsedTrace.Meta.traceBounds, requests);
        ...
        it('works even without network requests', async function () {
            const { parsedTrace } = await TraceLoader.traceEngine(this, 'load-simple.json.gz');
            const thirdPartySummary = Trace.Extras.ThirdParties.summarizeThirdParties(parsedTrace, parsedTrace.Meta.traceBounds, []);
            const results = [...thirdPartySummary.byEntity.entries()].map(([entity, summary]) => [entity.name, summary]);
            assert.deepEqual(results, [
        ...
        it('partial trace bounds', async function () {
            ...
            const requests = parsedTrace.NetworkRequests.byTime.filter(request => Trace.Helpers.Timing.eventIsInBounds(request, bounds));
