...
describeWithEnvironment('TraceModel', function () {
    it('dispatches an end event when the trace is done', async function () {
        ...
        const events = [];
        model.addEventListener(Trace.TraceModel.ModelUpdateEvent.eventName, (evt) => {
    ...
    it('supports a custom time formatter for insights', async function () {
        ...
        const insight = getInsightOrError('DocumentLatency', result.insights, getFirstOrError(result.data.Meta.navigationsByNavigationId.values()));
