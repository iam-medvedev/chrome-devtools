  ...
  Panels.prototype = {
    ...
    setOpenResourceHandler: function(callback) {
      ...
      if (hadHandler === !callback) {
        extensionServer.sendRequest({ command: "setOpenResourceHandler", handlerPresent: Boolean(callback) });
      }
    },
...
var ExtensionSidebarPane = class extends UI2.View.SimpleView {
  ...
  setObjectInternal(object, title, callback) {
    ...
    void UI2.UIUtils.Renderer.render(object, { title, editable: false }).then((result) => {
      if (!result) {
        callback();
        return;
      }
