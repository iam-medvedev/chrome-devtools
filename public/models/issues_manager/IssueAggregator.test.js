...
function createModel() {
    const model = new MockIssuesModel([]);
    assert.exists(model);
    return model;
}
...
describe('IssueAggregator', () => {
    ...
    it('aggregates hidden issues correctly on updating settings', () => {
        ...
        issuesManager.dispatchEventToListeners("FullUpdateRequired" /* IssuesManager.IssuesManager.Events.FULL_UPDATE_REQUIRED */);
        assert.strictEqual(aggregator.numberOfAggregatedIssues(), 2);
        assert.strictEqual(aggregator.numberOfHiddenAggregatedIssues(), 2);
    });
...
describe('IssueAggregator', () => {
    ...
    it('should not aggregate third-party cookie phaseout or mitigation related issues', async () => {
        ...
        issuesManager.addIssue(getTestCookieIssue("WarnDomainNonASCII" /* Protocol.Audits.CookieWarningReason.WarnDomainNonASCII */));
        issuesManager.addIssue(getTestCookieIssue(undefined, "ExcludeDomainNonASCII" /* Protocol.Audits.CookieExclusionReason.ExcludeDomainNonASCII */));
        assert.strictEqual(aggregator.numberOfAggregatedIssues(), 2);
        assert.strictEqual(aggregator.numberOfHiddenAggregatedIssues(), 0);
    });
});
