...
describe('IgnoreListManager', () => {
    ...
    beforeEach(async () => {
        ...
        const scripts = SCRIPTS.map(({ scriptId, startLine, startColumn, endLine, endColumn, executionContextAuxData, sourceURL, hasSourceURLComment, sourceMapURL, }) => {
            return debuggerModel.parsedScriptSource(scriptId, sourceURL, startLine, startColumn, endLine, endColumn, executionContextId, hash, executionContextAuxData, sourceMapURL, hasSourceURLComment, false, length, false, null, null, null, null, embedderName, null);
        });
    ...
    function getFolderContextMenu(url) {
        ...
        const options = {
            ...
            isCurrentlyIgnoreListed: ALL_URLS.every(scriptUrl => !scriptUrl.startsWith(url) ||
                ignoreListManager.isUserOrSourceMapIgnoreListedUISourceCode(notNull(universe.workspace.uiSourceCodeForURL(scriptUrl)))),
        };
