...
import { getMainThread, makeCompleteEvent, makeProfileCall, } from '../../../test/unittests/front_end/helpers/TraceHelpers.js';
...
describeWithEnvironment('TimelineProfileTree', () => {
    ...
    describe('BottomUpRootNode', () => {
        ...
        it('builds a bottom up tree from an event tree with multiple levels 1', () => {
            ...
            const root = new TimelineModel.TimelineProfileTree.BottomUpRootNode(events, new TimelineModel.TimelineModelFilter.TimelineInvisibleEventsFilter([]), [], 0, 200_000, null);
            const rootChildren = root.children();
        ...
        it('correctly keeps ProfileCall nodes and uses them to build up the tree', async function () {
            ...
            const profileCalls = values
                .filter(node => TraceEngine.Legacy.eventIsFromNewEngine(node.event) &&
