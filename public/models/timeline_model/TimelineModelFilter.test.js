    ...
    describe('TimelineVisibleEventsFilter', () => {
        ...
        describe('eventType', () => {
            it('returns ConsoleTime if the event has the blink.console category', async function () {
                ...
                assert.strictEqual(TimelineModel.TimelineModelFilter.TimelineVisibleEventsFilter.eventType(consoleTimingEvent), "ConsoleTime" /* Trace.Types.Events.Name.CONSOLE_TIME */);
            });
            it('returns UserTiming if the event has the blink.user_timing category', async function () {
                const { parsedTrace } = await TraceLoader.traceEngine(this, 'timings-track.json.gz');
                const userTimingEvent = (parsedTrace.UserTimings.performanceMeasures).at(0);
                assert.isOk(userTimingEvent);
                assert.strictEqual(TimelineModel.TimelineModelFilter.TimelineVisibleEventsFilter.eventType(userTimingEvent), "UserTiming" /* Trace.Types.Events.Name.USER_TIMING */);
            });
            it('returns the event name if the event is any other category', async function () {
                ...
                assert.strictEqual(TimelineModel.TimelineModelFilter.TimelineVisibleEventsFilter.eventType(layoutShiftEvent), "LayoutShift" /* Trace.Types.Events.Name.LAYOUT_SHIFT */);
            });
        });
    });
