    ...
    describe('TimelineVisibleEventsFilter', () => {
        ...
        describe('eventType', () => {
            it('returns ConsoleTime if the event has the Console category', () => {
                assert.strictEqual(TimelineModel.TimelineModelFilter.TimelineVisibleEventsFilter.eventType(consoleEvent), TimelineModel.TimelineModel.RecordType.ConsoleTime);
            });
            it('returns UserTiming if the event has the UserTiming category', () => {
                assert.strictEqual(TimelineModel.TimelineModelFilter.TimelineVisibleEventsFilter.eventType(userTimingEvent), TimelineModel.TimelineModel.RecordType.UserTiming);
            });
            it('returns the event name if the event is any other category', () => {
                ...
                assert.strictEqual(TimelineModel.TimelineModelFilter.TimelineVisibleEventsFilter.eventType(otherEvent), 'other');
            });
        });
    });
