...
describe('CompilerScriptMapping', () => {
    ...
    it('correctly maps to multiple raw locations if the source map has multiple entries for a single source line/column', async () => {
        const target = backend.createTarget();
        const sourceRoot = 'http://example.com';
        const scriptInfo = {
            url: `${sourceRoot}/test.out.js`,
    ...
    describe('translateRawFramesStep', () => {
        ...
        it('translates a single frame using "proposal scopes" information', async () => {
            ...
            const script = await backend.addScript(target, { url: 'http://example.com/index.js', content: 'function f(){debugger;}' }, {
        ...
        it('expands inlined frames and populates UISourceCode', async () => {
            ...
            builder.startRange(0, 0, { scopeKey: 'global' })
                ...
                .endRange(0, 14)
                .endRange(1, 0);
            const sourceMap = ScopesCodec.encode(builder.build(), encodeSourceMap(['0:5 => index.ts:1:7']));
            const script = await backend.addScript(target, { url: 'http://example.com/index.js', content: 'print(\'hello\')' }, {
                url: 'http://example.com/index.js.map',
                content: sourceMap,
            });
            const translatedFrames = [];
