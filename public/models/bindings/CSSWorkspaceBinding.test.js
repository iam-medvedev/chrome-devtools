...
describe('CSSWorkspaceBinding', () => {
    ...
    const waitForUISourceCodeAdded = (url) => new Promise(resolve => {
        const uiSourceCode = workspace.uiSourceCodeForURL(url);
        if (uiSourceCode) {
            resolve(uiSourceCode);
            return;
        }
        const { eventType, listener } = workspace.addEventListener(Workspace.Workspace.Events.UISourceCodeAdded, event => {
    ...
    it('correctly maps locations using source map', async () => {
        ...
        assert.deepEqual(cssWorkspaceBinding.sourceMapURLsForUISourceCode(scssUISourceCode), [urlString `http://example.com/example.css.map`]);
        const header = cssModel.styleSheetHeaderForId(headerPayload.styleSheetId);
        assert.exists(header);
        const testLocation = (line, column, expectedUISourceCode, expectedLine, expectedColumn, expectedReverseLine, expectedReverseColumn) => {
            ...
            assert.strictEqual(uiLocation.uiSourceCode, expectedUISourceCode, `Mismatched UISourceCode for ${line}:${column}`);
            ...
            assert.isNotEmpty(reverseRawLocations, `Could not reverse map UI location ${uiLocation.lineNumber}:${uiLocation.columnNumber}`);
            const reverseRaw = reverseRawLocations[0];
            assert.strictEqual(reverseRaw.lineNumber, expectedReverseLine, `Mismatched reverse line number for ${line}:${column}`);
            assert.strictEqual(reverseRaw.columnNumber, expectedReverseColumn, `Mismatched reverse column number for ${line}:${column}`);
        };
    ...
    it('correctly maps property UI locations', async () => {
        ...
        const mockProperty = (forName, line, column) => {
        ...
        const testPropertyUILocation = (forName, line, column, expectedUISourceCode, expectedLine, expectedColumn) => {
...
//# sourceMappingURL=CSSWorkspaceBinding.test.js.map