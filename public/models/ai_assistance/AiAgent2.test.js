...
function getFunctionDeclarations(aidaClient, callIndex) {
...
describeWithEnvironment('AiAgent2', () => {
    ...
    it('handles learning skills correctly (UI step and AIDA response)', async () => {
        ...
        const actionResponse = responses.find(r => r.type === "action" /* AiAssistance.AiAgent.ResponseType.ACTION */);
        ...
        const responseObj = functionResponse.response;
    ...
    it('prevents duplicate tool declarations if multiple learned skills share the same tool', async () => {
        ...
        await agent.learnSkill(['styling', 'dummy']);
        assert.isTrue(agent.activeSkills.has('styling'));
        assert.isTrue(agent.activeSkills.has('dummy'));
    });
    it('enhances the query with the selected element description', async () => {
        ...
        assert.isTrue(enhancedQuery.includes('# Inspected element\n\nelement-description\n\n# User request\n\nQUERY: my query'));
    });
    ...
    it('supports tools with side-effect approval flow (e.g. executeJavaScript)', async () => {
        ...
        handlerStub.onFirstCall().resolves({ requiresApproval: true, description: 'This code may modify page content. Continue?' });
        ...
        while (!next.done) {
            ...
            if (response.type === "side-effect" /* AiAssistance.AiAgent.ResponseType.SIDE_EFFECT */) {
        ...
        const sideEffectResponse = responses.find(r => r.type === "side-effect" /* AiAssistance.AiAgent.ResponseType.SIDE_EFFECT */);
        assert.isDefined(sideEffectResponse);
        const actionResponses = responses.filter(r => r.type === "action" /* AiAssistance.AiAgent.ResponseType.ACTION */);
