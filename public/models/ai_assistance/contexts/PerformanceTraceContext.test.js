...
Mock Longest Tasks`);
    ...
    it('returns CORE_VITALS widget when primaryInsightSet is present', async () => {
        ...
        const mockTrace = {
            insights: new Map([['set-1', mockInsightSet]]),
            data: {
                Meta: {
                    mainFrameURL: 'https://example.com',
                    traceBounds: { min: 0, max: 100 },
    ...
    it('returns PERF_INSIGHT and CORE_VITALS widget when insight is focused', async () => {
        ...
        const context = AiAssistance.PerformanceTraceContext.PerformanceTraceContext.fromInsight(mockTrace, mockInsight, universe.targetManager, undefined, universe.debuggerWorkspaceBinding);
        const widgets = await context.getWidgets();
        assert.deepEqual(widgets, [
            {
                name: 'PERF_INSIGHT',
    ...
    it('returns TIMELINE_RANGE_SUMMARY and BOTTOM_UP_TREE widget when callTree is focused', async () => {
        ...
        const context = AiAssistance.PerformanceTraceContext.PerformanceTraceContext.fromCallTree(mockCallTree, universe.targetManager, undefined, universe.debuggerWorkspaceBinding);
        const widgets = await context.getWidgets();
        assert.deepEqual(widgets, [
            {
                name: 'TIMELINE_RANGE_SUMMARY',
                data: {
                    bounds: Trace.Helpers.Timing.traceWindowFromMicroSeconds(Trace.Types.Timing.Micro(1000), Trace.Types.Timing.Micro(3000)),
