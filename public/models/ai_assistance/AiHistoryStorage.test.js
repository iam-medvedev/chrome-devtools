...
describe('AiHistoryStorage', () => {
    ...
    it('should delete all entries', async () => {
        ...
        const historyDeletedPromise = storage.once('AiHistoryDeleted');
        await storage.deleteAll();
        assert.deepEqual(storage.getHistory(), []);
        assert.deepEqual(storage.getImageHistory(), []);
        assert.deepEqual(storage.getRecentPrompts(), []);
    ...
    describe('Conversation', () => {
        describe('title', () => {
            it('should return undefined if there is not USER_QUERY entry in history', () => {
                const conversation = new AiAssistance.AiConversation.AiConversation({ type: "freestyler" /* AiAssistance.AiHistoryStorage.ConversationType.STYLING */, data: [] });
