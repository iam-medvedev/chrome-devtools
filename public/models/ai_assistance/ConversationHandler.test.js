...
import { cleanup, createNetworkRequest, mockAidaClient, } from '../../testing/AiAssistanceHelpers.js';
...
describeWithMockConnection('ConversationHandler', () => {
    describe('handleExternalRequest', () => {
        ...
        it('returns an explanation for styling assistance requests', async () => {
            ...
            conversationHandler.addEventListener("ExternalRequestReceived" /* AiAssistanceModel.ConversationHandler.ConversationHandlerEvents.EXTERNAL_REQUEST_RECEIVED */, requestReceivedSpy);
            const generator = await conversationHandler.handleExternalRequest({
                prompt: 'Please help me debug this problem',
                conversationType: "freestyler" /* AiAssistanceModel.AiHistoryStorage.ConversationType.STYLING */
            });
        ...
        it('persists external conversations to history', async () => {
            ...
            assert.strictEqual(historicalConversations[0].type, "freestyler" /* AiAssistanceModel.AiHistoryStorage.ConversationType.STYLING */);
            assert.isTrue(historicalConversations[0].isExternal);
            assert.strictEqual(JSON.stringify(historicalConversations[0].history), '[{"type":"user-query","query":"Please help me debug this problem"},{"type":"querying"},{"type":"answer","text":"I need more information","complete":true}]');
        });
...
//# sourceMappingURL=ConversationHandler.test.js.map