...
describeWithEnvironment('StylingAgent', () => {
    ...
    describe('parseResponse', () => {
        ...
        it('parses a thought', async () => {
            ...
            assert.deepEqual(getParsedTextResponse(`THOUGHT: ${payload}`), {
                title: undefined,
                thought: payload,
            });
            assert.deepEqual(getParsedTextResponse(`   THOUGHT: ${payload}`), {
            ...
            assert.deepEqual(getParsedTextResponse(`Something\n   THOUGHT: ${payload}`), {
                title: undefined,
                thought: payload,
            });
        });
        it('parses a answer', async () => {
            const payload = 'some response';
            assert.deepEqual(getParsedTextResponse(`ANSWER: ${payload}`), {
                answer: payload,
...
c`;
  ...
  someKey: "value",
            ...
            assert.deepEqual(getParsedTextResponse(`ACTION\n${payload}\nSTOP`), {
                action: payload,
                title: undefined,
                thought: undefined,
            });
  ...
  someKey: "value",
            ...
            assert.deepEqual(getParsedTextResponse(`THOUGHT:${thoughtPayload}\nACTION\n${actionPayload}\nSTOP`), {
        ...
        it('parses an answer and suggestions', async () => {
            ...
            assert.deepEqual(getParsedTextResponse(`ANSWER:${answerPayload}\nSUGGESTIONS: ${suggestionsText}`), {
                answer: answerPayload,
                suggestions,
            });
        });
  ...
  someKey: "value",
        ...
        it('parses a response as an answer', async () => {
            assert.deepEqual(getParsedTextResponse('This is also an answer'), {
        ...
        it('parses multi line thoughts', () => {
            ...
            assert.deepEqual(getParsedTextResponse(`THOUGHT: ${thoughtText}`), {
                thought: thoughtText,
                title: undefined,
            });
        });
...
STOP`,
            ...
            assert.deepEqual(requests[0].current_message.parts[0], {
            ...
            assert.deepEqual(requests[1].historical_contexts, [
                {
                    role: 1,
                    parts: [{ text: '# Inspected element\n\n* Its selector is `undefined`\n\n# User request\n\nQUERY: test' }],
                },
            ...
            assert.deepEqual(requests[1].current_message.parts[0], { text: 'OBSERVATION: test data' }, 'Unexpected input in the follow-up request');
        });
...
STOP
        ...
        it('generates a response if nothing is returned', async () => {
            ...
            assert.deepEqual(responses, [
                ...
                {
                    type: "error" /* AiAssistance.ResponseType.ERROR */,
                    error: "unknown" /* AiAssistance.ErrorType.UNKNOWN */,
                },
            ]);
            sinon.assert.notCalled(execJs);
    ...
    describe('enhanceQuery', () => {
        ...
        it('does not add multimodal input evaluation prompt when multimodal is enabled but multimodalInputType is missing', async () => {
        ...
        it('adds multimodal input evaluation prompt when multimodal is enabled and multimodalInputType is screenshot', async () => {
            mockHostConfig('test model', 1, 'PUBLIC', Root.Runtime.HostConfigFreestylerExecutionMode.NO_SCRIPTS, true);
            const enhancedQuery = await agent.enhanceQuery('test query', new AiAssistance.NodeContext(element), "screenshot" /* AiAssistance.MultimodalInputType.SCREENSHOT */);
...
QUERY: test query`);
        ...
        it('adds multimodal input evaluation prompt when multimodal is enabled and multimodalInputType is uploaded image', async () => {
            mockHostConfig('test model', 1, 'PUBLIC', Root.Runtime.HostConfigFreestylerExecutionMode.NO_SCRIPTS, true);
            ...
            assert.strictEqual(enhancedQuery, `The user has uploaded an image in base64-encoded format. You SHOULD use it while answering user's queries.
# Considerations for evaluating image:
* Pay close attention to the spatial details as well as the visual appearance of the selected element in the image, particularly in relation to layout, spacing, and styling.
* Analyze the image to identify the layout structure surrounding the element, including the positioning of neighboring elements.
* Extract visual information from the image, such as colors, fonts, spacing, and sizes, that might be relevant to the user's query.
