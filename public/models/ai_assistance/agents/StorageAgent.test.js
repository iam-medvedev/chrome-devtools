...
describeWithMockConnection('StorageAgent', function () {
    ...
    it('can list cookies for the origin, excluding HttpOnly ones', async () => {
        ...
        mockCookie.addAttribute("domain" /* SDK.Cookie.Attribute.DOMAIN */, 'example.com');
        mockCookie.addAttribute("path" /* SDK.Cookie.Attribute.PATH */, '/');
        ...
        httpOnlyCookie.addAttribute("path" /* SDK.Cookie.Attribute.PATH */, '/');
    ...
    it('can get cookie values with approval', async () => {
        ...
        mockCookie.addAttribute("domain" /* SDK.Cookie.Attribute.DOMAIN */, 'example.com');
        mockCookie.addAttribute("path" /* SDK.Cookie.Attribute.PATH */, '/');
        const getCookiesStub = sinon.stub(cookieModel, 'getCookiesForDomain').withArgs('https://example.com').resolves([mockCookie]);
        sideEffectPromise.resolve(true);
    ...
    it('can resolve duplicate same-name cookies with different metadata', async () => {
        ...
        cookie2.addAttribute("domain" /* SDK.Cookie.Attribute.DOMAIN */, '.example.com');
        cookie2.addAttribute("path" /* SDK.Cookie.Attribute.PATH */, '/sub');
    ...
    it('strictly filters out HttpOnly cookies from getCookieValues requested names', async () => {
        ...
        publicCookie.addAttribute("domain" /* SDK.Cookie.Attribute.DOMAIN */, 'example.com');
        publicCookie.addAttribute("path" /* SDK.Cookie.Attribute.PATH */, '/');
        // HttpOnly cookie
        const secretCookie = new SDK.Cookie.Cookie('secret-cookie', 'secret-value');
        secretCookie.addAttribute("domain" /* SDK.Cookie.Attribute.DOMAIN */, 'example.com');
        secretCookie.addAttribute("path" /* SDK.Cookie.Attribute.PATH */, '/');
        secretCookie.addAttribute("http-only" /* SDK.Cookie.Attribute.HTTP_ONLY */);
    ...
    describe('resolveDOMStorages', () => {
        ...
        it('targets a specific storageKey partition if specified', () => {
            const context = new AiAssistance.StorageAgent.StorageContext(new AiAssistance.StorageItem.DOMStorageItem('https://example.com', 'https://example.com', 'https://example.com/', 'localStorage'));
            const resolved = AiAssistance.StorageAgent.resolveDOMStorages(context, 'localStorage', 'https://example.com', 'https://example.com/');
            assert.lengthOf(resolved, 1);
            const nonMatching = AiAssistance.StorageAgent.resolveDOMStorages(context, 'localStorage', 'https://example.com', 'https://different-partition/');
...
//# sourceMappingURL=StorageAgent.test.js.map