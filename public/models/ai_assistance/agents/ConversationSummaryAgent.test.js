...
import { ConversationSummaryAgent } from '../ai_assistance.js';
describeWithEnvironment('ConversationSummaryAgent', () => {
    it('summarizes a conversation', async () => {
        ...
        assert.isTrue(summary.includes('Note: The code fixes and findings above were identified on a live page in DevTools'));
    });
    it('summarizes a conversation using a context', async () => {
        ...
        assert.strictEqual(lastResponse.text, 'Summary from context');
    });
    it('throws an error if no response is received', async () => {
        ...
        try {
            ...
            assert.strictEqual(err.message, 'Failed to summarize conversation');
        }
    });
});
//# sourceMappingURL=ConversationSummaryAgent.test.js.map