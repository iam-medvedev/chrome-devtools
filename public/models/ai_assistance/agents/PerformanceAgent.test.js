...
import { AICallTree, AIContext, PerformanceAgent, PerformanceTraceContext, PerformanceTraceFormatter, } from '../ai_assistance.js';
/**
...
\`\`\``,
        ...
        it('translates eventKey: URLs in link destinations', async function () {
            ...
            const response = agent.parseTextResponse('The LCP image [https://www.diy.com/](urlIndex: 0, eventKey: r-14746) is a background image');
            assert.deepEqual(response, { answer: 'The LCP image [https://www.diy.com/](#r-14746) is a background image' });
            const response2 = agent.parseTextResponse('The LCP image [https://www.diy.com/](eventKey: r-14746, urlIndex: 0) is a background image');
        ...
        it('translates plain eventKeys in link destinations', async function () {
            ...
            const response = agent.parseTextResponse('The LCP image [https://www.diy.com/](valid-event-key) is a background image');
        ...
        it('translates eventKey: URLs with spaces between bracket and parenthesis', async function () {
            ...
            const context = PerformanceTraceContext.PerformanceTraceContext.fromParsedTrace(parsedTrace);
            await agent.run('', { selected: context }).next();
            const response = agent.parseTextResponse('The LCP element is an image [IMG class=\'h-auto w-full\'] (eventKey: r-12227) loaded from [https://media.diy.com/is/image] (eventKey: s-2069)');
            assert.deepEqual(response, {
                answer: 'The LCP element is an image [IMG class=\'h-auto w-full\'](#r-12227) loaded from [https://media.diy.com/is/image](#s-2069)',
            });
        });
    });
    ...
    describe('function calls', () => {
        it('can call getNetworkTrackSummary', async function () {
            ...
            const networkWidget = action.widgets[0];
        ...
        it('can call getEventByKey and yields TIMELINE_EVENT_SUMMARY widget', async function () {
            ...
            const eventKey = 'r-0';
            const agent = createAgentForConversation({
                aidaClient: mockAidaClient([[{ explanation: '', functionCalls: [{ name: 'getEventByKey', args: { eventKey } }] }], [{ explanation: 'done' }]]),
            });
            const context = PerformanceTraceContext.PerformanceTraceContext.fromInsight(parsedTrace, lcpBreakdown);
        ...
        it('will not send facts from a previous insight if the context changes', async function () {
            ...
            const renderBlockingContext = PerformanceTraceContext.PerformanceTraceContext.fromInsight(parsedTrace, renderBlocking);
                ...
                model: {
                    ...
                    INPBreakdown: {
                        ...
                        longestInteractionEvent: {
                            dur: 1000000, // 1 second = Poor
                        },
                    },
                    CLSCulprits: {
                        insightKey: Trace.Insights.Types.InsightKeys.CLS_CULPRITS,
                        state: 'fail',
                        clusters: [{ clusterCumulativeScore: 0.5 }],
                        worstCluster: {
        ...
        it('limits failing insight suggestions so there is a max of 4 total suggestions', async () => {
            const insightSet = {
                ...
                model: {
                    ...
                    Insight2: { insightKey: Trace.Insights.Types.InsightKeys.RENDER_BLOCKING, state: 'fail', title: 'Render Blocking' },
                    Insight3: { insightKey: Trace.Insights.Types.InsightKeys.DOCUMENT_LATENCY, state: 'fail', title: 'Document Latency' },
                    Insight4: { insightKey: Trace.Insights.Types.InsightKeys.IMAGE_DELIVERY, state: 'fail', title: 'Image Delivery' },
                },
            };
    ...
    describe('getLabelName', () => {
        ...
        it('returns the label itself for prototype properties', async function () {
            const parsedTrace = await TraceLoader.traceEngine(this, 'lcp-discovery-delay.json.gz');
            const focus = AIContext.AgentFocus.fromParsedTrace(parsedTrace);
            assert.strictEqual(PerformanceAgent.getLabelName('toString', focus), 'toString');
        });
    });
    describe('clearCache', () => {
