...
describe('BuiltInAi', () => {
    ...
    it('can download the AI model', async () => {
        ...
        const downloadProgressPromise = new Promise(resolve => {
            builtInAi.addEventListener("downloadProgressChanged" /* AiAssistanceModel.BuiltInAi.Events.DOWNLOAD_PROGRESS_CHANGED */, () => {
                assert.strictEqual(builtInAi.getDownloadProgress(), 0.4);
                resolve();
            });
        });
        const sessionCreatedPromise = new Promise(resolve => {
            builtInAi.addEventListener("downloadedAndSessionCreated" /* AiAssistanceModel.BuiltInAi.Events.DOWNLOADED_AND_SESSION_CREATED */, () => {
                resolve();
            });
        });
