...
describeWithEnvironment('PerformanceInsightFormatter', () => {
    ...
    describe('Formatting TraceEvents', () => {
        it('formats network requests that have redirects', async function () {
            ...
            assert.isOk(request);
            const output = TraceEventFormatter.networkRequests(PERF_AGENT_UNIT_FORMATTERS, [request], parsedTrace, { verbose: true });
...
`);
        ...
        it('getNetworkRequestsNewFormat correctly formats network requests for bad request latency trace', async function () {
            ...
            const output = TraceEventFormatter.networkRequests(PERF_AGENT_UNIT_FORMATTERS, [requests[0], requests[0]], parsedTrace);
            ...
            assert.strictEqual(fields[17], '[[1|3.0 ms|512.0 ms],[2|515.1 ms|505.7 ms],[3|1,020.7 ms|507.1 ms]]', 'redirects');
            assert.strictEqual(fields[18], '[Transfer-Encoding: chunked|Keep-Alive: <redacted>|Date: Tue, 11 Mar 2025 10:19:12 GMT|Content-Type: text/html|Connection: keep-alive]\n0;3.0 ms;1,529.5 ms;3,532.6 ms;3,537.8 ms;3,534.7 ms;0.1 ms;5.1 ms;200;text/html;VeryHigh;VeryHigh;VeryHigh;f;http/1.1;f;;[[1|3.0 ms|512.0 ms],[2|515.1 ms|505.7 ms],[3|1,020.7 ms|507.1 ms]];[Transfer-Encoding: chunked|Keep-Alive: <redacted>|Date: Tue, 11 Mar 2025 10:19:12 GMT|Content-Type: text/html|Connection: keep-alive]', 'responseHeaders');
        });
