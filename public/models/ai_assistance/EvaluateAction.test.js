...
describe('FreestylerEvaluateAction', () => {
    ...
    describe('serialization', () => {
        it('should serialize primitive values correctly', async () => {
            assert.strictEqual(await AiAssistance.stringifyRemoteObject(new SDK.RemoteObject.LocalJSONObject('string')), '\'string\'');
            ...
            assert.strictEqual(await AiAssistance.stringifyRemoteObject(new SDK.RemoteObject.LocalJSONObject(undefined)), 'undefined');
            assert.strictEqual(await AiAssistance.stringifyRemoteObject(new SDK.RemoteObject.LocalJSONObject(42)), '42');
            assert.strictEqual(await AiAssistance.stringifyRemoteObject(new SDK.RemoteObject.LocalJSONObject(Symbol('sym'))), 'Symbol(sym)');
        });
        ...
        describe('HTMLElement', () => {
            ...
            it('should serialize node with classes', async () => {
                ...
                assert.strictEqual(AiAssistance.stringifyObjectOnThePage.apply(el), '"<div class=\\"section section-main\\"></div>"');
            });
            ...
            it('should serialize node with class and id', async () => {
                ...
                el.id = 'promotion-section';
                el.classList.add('section');
                assert.strictEqual(AiAssistance.stringifyObjectOnThePage.apply(el), '"<div id=\\"promotion-section\\" class=\\"section\\"></div>"');
            });
        ...
        it('should serialize objects correctly', async () => {
            ...
            assert.strictEqual(AiAssistance.stringifyObjectOnThePage.apply({ key: 'str', secondKey: 'str2' }), '{"key":"str","secondKey":"str2"}');
            assert.strictEqual(AiAssistance.stringifyObjectOnThePage.apply({ key: 1 }), '{"key":1}');
        });
