...
describe('FreestylerEvaluateAction', () => {
    describe('error handling', () => {
        function executeWithResult(mockResult, pausedOnBreakpoint = false) {
            ...
            return AiAssistance.EvaluateAction.EvaluateAction.execute('', [], executionContextStub, { throwOnSideEffect: false });
        }
    ...
    describe('serialization', () => {
      ...
      at Object.anotherFunc (http://localhost/file2.js:56:78)`;
                assert.strictEqual(AiAssistance.EvaluateAction.EvaluateAction.getExecutedLineFromStack(stack, PAGE_EXPOSED_FUNCTIONS), 11);
            });
            it('handles function names within object methods', () => {
                const stack = `Error: Test
      at async MyClass.myMethod (http://localhost/my_class.js:45:12)`;
                assert.strictEqual(AiAssistance.EvaluateAction.EvaluateAction.getExecutedLineFromStack(stack, PAGE_EXPOSED_FUNCTIONS), 44);
            });
            it('handles line and column numbers with parentheses', () => {
                const stack = `Error: Test
      at foo (C:/path/to/file.ts:15:20)`;
                assert.strictEqual(AiAssistance.EvaluateAction.EvaluateAction.getExecutedLineFromStack(stack, PAGE_EXPOSED_FUNCTIONS), 14);
            });
            it('handles line numbers without column numbers', () => {
                const stack = `Error: Test
      at bar (/path/to/another.js:22)`;
                assert.strictEqual(AiAssistance.EvaluateAction.EvaluateAction.getExecutedLineFromStack(stack, PAGE_EXPOSED_FUNCTIONS), 21);
            });
      ...
      at async main (http://localhost/main.js:10:1)`;
                assert.strictEqual(AiAssistance.EvaluateAction.EvaluateAction.getExecutedLineFromStack(stack, PAGE_EXPOSED_FUNCTIONS), 100);
            });
            it('returns null if all relevant lines are in the list of page exposed functions', () => {
                const stack = `Error: Test
      at async setElementStyles (tslib_es6.js:113:24)
      at async myCustomBinding (runtime.js:312:22)`;
                assert.isNull(AiAssistance.EvaluateAction.EvaluateAction.getExecutedLineFromStack(stack, PAGE_EXPOSED_FUNCTIONS));
            });
      ...
      at <anonymous> (http://localhost/file.js:20:10)`;
                assert.strictEqual(AiAssistance.EvaluateAction.EvaluateAction.getExecutedLineFromStack(stack, PAGE_EXPOSED_FUNCTIONS), 19);
            });
...
Cannot read properties of undefined (reading 'bar')`);
            ...
            it('should serialize only the message if no line number is returned', () => {
                const getExecutedLineFromStackStub = sinon.stub(AiAssistance.EvaluateAction.EvaluateAction, 'getExecutedLineFromStack');
        ...
        describe('stringifyRemoteObject', () => {
            ...
            it('should serialize primitive values correctly', async () => {
                assert.strictEqual(await AiAssistance.EvaluateAction.stringifyRemoteObject(new SDK.RemoteObject.LocalJSONObject('string'), exampleCode), '\'string\'');
                assert.strictEqual(await AiAssistance.EvaluateAction.stringifyRemoteObject(new SDK.RemoteObject.LocalJSONObject(999n), exampleCode), '999n');
                assert.strictEqual(await AiAssistance.EvaluateAction.stringifyRemoteObject(new SDK.RemoteObject.LocalJSONObject(true), exampleCode), 'true');
                assert.strictEqual(await AiAssistance.EvaluateAction.stringifyRemoteObject(new SDK.RemoteObject.LocalJSONObject(undefined), exampleCode), 'undefined');
                assert.strictEqual(await AiAssistance.EvaluateAction.stringifyRemoteObject(new SDK.RemoteObject.LocalJSONObject(42), exampleCode), '42');
                assert.strictEqual(await AiAssistance.EvaluateAction.stringifyRemoteObject(new SDK.RemoteObject.LocalJSONObject(Symbol('sym')), exampleCode), 'Symbol(sym)');
            });
        });
        it('runs stringification on the page for objects', async () => {
            ...
            sinon.assert.calledOnceWithExactly(callFunctionStub, AiAssistance.EvaluateAction.stringifyObjectOnThePage, undefined, { throwOnSideEffect: true });
        });
        describe('HTMLElement', () => {
            ...
            it('should serialize node with classes', async () => {
                ...
                assert.strictEqual(AiAssistance.EvaluateAction.stringifyObjectOnThePage.apply(el), '"<div class=\\"section section-main\\"></div>"');
            });
            it('should serialize node with id', async () => {
                const el = document.createElement('div');
                el.id = 'promotion-section';
                assert.strictEqual(AiAssistance.EvaluateAction.stringifyObjectOnThePage.apply(el), '"<div id=\\"promotion-section\\"></div>"');
            });
            it('should serialize node with class and id', async () => {
                const el = document.createElement('div');
                el.id = 'promotion-section';
                el.classList.add('section');
                assert.strictEqual(AiAssistance.EvaluateAction.stringifyObjectOnThePage.apply(el), '"<div id=\\"promotion-section\\" class=\\"section\\"></div>"');
            });
        ...
        it('should serialize objects correctly', async () => {
            ...
            assert.strictEqual(AiAssistance.EvaluateAction.stringifyObjectOnThePage.apply({ key: 'str', secondKey: 'str2' }), '{"key":"str","secondKey":"str2"}');
            assert.strictEqual(AiAssistance.EvaluateAction.stringifyObjectOnThePage.apply({ key: 1 }), '{"key":1}');
        });
