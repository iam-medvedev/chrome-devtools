...
describe('FreestylerEvaluateAction', () => {
    ...
    describe('serialization', () => {
        it('should serialize primitive values correctly', async () => {
            assert.strictEqual(await EvaluateAction.stringifyRemoteObject(new SDK.RemoteObject.LocalJSONObject('string')), '\'string\'');
            assert.strictEqual(await EvaluateAction.stringifyRemoteObject(new SDK.RemoteObject.LocalJSONObject(999n)), '999n');
            assert.strictEqual(await EvaluateAction.stringifyRemoteObject(new SDK.RemoteObject.LocalJSONObject(true)), 'true');
            assert.strictEqual(await EvaluateAction.stringifyRemoteObject(new SDK.RemoteObject.LocalJSONObject(undefined)), 'undefined');
            assert.strictEqual(await EvaluateAction.stringifyRemoteObject(new SDK.RemoteObject.LocalJSONObject(42)), '42');
            assert.strictEqual(await EvaluateAction.stringifyRemoteObject(new SDK.RemoteObject.LocalJSONObject(Symbol('sym'))), 'Symbol(sym)');
        });
        ...
        describe('HTMLElement', () => {
            ...
            it('should serialize node with classes', async () => {
                ...
                assert.strictEqual(EvaluateAction.stringifyObjectOnThePage.apply(el), '"<div class=\\"section section-main\\"></div>"');
            });
            it('should serialize node with id', async () => {
                const el = document.createElement('div');
                el.id = 'promotion-section';
                assert.strictEqual(EvaluateAction.stringifyObjectOnThePage.apply(el), '"<div id=\\"promotion-section\\"></div>"');
            });
            it('should serialize node with class and id', async () => {
                const el = document.createElement('div');
                el.id = 'promotion-section';
                el.classList.add('section');
                assert.strictEqual(EvaluateAction.stringifyObjectOnThePage.apply(el), '"<div id=\\"promotion-section\\" class=\\"section\\"></div>"');
            });
        ...
        it('should serialize objects correctly', async () => {
            ...
            assert.strictEqual(EvaluateAction.stringifyObjectOnThePage.apply({ key: 'str', secondKey: 'str2' }), '{"key":"str","secondKey":"str2"}');
            assert.strictEqual(EvaluateAction.stringifyObjectOnThePage.apply({ key: 1 }), '{"key":1}');
        });
