...
describeWithEnvironment('FileSystemWorkspaceBinding', () => {
    it('does not conflict when file system paths share a prefix', async () => {
        ...
        assert.strictEqual(fooContent.text, 'foo');
    ...
    it('creates file atomically with content', async () => {
        ...
        createFileStub.callsFake(async (path, name) => {
        ...
        const uiSourceCode = await project.createFile('', 'new_file.js', newFileContent);
        ...
        assert.strictEqual(contentData.text, newFileContent);
    });
    describe('FileSystem', () => {
            ...
            const uiSourceCodeRemovedPromise = new Promise(resolve => {
                const listener = (event) => {
                    if (event.data === uiSourceCode) {
                        workspace.removeEventListener(Workspace.Workspace.Events.UISourceCodeRemoved, listener);
                        resolve();
                    }
                };
                workspace.addEventListener(Workspace.Workspace.Events.UISourceCodeRemoved, listener);
            });
            project.deleteFile(uiSourceCode);
            await uiSourceCodeRemovedPromise;
            sinon.assert.callCount(deleteFileStub, 1);
            assert.strictEqual(deleteFileStub.firstCall.args[0], '/script.js');
...
//# sourceMappingURL=FileSystemWorkspaceBinding.test.js.map