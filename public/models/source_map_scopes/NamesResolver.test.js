...
describe('NameResolver', () => {
    ...
    describe('Function name resolving', () => {
        ...
        it('resolves function names at scope start for a profiler frame', async () => {
            ...
            const functionName = await SourceMapScopes.NamesResolver.resolveProfileFrameFunctionName({ scriptId, columnNumber, lineNumber }, target, backend.universe.debuggerWorkspaceBinding);
            assert.strictEqual(functionName, 'unminified');
        });
    });
...
`;
    ...
    describe('getTextFor', () => {
        ...
        it('caches Text instances for UISourceCodes', async () => {
            const { uiSourceCode } = createContentProviderUISourceCode({ target, url: URL, mimeType: 'text/typescript', content: 'console.log(42)' });
