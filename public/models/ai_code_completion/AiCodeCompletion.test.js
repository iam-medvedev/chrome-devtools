...
describeWithEnvironment('AiCodeCompletion', () => {
    ...
    it('dispatches a suggestion to the editor when AIDA returns one', async () => {
        ...
        assert.deepEqual(editor.dispatch.firstCall.args[0], { effects: TextEditor.Config.setAiAutoCompleteSuggestion.of({ text: 'suggestion', from: 1 }) });
    });
    ...
    it('does not dispatch suggestion or citation if recitation action is BLOCK', async () => {
        ...
        sinon.assert.calledWith(dispatchSpy, sinon.match("ResponseReceived" /* AiCodeCompletion.Events.RESPONSE_RECEIVED */), sinon.match({}));
    });
