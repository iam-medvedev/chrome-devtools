...
describeWithEnvironment('AiCodeCompletion', () => {
    ...
    it('builds a request and calls the AIDA client on text changed', async () => {
        ...
        const aiCodeCompletion = new AiCodeCompletion.AiCodeCompletion({ aidaClient: mockAidaClient }, sinon.createStubInstance(TextEditor.TextEditor.TextEditor), throttler);
    ...
    it('dispatches a suggestion to the editor when AIDA returns one', async () => {
        ...
        const aiCodeCompletion = new AiCodeCompletion.AiCodeCompletion({ aidaClient: mockAidaClient }, editor, throttler);
        aiCodeCompletion.onTextChanged('prefix', '\n');
        await throttler.processCompleted;
        sinon.assert.calledOnce(mockAidaClient.completeCode);
        sinon.assert.calledOnce(editor.dispatch);
        assert.deepEqual(editor.dispatch.firstCall.args[0], { effects: TextEditor.Config.setAiAutoCompleteSuggestion.of('suggestion') });
    });
