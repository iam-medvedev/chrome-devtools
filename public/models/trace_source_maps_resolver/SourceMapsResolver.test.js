...
describeWithMockConnection('SourceMapsResolver', () => {
    describe('function name resolving', () => {
        ...
        it('renames nodes from the profile models when the corresponding scripts and source maps have loaded', async function () {
            ...
            assert.strictEqual(Trace.Handlers.ModelHandlers.Samples.getProfileCallFunctionName(parsedTrace.data.Samples, profileCallForNameResolving), AUTHORED_FUNCTION_NAME);
            // Ensure we populate the cache
            assert.strictEqual(SourceMapsResolver.resolvedCodeLocationForEntry(profileCallForNameResolving)?.name, AUTHORED_FUNCTION_NAME);
        });
    ...
    describe('updating entity mapping', () => {
        it('correctly updates mapping for event that maps to a script', async function () {
            ...
            const profileCallUnmapped = makeProfileCall('function', 10, 100, Trace.Types.Events.ProcessID(1), Trace.Types.Events.ThreadID(1));
