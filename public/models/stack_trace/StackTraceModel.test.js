...
describe('StackTraceModel', () => {
    ...
    describe('createFromProtocolRuntime', () => {
        ...
        it('correctly handles a async fragments from different targets', async () => {
            ...
            sinon.stub(model1, 'fetchAsyncStackTrace').returns(Promise.resolve({
                ...
                parentId: { id: 'async-fragment-2', debuggerId: model2.debuggerId() },
            }));
            ...
            const stackTrace = await model.createFromProtocolRuntime({
                ...
                parentId: { id: 'async-fragment-1', debuggerId: model1.debuggerId() },
            }, identityTranslateFn);
            assert.strictEqual(stringifyStackTrace(stackTrace), [
                'at foo (foo.js:1:10)',
                'at bar (foo.js:2:20)',
        ...
        it('translates different stack traces sequentially', async () => {
            ...
            const delayedTranslateFn = async (frames, target) => {
    ...
    describe('scriptInfoChanged', () => {
        const createUpdatedSpy = (stackTrace) => {
            ...
            stackTrace.addEventListener("UPDATED" /* StackTrace.StackTrace.Events.UPDATED */, updatedSpy);
            return updatedSpy;
        };
        ...
        it('does nothing if no fragments are affected', async () => {
            ...
            const stackTrace = await model.createFromProtocolRuntime({ callFrames }, identityTranslateFn);
            const updatedSpy = createUpdatedSpy(stackTrace);
            const script = { scriptId: 'otherScriptId', sourceURL: 'bar.js' };
            await model.scriptInfoChanged(script, translateSpy);
            sinon.assert.notCalled(translateSpy);
            sinon.assert.notCalled(updatedSpy);
        });
        it('does not re-translate fragments that are a complete sub-set of another fragment (but notifies stack traces)', async () => {
            const { model, translateSpy } = setup();
            ...
            const fullStackTrace = await model.createFromProtocolRuntime({ callFrames: fullCallFrames }, identityTranslateFn);
            const subSetStackTrace = await model.createFromProtocolRuntime({ callFrames: subSetFrames }, identityTranslateFn);
    ...
    describe('createFromDebuggerPaused', () => {
        it('assigns the right DebuggerModel.CallFrame to the right StackTrace.Frame', async () => {
            ...
            const details = new SDK.DebuggerModel.DebuggerPausedDetails(debuggerModel, [
                'foo.js:id1:foo:1:10',
                'bar.js:id2:bar:2:20',
            ].map(debuggerCallFrame), "other" /* Protocol.Debugger.PausedEventReason.Other */, undefined, []);
    ...
    describe('createFromErrorStackLikeString', () => {
        ...
        it('correctly handles a stack trace with sync and async fragments', async () => {
            ...
            const stackTrace = await model.createFromErrorStackLikeString(`Error: foo
              at foo (foo.js:1:10)
              at bar (foo.js:2:20)`, identityTranslateFn, {
                exceptionId: 1,
        ...
        it('correctly translates evalOrigin frames', async () => {
            const { model } = setup();
            const translateFn = (frames, _target) => {
        ...
        it('correctly translates complex recursive nested evalOrigin frames', async () => {
            ...
            const stackTrace = await model.createFromErrorStackLikeString(`Error: foo
              at end (eval at intermediate2 (eval at intermediate1 (eval at baseCaller (foo.js:10:5))), <anonymous>:1:1)`, translateFn);
            assert.exists(stackTrace);
...
//# sourceMappingURL=StackTraceModel.test.js.map