...
describeWithEnvironment('emulatedDevices', () => {
    ...
    it('drops user agent metadata when the user agent string is empty', () => {
        const rawDevice = structuredClone(EmulationModel.EmulatedDevices.EmulatedDevicesList.rawEmulatedDevicesForTest()[0]);
    ...
    it('does not serialize user agent metadata when the user agent string is empty', () => {
        ...
        device.userAgent = '';
        device.userAgentMetadata = { mobile: false };
    ...
    it('round-trips safe-area-insets through toJSON and omits the key when absent', () => {
        const withSafeArea = structuredClone(EmulationModel.EmulatedDevices.EmulatedDevicesList.rawEmulatedDevicesForTest()[0]);
        withSafeArea['modes'] = [{
                title: 'default',
                orientation: 'vertical',
                insets: { left: 0, top: 0, right: 0, bottom: 0 },
        ...
        const withoutSafeArea = structuredClone(EmulationModel.EmulatedDevices.EmulatedDevicesList.rawEmulatedDevicesForTest()[0]);
    ...
    it('round-trips notch cutout geometry through parse and toJSON', () => {
        ...
        const json = device?.toJSON();
        assert.deepEqual(json.modes[0].cutout, { shape: 'notch', x: 114, y: 0, width: 162, height: 34, 'upper-radius': 5, 'lower-radius': 22 });
    });
    it('round-trips circle cutout geometry through parse and toJSON', () => {
        ...
        const json = device?.toJSON();
        assert.deepEqual(json.modes[0].cutout, { shape: 'circle', x: 162, y: 0, width: 37, height: 58, cx: 180, cy: 29, radius: 14 });
    });
    ...
    it('correctly categorizes devices into form factors', () => {
        ...
        assert.strictEqual(EmulationModel.EmulatedDevices.deviceCategory(phoneDevice), "mobile" /* EmulationModel.EmulatedDevices.Category.MOBILE */);
        ...
        assert.strictEqual(EmulationModel.EmulatedDevices.deviceCategory(foldableDevice), "foldable" /* EmulationModel.EmulatedDevices.Category.FOLDABLE */);
        ...
        assert.strictEqual(EmulationModel.EmulatedDevices.deviceCategory(tabletDevice), "tablet_desktop" /* EmulationModel.EmulatedDevices.Category.TABLET_DESKTOP */);
        ...
        assert.strictEqual(EmulationModel.EmulatedDevices.deviceCategory(smartDisplayDevice), "smart_display" /* EmulationModel.EmulatedDevices.Category.SMART_DISPLAY */);
    });
});
