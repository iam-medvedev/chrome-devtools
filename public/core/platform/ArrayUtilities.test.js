                ...
                for (let right = left, r = fixture.length; right < r; ++right) {
                    for (let first = left; first <= right; ++first) {
                        for (let count = 1, k = right - first + 1; count <= k; ++count) {
                            ...
                            assert.deepEqual(fixture.slice(0, left), actual.slice(0, left), 'left ' + left + ' ' + right + ' ' + count);
                            assert.deepEqual(fixture.slice(right + 1), actual.slice(right + 1), 'right ' + left + ' ' + right + ' ' + count);
                            const middle = fixture.slice(left, right + 1);
                            middle.sort(comparator);
                            assert.deepEqual(middle.slice(first - left, first - left + count), actual.slice(first, first + count), 'sorted ' + left + ' ' + right + ' ' + first + ' ' + count);
                            const actualRest = actual.slice(first + count, right + 1);
                            actualRest.sort(comparator);
                            assert.deepEqual(middle.slice(first - left + count), actualRest, 'unsorted ' + left + ' ' + right + ' ' + first + ' ' + count);
                        }
                    }
                }
            }
