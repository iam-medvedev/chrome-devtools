...
describe('PuppeteerDevToolsConnection', () => {
    ...
    beforeEach(() => {
        ...
        cdpConnection = new PuppeteerDevToolsConnection(rootSession);
    });
    ...
    it('sends command to a known child session', async () => {
        ...
        const response = await cdpConnection.send('Browser.getVersion', undefined, 'child-1');
    ...
    it('returns SESSION_NOT_FOUND error when sending to unknown session ID', async () => {
        const response = await cdpConnection.send('Browser.getVersion', undefined, 'unknown-session');
        assert.isTrue('error' in response);
        if ('error' in response) {
    ...
    it('throws synchronously when attempting to send on root session', () => {
        assert.throws(() => {
            void cdpConnection.send('Browser.getVersion', undefined, undefined);
...
//# sourceMappingURL=PuppeteerDevToolsConnection.test.js.map