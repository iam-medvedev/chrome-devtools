    ...
    describe('dispatch', () => {
        it('logs a protocol error for unknown domains', () => {
            ...
            const message = {
                ...
                method: 'WrongDomain.somethingStrange',
            };
            target.dispatch(message);
            sinon.assert.calledOnceWithExactly(reportStub, 'Protocol Error: the message WrongDomain.somethingStrange is for non-existing domain \'WrongDomain\'', message);
        });
        it('logs a protocol error for unknown methods', () => {
            ...
            const message = {
                sessionId: 'session ID',
                method: 'Runtime.somethingStrange',
            };
            target.dispatch(message);
            sinon.assert.calledOnceWithExactly(reportStub, 'Protocol Warning: Attempted to dispatch an unspecified event \'Runtime.somethingStrange\'', message);
        });
    });
});
//# sourceMappingURL=InspectorBackend.test.js.map