...
describe('NetworkManager', () => {
    ...
    describe('request post data', () => {
        async function createPostRequestWithHeaders(headers, universe, connection) {
    ...
    describe('Direct TCP socket handling', () => {
        it('on CDP created event creates request ', () => {
            ...
            const req = startedRequests[0];
            assert.strictEqual(req.requestId(), 'mockId');
        ...
        describe('on CDP chunk event', () => {
            ...
            let updatedRequests;
            let targetRequest;
            const mockIdentifier = 'mockParamChunkId';
            ...
            testCases.forEach(testCase => {
                it(testCase.description, () => {
                    ...
                    assert.strictEqual(req.responseReceivedTime, testCase.eventPayload.timestamp, 'responseReceivedTime should be updated');
    ...
    describe('Direct UDP socket handling', () => {
        describe('on CDP created event', () => {
            it('creates request for connected UDP', () => {
                ...
                networkDispatcher.directUDPSocketCreated({
                    identifier: 'mockUdpId1',
                    options: {
                        remoteAddr: 'example.com',
                        remotePort: 2001,
                        sendBufferSize: 2003,
                        receiveBufferSize: 2004,
                        dnsQueryType: "ipv6" /* Protocol.Network.DirectSocketDnsQueryType.Ipv6 */,
                    },
                    timestamp: 2000,
                });
                assert.lengthOf(startedRequests, 1);
                const req = startedRequests[0];
                assert.strictEqual(req.requestId(), 'mockUdpId1');
            ...
            it('creates request for bound UDP', () => {
                ...
                assert.lengthOf(startedRequests, 1);
                const req = startedRequests[0];
                assert.strictEqual(req.requestId(), 'mockUdpId2');
        ...
        describe('on CDP chunk event', () => {
            ...
            let updatedRequests;
            let targetRequest;
            const mockIdentifier = 'mockUdpChunkId';
            ...
            const testCases = [
                ...
                },
                {
                    ...
                    eventPayload: { message: { data: 'Ym91bmQgc2VudA==', remoteAddr: '10.0.0.1', remotePort: 4000 }, timestamp: 3150 },
                    expectedChunk: {
                        type: SDK.NetworkRequest.DirectSocketChunkType.SEND,
                        data: 'Ym91bmQgc2VudA==',
                        timestamp: 3150,
                ...
                },
                {
                    ...
                    eventPayload: { message: { data: 'Ym91bmQgcmVjZWl2ZWQ=', remoteAddr: '10.0.0.3', remotePort: 4001 }, timestamp: 3250 },
                    expectedChunk: {
                        type: SDK.NetworkRequest.DirectSocketChunkType.RECEIVE,
                    ...
                    switch (testCase.expectedChunk.type) {
                        case SDK.NetworkRequest.DirectSocketChunkType.SEND:
                            networkDispatcher.directUDPSocketChunkSent({
                                identifier: mockIdentifier,
                                message: testCase.eventPayload.message,
                                timestamp: testCase.eventPayload.timestamp,
                            });
                            break;
                        case SDK.NetworkRequest.DirectSocketChunkType.RECEIVE:
        ...
        describe('on Join Multicast Group events', () => {
            ...
            let updatedRequests;
            let request;
            const mockIdentifier = 'mockUdpMulticastId';
            beforeEach(() => {
                networkManager = new SDK.NetworkManager.NetworkManager(new TestUniverse().createTarget({}));
                networkDispatcher = new SDK.NetworkManager.NetworkDispatcher(networkManager);
                updatedRequests = [];
            ...
            it('directUDPSocketJoinedMulticastGroup adds subsequent groups', () => {
                ...
                assert.deepEqual(request.directSocketInfo?.joinedMulticastGroups, new Set(['237.132.100.17', '237.132.100.18']));
                assert.lengthOf(updatedRequests, 1);
                assert.strictEqual(updatedRequests[0], request);
            });
    ...
    describe('canResendRequest', () => {
        ...
        setupSettingsHooks();
        setupRuntimeHooks();
        function createRequestWithType(universe, connection, resourceType) {
            const networkManager = new SDK.NetworkManager.NetworkManager(universe.createTarget({ connection }));
            const requestStartedPromise = networkManager.once(SDK.NetworkManager.Events.RequestStarted);
            networkManager.dispatcher.requestWillBeSent({
                requestId: 'req-' + resourceType,
                loaderId: 'loader-1',
                documentURL: 'https://example.test/',
        ...
        it('fullFidelity parameter distinguishes partial-fidelity types', async () => {
            const universe = new TestUniverse();
            const connection = new MockCDPConnection();
            const request = await createRequestWithType(universe, connection, "Document" /* Protocol.Network.ResourceType.Document */);
            assert.isFalse(SDK.NetworkManager.NetworkManager.canResendRequest(request, true));
...
describe('MultitargetNetworkManager', () => {
    ...
    it('blocking settings are consistent after change', async () => {
        ...
        multitargetNetworkManager.requestConditions.add(SDK.NetworkManager.RequestCondition.createFromSetting({ url: 'example.com', enabled: true }, universe.settings));
    ...
    it('blocking settings allow deleting an item in the middle of the list', () => {
        ...
        const condition1 = SDK.NetworkManager.RequestCondition.createFromSetting({ url: 'url1', enabled: true }, universe.settings);
        const condition2 = SDK.NetworkManager.RequestCondition.createFromSetting({ url: 'url2', enabled: true }, universe.settings);
        const condition3 = SDK.NetworkManager.RequestCondition.createFromSetting({ url: 'url3', enabled: true }, universe.settings);
    ...
    it('calls the request conditions model for global throttling if individual request throttling is enabled', () => {
        ...
        const emulateNetworkConditions = sinon.stub(targetManager.target().networkAgent(), 'invoke_emulateNetworkConditions');
...
describe('RequestURLPattern', () => {
    it('successfully upgrades url block patterns from wildcards', () => {
        const testPattern = (pattern, expectation) => {
            ...
            const relevantProperties = urlPattern?.pattern &&
                Object.assign({}, ...keys.map(key => key in urlPattern.pattern && urlPattern.pattern[key] !== '*' ?
                    { [key]: urlPattern.pattern[key] } :
                    {}));
            assert.deepEqual(relevantProperties, expectation, pattern);
        };
        testPattern('http://example.com/foo/bar', { port: '', protocol: 'http', hostname: 'example.com', pathname: '/foo/bar' });
        testPattern('http://example.com', { port: '', protocol: 'http', hostname: 'example.com' });
        testPattern('http://example.com/', { port: '', protocol: 'http', hostname: 'example.com', pathname: '/' });
        testPattern('*://example.com', { port: '', hostname: 'example.com' });
        testPattern('example.com', { port: '', hostname: 'example.com*' });
        testPattern('example.com/foo/bar', { port: '', hostname: 'example.com', pathname: '/foo/bar*' });
        testPattern('http://*.com/foo', { port: '', protocol: 'http', hostname: '*.com', pathname: '/foo' });
        testPattern('http://localhost:*/', { protocol: 'http', hostname: 'localhost', pathname: '/' });
        testPattern('http://localhost:1234', { port: '1234', protocol: 'http', hostname: 'localhost' });
        testPattern('localhost:1234', { port: '1234', hostname: 'localhost' });
        testPattern('http://example.com*', { port: '', protocol: 'http', hostname: 'example.com*' });
        testPattern('e*m', { port: '', hostname: 'e*m*' });
        testPattern('ht tp://*', undefined);
        testPattern('http://*/(:id)', undefined);
    });
    it('correctly reports pattern constructor string validity', () => {
        assert.strictEqual(SDK.NetworkManager.RequestURLPattern.isValidPattern('ht tp://*'), "failed-to-parse" /* SDK.NetworkManager.RequestURLPatternValidity.FAILED_TO_PARSE */);
        assert.strictEqual(SDK.NetworkManager.RequestURLPattern.isValidPattern('http://*/(:id)'), "has-regexp-groups" /* SDK.NetworkManager.RequestURLPatternValidity.HAS_REGEXP_GROUPS */);
        assert.strictEqual(SDK.NetworkManager.RequestURLPattern.isValidPattern('http://*/*'), "valid" /* SDK.NetworkManager.RequestURLPatternValidity.VALID */);
    });
});
describe('RequestConditions', () => {
    ...
    it('stores settings correctly', () => {
        const setting = universe.settings.createSetting('network-blocked-patterns', []);
        const conditions = new SDK.NetworkManager.RequestConditions(universe.settings);
    ...
    describe('applyConditions', () => {
        ...
        it('applies blocking, global, and local throttling if individual request throttling is enabled', () => {
            ...
            conditions.add(SDK.NetworkManager.RequestCondition.createFromSetting({ url: 'foo', enabled: true }, universe.settings));
            conditions.add(SDK.NetworkManager.RequestCondition.createFromSetting({ url: 'bar', enabled: false }, universe.settings));
        ...
        it('disables throttling and blocking when the effect gets disabled globally', () => {
            ...
            sinon.assert.notCalled(emulateNetworkConditions);
            sinon.assert.calledOnceWithExactly(emulateNetworkConditionsByRule, { offline: false, emulateOfflineServiceWorker: false, matchedNetworkConditions: [] });
...
describe('NetworkDispatcher', () => {
    ...
    describe('request', () => {
        ...
        it('response headers are overwritten by request interception', () => {
            ...
            universe.multitargetNetworkManager.dispatchEventToListeners("RequestIntercepted" /* SDK.NetworkManager.MultitargetNetworkManager.Events.REQUEST_INTERCEPTED */, 'mockId');
...
describe('InterceptedRequest', () => {
    ...
    async function checkRequestOverride(target, request, requestId, responseStatusCode, responseHeaders, responseBody, expectedOverriddenResponse, expectedSetCookieHeaders = []) {
        ...
        const interceptedRequest = new SDK.NetworkManager.InterceptedRequest(multitargetNetworkManager, fetchAgent, request, "Document" /* Protocol.Network.ResourceType.Document */, requestId, networkRequest, responseStatusCode, filteredResponseHeaders);
    ...
    it('does not intercept OPTIONS requests', async () => {
        ...
        const networkRequest = SDK.NetworkRequest.NetworkRequest.create(requestId, urlString `${request.url}`, urlString `${request.url}`, null, null, null);
        const interceptedRequest = new SDK.NetworkManager.InterceptedRequest(universe.multitargetNetworkManager, fetchAgent, request, "Document" /* Protocol.Network.ResourceType.Document */, requestId, networkRequest);
        interceptedRequest.responseBody = async () => {
    ...
    describe('NetworkPersistenceManager', () => {
        it('decodes the intercepted response body with the right charset', async () => {
            ...
            const networkProject = new Bindings.ContentProviderBasedProject.ContentProviderBasedProject(universe.workspace, 'testing-network', Workspace.Workspace.projectTypes.Network, 'Override network project', false);
            ...
            const interceptedRequest = new SDK.NetworkManager.InterceptedRequest(universe.multitargetNetworkManager, fetchAgent, request, "Document" /* Protocol.Network.ResourceType.Document */, requestId, networkRequest, 200, [{ name: 'content-type', value: 'text/html; charset-utf-16' }]);
            interceptedRequest.responseBody = async () => {
    ...
    describe('getRecommendedNetworkPreset', () => {
        ...
        it('should handle boundary conditions correctly', () => {
            ...
            assert.strictEqual(slow4GAtFast4GBoundary.key, SDK.NetworkManager.Slow4GConditions.key, 'RTT 106 should be Slow 4G');
            const fast4GAtSlow4GBoundary = SDK.NetworkManager.getRecommendedNetworkPreset(105);
            assert.isNotNull(fast4GAtSlow4GBoundary);
            assert.strictEqual(fast4GAtSlow4GBoundary.key, SDK.NetworkManager.Fast4GConditions.key, 'RTT 105 should be Fast 4G');
            ...
            assert.strictEqual(fast4GAtNoThrottlingBoundary.key, SDK.NetworkManager.Fast4GConditions.key, 'RTT 60 should be Fast 4G');
