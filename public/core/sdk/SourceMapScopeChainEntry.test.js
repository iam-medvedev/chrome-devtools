...
describeWithMockConnection('SourceMapScopeRemoteObject', () => {
    ...
    it('returns "value unavailable" for unavailable scope variables', async () => {
        ...
        const entry = new SDK.SourceMapScopeChainEntry.SourceMapScopeChainEntry(callFrame, originalScope, undefined, false, undefined);
        const { properties } = await entry.object().getAllProperties(/* accessorPropertiesOnly */ false, /* generatePreview */ true);
    ...
    it('resolves variable values using binding expressions and evaluateOnCallFrame', async () => {
        ...
        const entry = new SDK.SourceMapScopeChainEntry.SourceMapScopeChainEntry(callFrame, originalScope, range, false, undefined);
        const { properties } = await entry.object().getAllProperties(/* accessorPropertiesOnly */ false, /* generatePreview */ true);
        assert.isNotNull(properties);
    ...
    it('uses the right binding expression when resolving variable values when ranges are split', async () => {
        ...
        const pauseLocations = [
            new SDK.DebuggerModel.Location(callFrame.debuggerModel, '0', 0, 25),
            new SDK.DebuggerModel.Location(callFrame.debuggerModel, '0', 0, 100),
            new SDK.DebuggerModel.Location(callFrame.debuggerModel, '0', 0, 175),
        ];
...
//# sourceMappingURL=SourceMapScopeChainEntry.test.js.map