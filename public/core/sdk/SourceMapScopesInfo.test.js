...
describe('SourceMapScopesInfo', () => {
    ...
    describe('isOutlinedFrame', () => {
        it('returns false for a global scope', () => {
            const sourceMap = sinon.createStubInstance(SDK.SourceMap.SourceMap);
            const scopeInfo = new SourceMapScopesInfo(sourceMap, new ScopeInfoBuilder().startRange(0, 0).endRange(0, 20).build());
            assert.isFalse(scopeInfo.isOutlinedFrame(0, 10));
        });
        it('returns false for a non-hidden function', () => {
            const sourceMap = sinon.createStubInstance(SDK.SourceMap.SourceMap);
            const scopeInfo = new SourceMapScopesInfo(sourceMap, new ScopeInfoBuilder()
                .startRange(0, 0)
    ...
    describe('isEmpty', () => {
        it('returns true if all original scopes are null', () => {
            const scopeInfo = new SourceMapScopesInfo(sinon.createStubInstance(SDK.SourceMap.SourceMap), { scopes: [], ranges: [] });
