...
describe('SourceMapManager', () => {
    ...
    setupRuntimeHooks();
    setupLocaleHooks();
    function createUniverse(sourceMapContent = content) {
    ...
    it('uses url for a worker\'s source maps from frame', async () => {
        ...
        const mainTarget = universe.createTarget({ id: 'main', name: 'main', type: SDK.Target.Type.FRAME });
    ...
    describe('lazy loading source maps', () => {
        it('defers loading until sourceMapForClientPromise is called when lazyLoading is enabled', async () => {
            ...
            assert.strictEqual(sourceMapManager.sourceMapForClient(client), sourceMap, 'sourceMapForClient should return loaded map');
        });
