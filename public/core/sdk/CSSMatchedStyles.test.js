...
const { assert } = chai;
function ruleMatch(selector, cssProperties) {
    return {
        rule: {
...
describe('CSSMatchedStyles', () => {
    describe('parseCSSVariableNameAndFallback', () => {
        ...
        it('correctly parses simple CSS variables without fallback', () => {
            ...
            assert.deepEqual(parseCSSVariableNameAndFallback('var(\n--foo-bar\n)'), { variableName: '--foo-bar', fallback: '' });
        ...
        it('need to correctly parse escaped characters', () => {
            ...
            assert.deepEqual(parseCSSVariableNameAndFallback('var(--fo\\ o,)'), { variableName: '--fo\\ o', fallback: '' });
            assert.deepEqual(parseCSSVariableNameAndFallback('var(--\\ bar\\ , blue)'), { variableName: '--\\ bar\\', fallback: 'blue' });
            // test \)
            assert.deepEqual(parseCSSVariableNameAndFallback('var( --ba\\ z\\) )'), { variableName: '--ba\\ z\\)', fallback: '' });
            assert.deepEqual(parseCSSVariableNameAndFallback('var( --ba\\ z\\), )'), { variableName: '--ba\\ z\\)', fallback: '' });
            assert.deepEqual(parseCSSVariableNameAndFallback('var( --ba\\ z\\),  90%  )'), { variableName: '--ba\\ z\\)', fallback: '90%  ' });
        });
        it('correctly parses simple CSS variables with fallback', () => {
            ...
            assert.deepEqual(parseCSSVariableNameAndFallback('var(--x-y , 100%  )'), { variableName: '--x-y', fallback: '100%  ' });
            assert.deepEqual(parseCSSVariableNameAndFallback('var(--A123, 666 )'), { variableName: '--A123', fallback: '666 ' });
        });
        ...
        it('correctly parses variables with escaped characters in name', () => {
            ...
            assert.deepEqual(parseCSSVariableNameAndFallback('var(--_-color-blue)'), { variableName: '--_-color-blue', fallback: '' });
            assert.deepEqual(parseCSSVariableNameAndFallback('var(-\-_-color-blue)'), { variableName: '-\-_-color-blue', fallback: '' });
            assert.deepEqual(parseCSSVariableNameAndFallback('var(\--_-color-blue)'), { variableName: '\--_-color-blue', fallback: '' });
            assert.deepEqual(parseCSSVariableNameAndFallback('var(---three_hyphens)'), { variableName: '---three_hyphens', fallback: '' });
        });
    });
    describe('computeCSSVariable', () => {
        const testCssValueEquals = async (text, value) => {
    ...
    describe('computeValue', () => {
        const testComputedValueEquals = async (text, value) => {
            const node = sinon.createStubInstance(SDK.DOMModel.DOMNode);
            node.id = 1;
            const matchedStyles = await SDK.CSSMatchedStyles.CSSMatchedStyles.create({
                cssModel: sinon.createStubInstance(SDK.CSSModel.CSSModel),
        ...
        it('should correctly compute the value of an expression that uses a variable', async () => {
            ...
            await testComputedValueEquals('rgb(100, 200, 300) var(--some-color, blue    ) 1px', 'rgb(100, 200, 300) blue 1px');
