...
describe('CSSPropertyParser', () => {
    ...
    function matchSingleValue(name, value, matcher) {
        ...
        const match = (matchedNode && matchedResult.getMatch(matchedNode)) ?? null;
        return {
            ast,
            match,
...
||| }`);
        ...
        it('computes longhand positions', () => {
            const matchedResult = SDK.CSSPropertyParser.matchDeclaration('--prop', 'a b /* c */ var(--d) e', [new SDK.CSSPropertyParserMatchers.BaseVariableMatcher(match => match.name === '--d' ? 'ddd' : null)]);
            assert.exists(matchedResult);
            const topLevelValues = SDK.CSSPropertyParser.ASTUtils.siblings(SDK.CSSPropertyParser.ASTUtils.declValue(matchedResult.ast.tree));
        ...
        it('parses attrs correctly', () => {
            ...
            for (const succeed of ['attr(a)', 'attr(a, "123")', 'attr(a px, 123px)', 'attr(a %, 123%)', 'attr(a type(<length>))',
                'attr(a type(*), 123)', 'attr(a, attr(b))', 'attr(a, 123, 123)']) {
                const { ast, match, text } = matchSingleValue('width', succeed, new SDK.CSSPropertyParserMatchers.AttributeMatcher(matchedStyles, style, () => null));
                assert.exists(ast, succeed);
                assert.exists(match, text);
                assert.strictEqual(match.text, succeed);
                assert.strictEqual(match.name, 'a');
            ...
            for (const fail of ['attr', 'attr(a', 'attr(a b c)', 'attr(a nottype(<length>))', 'attr(a *)']) {
                const { match, text } = matchSingleValue('width', fail, new SDK.CSSPropertyParserMatchers.AttributeMatcher(matchedStyles, style, () => null));
                assert.isNull(match, text);
            }
        });
