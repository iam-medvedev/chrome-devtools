...
describe('Matchers for SDK.CSSPropertyParser.BottomUpTreeMatching', () => {
    ...
    it('parses light-dark correctly', () => {
        ...
        for (const succeed of ['light-dark(red, blue)', 'light-dark(var(--foo), red)', 'light-dark(red, var(--foo))',
            ...
            const { ast, match, text } = matchSingleValue('color', succeed, new SDK.CSSPropertyParserMatchers.LightDarkColorMatcher());
            assert.exists(ast, text);
            assert.exists(match, text);
        ...
        const { match, text } = matchSingleValue('width', 'light-dark(red, blue)', new SDK.CSSPropertyParserMatchers.LightDarkColorMatcher());
        assert.isNull(match, text);
    });
    ...
    describe('SelectFunctionMatcher', () => {
        it('matches selecting functions', () => {
            ...
            for (const value of success) {
                const { match, text } = matchSingleValue('width', value, new SDK.CSSPropertyParserMatchers.SelectFunctionMatcher());
                assert.exists(match, text);
