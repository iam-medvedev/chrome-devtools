...
describeWithMockConnection('ConsoleMessage', () => {
    ...
    it('logs a message on main frame navigation', async () => {
        ...
        const navigateTarget = (target) => {
            ...
            const frame = { url: 'http://example.com/', backForwardCacheDetails: {} };
    ...
    it('logs a message on main frame navigation via bfcache', async () => {
        ...
        const navigateTarget = (target) => {
            ...
            assertNotNullOrUndefined(resourceTreeModel);
            resourceTreeModel.dispatchEventToListeners(SDK.ResourceTreeModel.Events.CachedResourcesLoaded, resourceTreeModel);
            const frame = { url: 'http://example.com/', backForwardCacheDetails: { restoredFromCache: true } };
            resourceTreeModel.dispatchEventToListeners(SDK.ResourceTreeModel.Events.PrimaryPageChanged, { frame, type: "Navigation" /* SDK.ResourceTreeModel.PrimaryPageChangeType.Navigation */ });
        };
        navigateTarget(subframeTarget);
        assert.isTrue(consoleLog.notCalled);
        navigateTarget(mainFrameUnderTabTarget);
        assert.isTrue(consoleLog.calledOnce);
        assert.isTrue(consoleLog.calledOnceWith('Navigation to http://example.com/ was restored from back/forward cache (see https://web.dev/bfcache/)'));
        navigateTarget(mainFrameWithoutTabTarget);
        assert.isTrue(consoleLog.calledTwice);
        assert.isTrue(consoleLog.secondCall.calledWith('Navigation to http://example.com/ was restored from back/forward cache (see https://web.dev/bfcache/)'));
    });
