    ...
    describe('updateVersion', () => {
        ...
        it('should have correct method count and names', () => {
            ...
            for (const prop of Object.getOwnPropertyNames(VersionController.prototype)) {
                ...
                const isFunction = typeof versionController[prop] === 'function';
                if (!matches) {
                    continue;
                }
                assert(isFunction, `expected ${prop} to be a function`);
                assert.strictEqual(Number(matches[1]) + 1, Number(matches[2]), `expected ${prop} to be a have version next to each other`);
                updateCount++;
            }
            assert.strictEqual(updateCount, VersionController.CURRENT_VERSION, 'CurrentVersion does not match the number of method needed to run');
        });
