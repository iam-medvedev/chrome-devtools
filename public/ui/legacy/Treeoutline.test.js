...
describe('TreeViewElement', () => {
    ...
    async function makeTree(template) {
    ...
    it('sends a `select` event when a node is selected', async () => {
        ...
        const component = await makeTree(html `<devtools-tree @select=${onSelect} .template=${html `
      <ul role="tree">
...
class TestTreeNode {
    ...
    contentsMap;
    #children = [];
    constructor(contents, contentsMap) {
    ...
    static make(spec, root = new TestTreeNode('', new Map())) {
...
describe('TreeSearch', () => {
    ...
    it('groups results by node', () => {
        ...
        assert.deepEqual(results, [{ node, isPostOrderMatch: false, matchIndexInNode: 0 }, { node, isPostOrderMatch: false, matchIndexInNode: 1 }]);
    });
