...
describe('StackedPane', () => {
    ...
    setupRuntimeHooks();
    let stackedPane;
    let createToolbarStub;
    const createMockView = (id, title) => {
        const view = sinon.createStubInstance(UI.View.SimpleView);
...
describe('ExpandableContainerWidget', () => {
    ...
    beforeEach(() => {
        ...
        mockView.title.returns('Test View');
    ...
    it('should handle materialization race conditions', async () => {
        ...
        const { promise: widgetPromise, resolve: resolveWidget } = Promise.withResolvers();
    ...
    it('should notify via onVisibilityChanged callback', async () => {
        const visibilitySpy = sinon.spy();
        const callbackContainer = new UI.StackedPane.ExpandableContainerWidget(mockView, createToolbarStub, () => { }, visibilitySpy);
        await callbackContainer.expand();
        sinon.assert.calledWith(visibilitySpy, true);
        const title = callbackContainer.element.shadowRoot.querySelector('.expandable-view-title');
        title.dispatchEvent(new Event('click', { bubbles: true }));
        await raf();
        sinon.assert.calledWith(visibilitySpy, false);
    });
});
//# sourceMappingURL=StackedPane.test.js.map