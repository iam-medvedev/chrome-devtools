...
describe('UIUtils', () => {
    ...
    describe('bindToAction', () => {
        ...
        function setup() {
            ...
            const buttonRef = Lit.Directives.createRef();
            Lit.render(html `<devtools-button ${Lit.Directives.ref(buttonRef)} ${bindToAction(actionId)}></devtools-button>`, container);
    ...
    describe('HTMLElementWithLightDOMTemplate', () => {
        ...
        it('observes its contents for modifications', async () => {
            ...
            const nodeContents = (nodes) => nodes.filter(node => node.nodeName !== '#comment').map(({ nodeName, textContent }) => ({ [nodeName]: textContent }));
            container.clear();
        ...
        it('attaches event handlers to nested elements', () => {
            ...
            Lit.render(html `<div @click=${on(divClickHandler)}><button @click=${on(buttonClickHandler)}></button></div>`, container);
