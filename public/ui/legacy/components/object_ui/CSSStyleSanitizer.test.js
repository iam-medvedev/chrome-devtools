...
describe('CSSStyleSanitizer', () => {
    describe('sanitizeStyle', () => {
        ...
        it('blocks block-listed url schemes in values', () => {
            ...
            ObjectUI.CSSStyleSanitizer.sanitizeStyle(styles, 'background-image:url(\'http://localhost/a.png\')');
            assert.isFalse(styles.has('background-image'));
            ObjectUI.CSSStyleSanitizer.sanitizeStyle(styles, 'background-image:url(data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAwAAAAMCAAAAABzHgM7AAAAF0lEQVR42mM4Awb/wYCBYg6EgghRzAEAWDWBGQVyKPMAAAAASUVORK5CYII=), url(http://localhost/a.png)');
            assert.isFalse(styles.has('background-image'));
            ObjectUI.CSSStyleSanitizer.sanitizeStyle(styles, 'background-image:if(supports():"url(data:";else:url(http://localhost/a.png))');
            assert.isFalse(styles.has('background-image'));
            ...
            assert.isFalse(styles.has('background-image'));
            ObjectUI.CSSStyleSanitizer.sanitizeStyle(styles, 'background-image:if(else:image\\-set("data:" 1x, "http://localhost/a.png" 2x))');
            assert.isFalse(styles.has('background-image'));
            ObjectUI.CSSStyleSanitizer.sanitizeStyle(styles, 'background-image:if(else:image-se\\74 ("data:" 1x, "http://localhost/a.png" 2x))');
            assert.isFalse(styles.has('background-image'));
            ObjectUI.CSSStyleSanitizer.sanitizeStyle(styles, 'background-image:image-set("data:" 1x, "http://localhost/a.png" 2x)');
            assert.isFalse(styles.has('background-image'));
        });
        it('allows data urls in values', () => {
            const dataUrl = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAwAAAAMCAAAAABzHgM7AAAAF0lEQVR42mM4Awb/wYCBYg6EgghRzAEAWDWBGQVyKPMAAAAASUVORK5CYII=';
            const styles = new Map();
            ...
            ObjectUI.CSSStyleSanitizer.sanitizeStyle(styles, `background-image:image-set( "${dataUrl}" 1.5x , url("${dataUrl}") type( "image/png" ) )`);
            assert.include(styles.get('background-image').value, 'data:image/png;base64');
        });
    });
});
//# sourceMappingURL=CSSStyleSanitizer.test.js.map