...
export function drawLegends(context, x, y, legends) {
    context.save();
    context.translate(x, y);
    let xForLegend = 0;
 ...
 */
function drawOneLegend(context, x, y, color, category) {
    context.save();
    context.beginPath();
...
function drawRoundedRectangle(context, startX, startY, width, height, borderRadius) {
    context.save();
    ...
    context.bezierCurveTo(endX - bezierCurveHandlerAmount, startY, endX, startY + bezierCurveHandlerAmount, endX, startY + borderRadius);
    ...
    context.bezierCurveTo(endX, endY - bezierCurveHandlerAmount, endX - bezierCurveHandlerAmount, endY, endX - borderRadius, endY);
    // Draw line to bottom left corner
    context.lineTo(startX + borderRadius, endY);
    // Curve round and up from bottom left corner
    context.bezierCurveTo(startX + bezierCurveHandlerAmount, endY, startX, endY - bezierCurveHandlerAmount, startX, endY - borderRadius);
    ...
    context.bezierCurveTo(startX, startY + bezierCurveHandlerAmount, startX + bezierCurveHandlerAmount, startY, startX + borderRadius, startY);
    context.closePath();
    context.restore();
}
