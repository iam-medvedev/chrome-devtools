...
describeWithEnvironment('ColorPicker aka Spectrum', () => {
    ...
    it('uses appropriate stepping and clipping for keyboard/mousewheel interactions', () => {
        const spectrum = new ColorPicker.Spectrum.Spectrum();
        const textInputs = Array.from(spectrum.contentElement.querySelectorAll('.spectrum-text:not(.spectrum-text-hex) .spectrum-text-value'));
        assert.lengthOf(textInputs, 4);
        const increment = (element) => element.dispatchEvent(new WheelEvent('wheel', { deltaX: -1 }));
        const decrement = (element) => element.dispatchEvent(new WheelEvent('wheel', { deltaX: 1 }));
        spectrum.setColor(new Common.Color.ColorFunction("xyz" /* Common.Color.Format.XYZ */, 0.5, 1, 0, 0.3));
        ...
        spectrum.setColor(new Common.Color.ColorFunction("srgb" /* Common.Color.Format.SRGB */, 0.5, 1, 0, 0.3));
        textInputs.forEach(increment);
        ...
        spectrum.setColor(new Common.Color.Legacy([0.5, 1, 0, 0.3], "rgba" /* Common.Color.Format.RGBA */));
    ...
    it('updates hue correctly for near-grayscale colors', () => {
        const spectrum = new ColorPicker.Spectrum.Spectrum();
        const colorElement = spectrum.contentElement.querySelector('.spectrum-color');
        assert.exists(colorElement);
        // Scenario 1: Input #3c3d3d only
        spectrum.setColor(Common.Color.parse('#3c3d3d'));
        ...
        spectrum.setColor(Common.Color.parse('#3d3d3d'));
...
describeWithMockConnection('PaletteGenerator', () => {
    ...
    #00f: {}
    `;
        stylesheet.requestContentData.resolves(new TextUtils.ContentData.ContentData(content, 
