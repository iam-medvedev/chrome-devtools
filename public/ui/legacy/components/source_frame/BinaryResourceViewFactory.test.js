...
describeWithEnvironment('BinaryResourceViewFactory', () => {
    ...
    it('produces views for utf8/base64 that update when the StreamingContentData changes', async () => {
        ...
        assert.strictEqual(base64View.textEditor.state.doc.toString(), 'c2VuZGluZyB0aGlzIHV0Zi04IHN0cmluZyBhcyBhIGJpbmFyeSBtZXNzYWdlLi4u');
        streamingContent.addChunk(window.btoa('\nadded another line'));
        await raf();
        assert.strictEqual(utf8View.textEditor.state.doc.toString(), 'sending this utf-8 string as a binary message...\nadded another line');
        assert.strictEqual(base64View.textEditor.state.doc.toString(), 'c2VuZGluZyB0aGlzIHV0Zi04IHN0cmluZyBhcyBhIGJpbmFyeSBtZXNzYWdlLi4uCmFkZGVkIGFub3RoZXIgbGluZQ==');
        utf8View.detach();
        base64View.detach();
    });
});
