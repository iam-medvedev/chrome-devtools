...
describeWithEnvironment('ViewManager', () => {
    ...
    describe('createTabbedLocation', () => {
        it('respects custom location visibility predicate', async () => {
            ...
            const tabbedLocation = viewManager.createTabbedLocation(() => { }, 'visibility-test-location', false, true, { isLocationVisible: () => locationIsVisible });
        ...
        it('does not include transient views in the `closeable-tabs` setting', async () => {
            ...
            await tabbedLocation.showView(closeableView);
            assert.notProperty(Common.Settings.Settings.instance().settingForTest('closeable-tabs').get(), 'transient-view', 'Transient views must never be included in `closeable-tabs` while they are shown');
        });
        ...
        it('installs the plus button BEFORE appending tabs so the very first overflow detection reserves its width', () => {
            ...
            class CapturingTabbedPane extends UI.TabbedPane.TabbedPane {
                plusButtonPresentAtFirstAppendTab;
                appendTab(...args) {
            ...
            viewManager.createTabbedLocation(() => { }, "panel" /* UI.ViewManager.ViewLocationValues.PANEL */, false, true, { tabbedPaneFactory: () => tabbedPane, plusButton: {} });
            assert.isTrue(tabbedPane.plusButtonPresentAtFirstAppendTab, 'installPlusButton must run before appendApplicableItems mounts any tabs');
        });
    });
    describe('createStackLocation', () => {
        ...
        it('correctly reports visibility based on location and expand state', async () => {
            ...
            await stackLocation.showView(view);
            const pane = stackLocation.widget();
            const container = pane.getContainerForView(view);
            const title = container.element.shadowRoot.querySelector('.expandable-view-title');
