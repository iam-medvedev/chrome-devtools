...
describeWithEnvironment('Widget', () => {
    ...
    describe('constructor', () => {
        ...
        it('correctly sets the `jslog` attribute of the `contentElement`', () => {
            ...
            assert.strictEqual(widget.contentElement.getAttribute('jslog'), 'Panel; context: foo');
        });
        it('correctly sets the `jslog` attribute of the `contentElement` in the presence of Shadow DOM', () => {
            ...
            assert.strictEqual(widget.element.getAttribute('jslog'), 'Section; context: bar');
        });
    });
    ...
    describe('focus', () => {
        ...
        it('does not recursively call focus when the widget element has autofocus', () => {
            ...
            const container = document.createElement('div');
            renderElementIntoDOM(container);
            Lit.render(html `<devtools-widget autofocus tabindex="-1" ${UI.Widget.widget(FocusableWidget)}></devtools-widget>`, container);
            const widgetElement = container.querySelector('devtools-widget');
    ...
    describe('WidgetDirective', () => {
        ...
        it('creates a new Widget when the widget class changes in a ternary operator', () => {
            ...
            const container = document.createElement('div');
            renderElementIntoDOM(container);
            const renderTernary = (condition) => Lit.render(html `${condition ? UI.Widget.widget(WidgetA) : UI.Widget.widget(WidgetB)}`, container);
            renderTernary(true);
            const widgetElement1 = container.querySelector('devtools-widget');
            const widget1 = widgetElement1.getWidget();
            assert.instanceOf(widget1, WidgetA);
            renderTernary(false);
            const widgetElement2 = container.querySelector('devtools-widget');
        ...
        it('passes and updates widget parameters', async () => {
            const container = document.createElement('div');
            renderElementIntoDOM(container);
            Lit.render(html `<devtools-widget ${UI.Widget.widget(TestWidget, { params: { foo: 'bar' } })}></devtools-widget>`, container);
            await new Promise(resolve => setTimeout(resolve, 0));
            const devtoolsWidget = container.querySelector('devtools-widget');
            assert.exists(devtoolsWidget);
            const widget = UI.Widget.Widget.get(devtoolsWidget);
            assert.deepEqual(widget.params, { foo: 'bar' });
            Lit.render(html `<devtools-widget ${UI.Widget.widget(TestWidget, { params: { foo: 'baz' } })}></devtools-widget>`, container);
            await new Promise(resolve => setTimeout(resolve, 0));
            const devtoolsWidget2 = container.querySelector('devtools-widget');
            const widget2 = UI.Widget.Widget.get(devtoolsWidget2);
    ...
    describe('WidgetDirective (non-devtools-widget elements)', () => {
        ...
        it('passes and updates widget parameters', async () => {
            ...
            const widget2 = UI.Widget.Widget.get(span2);
            assert.deepEqual(widget2.params, { foo: 'baz' });
        });
        const testWidgetReuse = (template) => async () => {
            const container = document.createElement('div');
            renderElementIntoDOM(container);
        ...
        it('reuses the widget if the factory reference changes at child directive', testWidgetReuse(() => html `${UI.Widget.widget(e => new TestWidget(e))}`));
        it('reuses the widget if the factory reference changes at <devtools-widget>', testWidgetReuse(() => html `<devtools-widget ${UI.Widget.widget(e => new TestWidget(e))}></devtools-widget>`));
        it('reuses the widget if the factory reference changes at <span>', testWidgetReuse(() => html `<span ${UI.Widget.widget(e => new TestWidget(e))}></span>`));
        it('detaches the widget when the Lit template re-renders and removes it', async () => {
            const container = document.createElement('div');
            renderElementIntoDOM(container);
            Lit.render(html `<span ${UI.Widget.widget(TestWidget)}></span>`, container);
        ...
        it('dispatches DOM events through eventMixin', async () => {
            ...
            Lit.render(html `<devtools-widget ${UI.Widget.widget(EventWidget)} @test-event=${stub}></devtools-widget>`, container);
            const widget = container.firstElementChild?.getWidget();
            assert.isOk(widget);
            widget.trigger();
    ...
    describe('Shadow DOM', () => {
        it('throws error when using DocumentFragment.appendChild with a widget', () => {
            ...
            const widget = new Widget();
            widget.markAsRoot();
            assert.throws(() => fragment.appendChild(widget.element), /Attempt to modify widget with native DOM method `appendChild`/);
        });
        it('keeps child widget in the Shadow Root when using pure shadow DOM', () => {
            const container = document.createElement('div');
            renderElementIntoDOM(container);
            const parentWidget = new Widget({ useShadowDom: 'pure' });
            parentWidget.markAsRoot();
            ...
            assert.strictEqual(childWidget.element.parentNode, shadowRoot, 'Child widget should be a child of the Shadow Root');
            assert.isNull(childWidget.element.parentElement, 'Child widget should have no parentElement (since it is in a Shadow Root)');
        });
    ...
    describe('Universe dependency injection', () => {
        it('passes injected dependencies to child widgets instantiated via RootView when static INJECT is defined', () => {
            let passedTargetManager = null;
            let passedSettings = null;
            class ChildWidget extends Widget {
                static INJECT = [SDK.TargetManager.TargetManager, Common.Settings.Settings];
                constructor(element, [targetManager, settings]) {
                    super(element);
        ...
        it('automatically resolves dependencies via TestUniverse when rendered into DOM', () => {
            let passedTargetManager = null;
            class ChildWidget extends Widget {
                static INJECT = [SDK.TargetManager.TargetManager];
                constructor(element, [targetManager]) {
                    super(element);
                    passedTargetManager = targetManager;
                }
            }
        ...
        it('does not pass dependencies to constructor when static INJECT is empty array by default', () => {
            let passedSecondArg = 'sentinel';
            class DefaultWidget extends Widget {
                constructor(element, secondArg) {
                    super(element);
