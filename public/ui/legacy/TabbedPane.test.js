...
describeWithEnvironment('TabbedPane', () => {
    ...
    it('places role="tab" on the focusable tab element, not on a child', () => {
        ...
        assert.strictEqual(focused.parentElement.getAttribute('role'), 'tablist');
    ...
    it('moveTab reorders the tab and dispatches TabOrderChanged', () => {
        const events = [];
        tabbedPane.addEventListener(UI.TabbedPane.Events.TabOrderChanged, (event) => events.push(event.data.tabId));
...
describeWithEnvironment('TabbedPaneElement', () => {
    ...
    `, container);
        ...
        const widget = UI.Widget.Widget.get(tabbedPaneElement);
        await raf();
        const tabElement = container.querySelector('#tab1');
    ...
    `, container);
        ...
        const tab1 = container.querySelector('#tab1');
        tab1.remove();
        await raf();
        assert.lengthOf(widget.tabs, 1);
    ...
    `, container);
        ...
        assert.strictEqual(rightToolbar.parentElement, widget.headerElement(), 'lazily-created right toolbar should be appended to the header element');
        let updates = 0;
    ...
    `, container);
        ...
        const events = [];
        tabbedPaneElement.addEventListener('overflow-tabs-changed', (event) => {
    ...
    `, container);
        const widget = UI.Widget.Widget.get(container.querySelector('devtools-tabbed-pane'));
        await doubleRaf();
        let count = 0;
    ...
    `, container);
        ...
        widget.measuredDropDownButtonWidth = 999;
        UI.ZoomManager.ZoomManager.instance().dispatchEventToListeners("ZoomChanged" /* UI.ZoomManager.Events.ZOOM_CHANGED */, { from: 1.0, to: 1.5 });
        await doubleRaf();
        const measuredWidth = widget.measuredDropDownButtonWidth;
