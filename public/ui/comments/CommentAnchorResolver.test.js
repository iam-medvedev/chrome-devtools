...
function createTextEditor(doc, filePath) {
...
describeWithEnvironment('CommentAnchorResolver', () => {
    ...
    describe('extractVeName', () => {
        it('extracts trailing VE name from full visual logging path', () => {
            assert.strictEqual(Comments.CommentAnchorResolver.extractVeName('Panel: elements > Pane: styles > TreeItem: color'), 'TreeItem');
    ...
    describe('getSiblingIndex', () => {
        ...
        it('correctly calculates indices across shadow DOM boundaries', () => {
            ...
            assert.strictEqual(Comments.CommentAnchorResolver.getSiblingIndex(item1, 'Panel: shadow-panel > TreeItem: item'), 0);
            assert.strictEqual(Comments.CommentAnchorResolver.getSiblingIndex(item2, 'Panel: shadow-panel > TreeItem: item'), 1);
        });
    });
    ...
    describe('deepQuerySelector', () => {
        ...
        it('finds element inside root element\'s own shadowRoot', () => {
            ...
            container.appendChild(host);
            const matched = Comments.CommentAnchorResolver.deepQuerySelector(host, '[data-network-request-id="req-in-shadow"]');
        ...
        it('finds element inside nested shadow roots within root element\'s shadowRoot', () => {
            ...
            rootShadow.appendChild(nestedHost);
            container.appendChild(rootHost);
            const matched = Comments.CommentAnchorResolver.deepQuerySelector(rootHost, '[data-network-request-id="req-deep-nested"]');
            assert.strictEqual(matched, target);
        });
...
//# sourceMappingURL=CommentAnchorResolver.test.js.map