...
describe('CommentOverlayManager', () => {
    ...
    it('removes comment threads and cleans up DOM observer and pin positions', () => {
        ...
        try {
            ...
            let eventCount = 0;
            manager.addEventListener("PositionsUpdated" /* Comments.CommentOverlayManager.Events.POSITIONS_UPDATED */, () => {
                eventCount++;
            });
    ...
    it('deduplicates HOVER_HIGHLIGHT_CHANGED events when hover data has not changed', () => {
        ...
        container.appendChild(anchorEl);
        let eventCount = 0;
        manager.addEventListener("HoverHighlightChanged" /* Comments.CommentOverlayManager.Events.HOVER_HIGHLIGHT_CHANGED */, () => {
            eventCount++;
        });
