...
describeWithEnvironment('MarkdownView', () => {
    ...
    describe('MarkdownLitRenderer renderToken', () => {
        ...
        it('wraps a codespan token in <code> tags', () => {
            const container = renderTemplateResult(renderer.renderToken(getFakeToken({ type: 'codespan', text: 'const foo = 42;' })));
            const code = container.querySelector('code');
            assert.exists(code);
            assert.deepEqual(code.textContent, 'const foo = 42;');
        });
        it('renders childless text tokens as-is', () => {
            const container = renderTemplateResult(renderer.renderToken(getFakeToken({ type: 'text', text: 'Simple text token' })));
        ...
        it('sets custom classes on the token types', () => {
            ...
            assert.isTrue(container.querySelector('em')?.classList.contains('custom-class'), 'Expected custom-class to be applied');
        });
    });
...
describeWithEnvironment('Issue description smoke test', () => {
    ...
    ISSUE_DESCRIPTIONS.forEach(descriptionFile => {
        it(`renders ${descriptionFile} without throwing`, async () => {
            ...
            descriptionContent = descriptionContent.replaceAll(/\{(PLACEHOLDER_[a-zA-Z][a-zA-Z0-9]*)\}/g, '$1'); // Identity substitute placeholders.
            const issueDescription = IssuesManager.MarkdownIssueDescription.createIssueDescriptionFromRawMarkdown(descriptionContent, {
    ...
    Object.keys(Deprecation.DEPRECATIONS_METADATA).forEach(deprecation => {
        ...
        it(`renders the deprecation description for ${deprecation} without throwing`, async () => {
            const description = Deprecation.UIStrings[deprecation];
