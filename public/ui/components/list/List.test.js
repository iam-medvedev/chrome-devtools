...
async function renderListComponent(items, editable, deletable) {
...
describe('List', () => {
    ...
    `, true, false);
        ...
        const editButton = secondItem.querySelector('devtools-button[title="Edit"]');
        assert.instanceOf(editButton, HTMLElement);
        const eventPromise = getEventPromise(component, 'edit');
    ...
    `, false, true);
        ...
        const removeButton = firstItem.querySelector('devtools-button[title="Remove"]');
        assert.instanceOf(removeButton, HTMLElement);
        const eventPromise = getEventPromise(component, 'delete');
