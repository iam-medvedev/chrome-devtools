...
describe('Card', () => {
    ...
    describe('Card', () => {
        ...
        it('slots its regular children into the content area', () => {
            ...
            assert.include(card.shadowRoot.querySelector('#content').assignedElements(), h1);
            assert.include(card.shadowRoot.querySelector('#content').assignedElements(), p);
        });
        it('slots `heading-prefix` children into the heading area', () => {
            ...
            assert.include(card.shadowRoot.querySelector('[name="heading-prefix"]').assignedElements(), span);
        });
        it('slots `heading-suffix` children into the heading area', () => {
            const card = document.createElement('devtools-card');
            const span = card.appendChild(document.createElement('span'));
            span.slot = 'heading-suffix';
            assert.include(card.shadowRoot.querySelector('[name="heading-suffix"]').assignedElements(), span);
        });
        describe('heading', () => {
            it('defaults to null', () => {
                const card = document.createElement('devtools-card');
