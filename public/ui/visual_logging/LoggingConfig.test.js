...
describe('LoggingConfig', () => {
    ...
    it('builds a debug string', () => {
        ...
        assert.strictEqual(VisualLogging.LoggingConfig.debugString({ ve: 1, track: new Map([['click', undefined]]) }), 'TreeItem; track: click');
        assert.strictEqual(VisualLogging.LoggingConfig.debugString({ ve: 1, track: new Map([['click', undefined], ['change', undefined]]) }), 'TreeItem; track: click, change');
        assert.strictEqual(VisualLogging.LoggingConfig.debugString({ ve: 1, track: new Map([['keydown', 'Enter']]) }), 'TreeItem; track: keydown: Enter');
        assert.strictEqual(VisualLogging.LoggingConfig.debugString({ ve: 1, context: '42', track: new Map([['keydown', 'Enter']]) }), 'TreeItem; context: 42; track: keydown: Enter');
    });
});
