...
class TestSidebarPane extends BrowserDebugger.CategorizedBreakpointsSidebarPane.CategorizedBreakpointsSidebarPane {
    view;
    breakpoints;
    breakpointFromPausedDetails = null;
    constructor() {
        const breakpoints = [
        ...
        const view = createViewFunctionStub(BrowserDebugger.CategorizedBreakpointsSidebarPane.CategorizedBreakpointsSidebarPane);
        super(breakpoints, 'view', 'view', view);
        this.view = view;
        this.breakpoints = breakpoints;
    }
    getBreakpointFromPausedDetails() {
...
describeWithMockConnection('CategorizedBreakpointsSidebarPane', () => {
    it('sorts and groups the breakpoint', async () => {
        ...
        assert.deepEqual(input.categories.get("animation" /* SDK.CategorizedBreakpoint.Category.ANIMATION */), [pane.breakpoints[1], pane.breakpoints[3]]);
        assert.deepEqual(input.categories.get("canvas" /* SDK.CategorizedBreakpoint.Category.CANVAS */), [pane.breakpoints[2]]);
        assert.deepEqual(input.categories.get("load" /* SDK.CategorizedBreakpoint.Category.LOAD */), [pane.breakpoints[0]]);
    });
    ...
    describe('View', () => {
        ...
        it('expands selected breakpoints', async () => {
            const target = document.createElement('div');
            renderElementIntoDOM(target, { includeCommonStyles: true });
            categories.get("canvas" /* SDK.CategorizedBreakpoint.Category.CANVAS */)?.[0].setEnabled(true);
            BrowserDebugger.CategorizedBreakpointsSidebarPane.DEFAULT_VIEW({
                onFilterChanged: function () {
                    throw new Error('Function not implemented.');
                },
...
//# sourceMappingURL=CategorizedBreakpointsSidebarPane.test.js.map