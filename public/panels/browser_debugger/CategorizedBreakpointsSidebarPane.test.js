...
class TestSidebarPane extends BrowserDebugger.CategorizedBreakpointsSidebarPane.CategorizedBreakpointsSidebarPane {
    ...
    constructor() {
        super(TestSidebarPane.breakpoints, 'view', "other" /* Protocol.Debugger.PausedEventReason.Other */);
        renderElementIntoDOM(this);
    }
    doPopulate(filterText) {
    ...
    *tree() {
    ...
    asObject() {
        function treeAsObject(root) {
            const object = {};
            let child = root.firstChild();
            while (child) {
                object[child.title.title] = treeAsObject(child);
                child = child.nextSibling;
            }
            return object;
        }
...
describeWithMockConnection('CategorizedBreakpointsSidebarPane', () => {
    ...
    it('includes the paused breakpoint entry when filtered', async () => {
        ...
        await debuggerModel.pausedScript([], "other" /* Protocol.Debugger.PausedEventReason.Other */, {}, []);
    ...
    it('preseves expanded nodes while filtering', () => {
        const pane = new TestSidebarPane();
        const category = pane.tree().find(element => element.title.title === 'Canvas');
        assert.exists(category);
        category.expand();
        pane.doPopulate('animation');
        const category2 = pane.tree().find(element => element.title.title === 'Canvas');
        assert.strictEqual(category, category2, 'This test assumes the tree element objects do not change. If this fails, update the test.');
