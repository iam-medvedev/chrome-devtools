...
describeWithLocale('DOMBreakpointsSidebarPane placeholder', () => {
    ...
    it('shows one-liner if in sources', () => {
        ...
        assert.deepEqual(domBreakpointsSidebarPane.contentElement.querySelector('.placeholder .gray-info-message')?.textContent, 'No DOM breakpoints');
    });
    it('shows empty widget if in elements panel', () => {
        ...
        assert.deepEqual(domBreakpointsSidebarPane.contentElement.querySelector('.empty-state-header')?.textContent, 'No DOM breakpoints');
        assert.deepEqual(domBreakpointsSidebarPane.contentElement.querySelector('.empty-state-description > span')?.textContent, 'DOM breakpoints pause on the code that changes a DOM node or its children.');
    });
});
