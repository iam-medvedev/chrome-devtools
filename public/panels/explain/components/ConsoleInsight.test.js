...
describeWithEnvironment('ConsoleInsight', () => {
    ...
    it('shows reminder on first run of console insights', async () => {
        ...
        assert.strictEqual(nextInput.state.type, "consent-reminder" /* Explain.State.CONSENT_REMINDER */);
    ...
    it('report if the navigator is offline', async () => {
        ...
        assert.strictEqual(view.input.state.type, "offline" /* Explain.State.OFFLINE */);
    });
...
`,
    ...
    const DEFAULT_INPUT = {
        state: { type: "loading" /* Explain.State.LOADING */ },
        closing: false,
        ...
        callbacks: {
            ...
            onSearch: () => { },
            onRating: (_isPositive) => undefined,
            onReport: () => { },
            onGoToSignIn: () => { },
            onConsentReminderConfirmed: async () => { },
            onToggleReferenceDetails: (_event) => { },
    ...
    const rendersInsight = (noLogging, golden) => async () => {
        ...
        Explain.DEFAULT_VIEW({
            ...
            noLogging,
            state: {
                type: "insight" /* Explain.State.INSIGHT */,
                tokens: [
                    {
                        type: 'heading',
                        raw: '## Result\n\n',
