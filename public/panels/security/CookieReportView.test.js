...
describeWithMockConnection('CookieReportView', () => {
    ...
    it('should ignore non-third-party-cookie related exclusionReason', async () => {
        ...
        await view.updateComplete;
        assert.lengthOf(mockView.input.cookieRows, 1);
        assert.strictEqual(mockView.input.cookieRows[0].status, 0 /* IssuesManager.CookieIssue.CookieStatus.BLOCKED */);
    });
    it('should ignore non-third-party-cookie related warningReason', async () => {
        ...
        assert.strictEqual(mockView.input.cookieRows[0].status, 2 /* IssuesManager.CookieIssue.CookieStatus.ALLOWED_BY_GRACE_PERIOD */);
        assert.strictEqual(mockView.input.cookieRows[1].status, 3 /* IssuesManager.CookieIssue.CookieStatus.ALLOWED_BY_HEURISTICS */);
        assert.strictEqual(mockView.input.cookieRows[2].status, 1 /* IssuesManager.CookieIssue.CookieStatus.ALLOWED */);
    });
