...
describeWithEnvironment('IssuesPane', () => {
    it('shows placeholder if only non-relevant issues have appeared', () => {
        ...
        assert.deepEqual(issuesPane.contentElement.querySelector('.empty-state-header')?.textContent, 'Only third-party cookie issues detected');
        assert.deepEqual(issuesPane.contentElement.querySelector('.empty-state-description > span')?.textContent, 'On this page you can find warnings from the browser.');
    });
