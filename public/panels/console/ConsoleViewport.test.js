...
describe('ConsoleViewport', () => {
    ...
    class MockProvider {
        ...
        itemElement(index) {
            ...
            Object.defineProperty(mockElement, 'offsetHeight', { value: this.itemHeights[index] ?? this.minimumRowHeightValue, configurable: true });
    ...
    function calculateCumulativeHeights(itemHeights) {
    ...
    function getExpectedVisibleIndices(scrollTop, viewportHeight, cumulativeHeights) {
    ...
    describe('stick-to-bottom with expanding objects', () => {
        ...
        function populateViewport(itemCount, itemHeight) {
            const heights = Array(itemCount).fill(itemHeight);
            setItemsAndRefresh(heights);
        }
        function forceSelectAndFocus(index) {
        ...
        function isElementFullyVisible(index) {
            ...
            const tolerance = 2.5;
            return (selectedRect.top + tolerance >= viewportRect.top && selectedRect.bottom - tolerance <= viewportRect.bottom);
        }
        function getInfo(selectedIndex) {
            viewport.refresh(); // Ensure viewport is up-to-date before getting info
            const isAtBottom = (viewport.element.scrollHeight - viewport.element.scrollTop - viewport.element.clientHeight) < 1;
...
//# sourceMappingURL=ConsoleViewport.test.js.map