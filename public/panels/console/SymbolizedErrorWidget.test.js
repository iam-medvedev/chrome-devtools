...
describe('SymbolizedErrorWidget', function () {
    ...
    function stubSymbolizedError(message, frames, cause = null) {
    ...
    it('renders an error without a cause', async () => {
        ...
        widget.error = error;
        renderElementIntoDOM(widget, { includeCommonStyles: true, extraStyles: [consoleViewStyles, symbolizedErrorWidgetStyles] });
    ...
    it('renders an error with a cause and various types of frames', async () => {
        ...
        const error = stubSymbolizedError('Error: main error', [
...
Caused by: Error: cause error
    ...
    it('renders an UnparsableError', async () => {
        const description = 'Error: This is an unparsable error. http://example.com/unparsable.js\n    at foo (http://example.com/a.js:1:1)\n    invalid-line-that-fails';
    ...
    it('triggers a re-render when the SymbolizedError updates', async () => {
        ...
        error.dispatchEventToListeners("UPDATED" /* Bindings.SymbolizedError.Events.UPDATED */);
    ...
    it('correctly renders a complex nested eval stack trace', async () => {
        ...
        const level1Eval = { name: '<anonymous>', url: '<anonymous>', line: 0, column: 0, isEval: true, evalOrigin: evalCallerOrigin };
        const level2Eval = { name: '<anonymous>', url: '<anonymous>', line: 0, column: 0, isEval: true, evalOrigin: level1Eval };
        ...
        assert.include(updatedText, 'at end (eval at <anonymous> (eval at <anonymous> (eval at evalCaller (example.com/index.html:11:45))), <anonymous>:1:23)');
        assert.include(updatedText, 'at eval (eval at <anonymous> (eval at <anonymous> (eval at evalCaller (example.com/index.html:11:45))), <anonymous>:1:44)');
        assert.include(updatedText, 'at eval (eval at <anonymous> (eval at evalCaller (example.com/index.html:11:45)), <anonymous>:1:1)');
    ...
    at example.com/error-demo.html:222:3`;
        ...
        it('Case 4: The main error that the user saw (Error.cause)', async () => {
            ...
            const intermediateCause = stubSymbolizedError('Error: The intermediate cause', [
   ...
   !!! random debug dump !!!`;
    ...
    at example.com/error-demo.html:134:22`;
        ...
        it('Case 8: Top-level application error (Mixed Constellation)', async () => {
            ...
            const intermediateCause = stubSymbolizedError('Error: Intermediate error inside eval in third-party', [
                { name: 'evalInThirdParty', url: 'http://example.com/node_modules/third-party/eval.js', line: 4, column: 18 },
                { name: '', url: 'http://example.com/error-demo.html', line: 157, column: 11 },
            ], rootCause);
...
Caused by: Error: Root cause inside eval in third-party
    ...
    at example.com/error-demo.html:208:7`;
        ...
        it('Case 11: Source map Class & Method (eval)', async () => {
            ...
            const project = new Bindings.ContentProviderBasedProject.ContentProviderBasedProject(universe.workspace, 'project', Workspace.Workspace.projectTypes.Network, '', false);
            project.addContentProvider(urlString `http://example.com/class_method_source_map.js`, TextUtils.StaticContentProvider.StaticContentProvider.fromString(urlString `http://example.com/class_method_source_map.js`, Common.ResourceType.resourceTypes.Script, ''), 'text/javascript');
...
//# sourceMappingURL=SymbolizedErrorWidget.test.js.map