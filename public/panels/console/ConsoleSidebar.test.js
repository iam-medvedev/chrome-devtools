...
function createMessage(level, text, url) {
    const message = new SDK.ConsoleModel.ConsoleMessage(null, "javascript" /* Protocol.Log.LogEntrySource.Javascript */, level, text, { url });
    return new Console.ConsoleViewMessage.ConsoleViewMessage(message, sinon.createStubInstance(Components.Linkifier.Linkifier), sinon.createStubInstance(Logs.RequestResolver.RequestResolver), sinon.createStubInstance(IssuesManager.IssueResolver.IssueResolver), () => { });
}
function addMessage(sidebar, level, text, url) {
    const consoleViewMessage = createMessage(level, text, url);
    sidebar.onMessageAdded(consoleViewMessage);
    return consoleViewMessage;
}
async function getGroups(view) {
...
describeWithMockConnection('ConsoleSidebar', () => {
    it('groups logs by URL', async () => {
        ...
        addMessage(sidebar, "info" /* Protocol.Log.LogEntryLevel.Info */, 'message 1', urlString `https://www.example.com/a.html`);
        addMessage(sidebar, "info" /* Protocol.Log.LogEntryLevel.Info */, 'message 2', urlString `https://www.example.com/b.html`);
        addMessage(sidebar, "info" /* Protocol.Log.LogEntryLevel.Info */, 'message 3', urlString `https://www.example.com/a.html`);
    ...
    it('groups logs by level', async () => {
        const view = createViewFunctionStub(Console.ConsoleSidebar.ConsoleSidebar);
        const sidebar = new Console.ConsoleSidebar.ConsoleSidebar(undefined, view);
        addMessage(sidebar, "verbose" /* Protocol.Log.LogEntryLevel.Verbose */, 'message 1', urlString `https://www.example.com/a.html`);
        addMessage(sidebar, "error" /* Protocol.Log.LogEntryLevel.Error */, 'message 2', urlString `https://www.example.com/b.html`);
        addMessage(sidebar, "error" /* Protocol.Log.LogEntryLevel.Error */, 'message 3', urlString `https://www.example.com/a.html`);
        assert.deepEqual(await getGroups(view), {
    ...
    it('filters by groups', async () => {
        ...
        sidebar.addEventListener("FilterSelected" /* Console.ConsoleSidebar.Events.FILTER_SELECTED */, filterSelected);
    ...
    it('applies filters on selection', async () => {
        ...
        const groups = [
            new Console.ConsoleSidebar.ConsoleFilterGroup("message" /* Console.ConsoleSidebar.GroupName.ALL */, [], Console.ConsoleFilter.ConsoleFilter.allLevelsFilterValue()),
            new Console.ConsoleSidebar.ConsoleFilterGroup("error" /* Console.ConsoleSidebar.GroupName.ERROR */, [], Console.ConsoleFilter.ConsoleFilter.singleLevelMask("error" /* Protocol.Log.LogEntryLevel.Error */)),
        ];
        groups[1].onMessage(createMessage("error" /* Protocol.Log.LogEntryLevel.Error */, 'message 1', urlString `https://www.example.com/a.html`));
        groups[1].onMessage(createMessage("error" /* Protocol.Log.LogEntryLevel.Error */, 'message 2', urlString `https://www.example.com/b.html`));
        groups[1].onMessage(createMessage("error" /* Protocol.Log.LogEntryLevel.Error */, 'message 3', urlString `https://www.example.com/a.html`));
        const input = {
            groups,
...
//# sourceMappingURL=ConsoleSidebar.test.js.map