...
export const DEFAULT_VIEW = (input, output, target) => {
    ...
    function renderStartPage() {
      ...
      <div class="empty-state" jslog=${VisualLogging.section().context('start-view')}>
        ...
        <devtools-button .variant=${"tonal" /* Buttons.Button.Variant.TONAL */} jslogContext=${"chrome-recorder.create-recording" /* Actions.RecorderActions.CREATE_RECORDING */} @click=${input.onCreateNewRecording}>${i18nString(UIStrings.createRecording)}</devtools-button>
      </div>
    `;
        // clang-format on
    }
    ...
    function renderCreateRecordingPage() {
      ...
      <devtools-widget
        ...
        ${UI.Widget.widgetRef(CreateRecordingView, widget => {
    ...
    render(html `
        ...
        jslogContext: "chrome-recorder.create-recording" /* Actions.RecorderActions.CREATE_RECORDING */,
            ...
            <select
              .disabled=${input.recordings.length === 0 ||
        ...
        input.isToggling}
            ...
            >
              ${repeat(values, item => item.value, item => {
        ...
        disabled: !input.currentRecording,
            ...
            >
              <devtools-menu-group .name=${i18nString(UIStrings.export)}>
                ${repeat(input.builtInConverters, converter => {
        return html `
                    <devtools-menu-item
              ...
              <devtools-menu-group .name=${i18nString(UIStrings.exportViaExtensions)}>
                ${repeat(input.extensionConverters, converter => {
        return html `
                    <devtools-menu-item
                     .value=${converter.getId()}
        ...
        shortcuts: input.shortcutsInfo,
          ...
          ${input.importError
        ? html `<div class='error'>Import error: ${input.importError.message}</div>`
        : ''}
          ${renderCurrentPage()}
        </div>
...
export class RecorderPanel extends UI.Widget.VBox {
    ...
    set currentRecordingSession(value) {
    ...
    #currentPage = "StartPage" /* Pages.START_PAGE */;
    get currentPage() {
    ...
    #disableRecorderImportWarningSetting = Common.Settings.Settings.instance().createSetting('disable-recorder-import-warning', false, "Synced" /* Common.Settings.SettingStorageType.SYNCED */);
    #selfXssWarningDisabledSetting = Common.Settings.Settings.instance().createSetting('disable-self-xss-warning', false, "Synced" /* Common.Settings.SettingStorageType.SYNCED */);
    #recordingView;
    #createRecordingView;
    #view;
    ...
    onDetach() {
        ...
        if (this.#extensionViewShowRequestedListener) {
            PublicExtensions.RecorderPluginManager.RecorderPluginManager.instance().removeEventListener("showViewRequested" /* PublicExtensions.RecorderPluginManager.Events.SHOW_VIEW_REQUESTED */, this.#extensionViewShowRequestedListener);
    ...
    async #onPlayViaExtension(extension) {
        ...
        if (this.#extensionViewShowRequestedListener) {
            pluginManager.removeEventListener("showViewRequested" /* PublicExtensions.RecorderPluginManager.Events.SHOW_VIEW_REQUESTED */, this.#extensionViewShowRequestedListener);
            this.#extensionViewShowRequestedListener = undefined;
        }
        let resolveView;
        ...
        pluginManager.addEventListener("showViewRequested" /* PublicExtensions.RecorderPluginManager.Events.SHOW_VIEW_REQUESTED */, this.#extensionViewShowRequestedListener);
    ...
    async #onPlayRecording(event) {
        ...
        const withPerformanceTrace = event.targetPanel === "timeline" /* TargetPanel.PERFORMANCE_PANEL */;
        ...
        switch (event.targetPanel) {
            case "timeline" /* TargetPanel.PERFORMANCE_PANEL */:
        ...
        if (performanceTracing) {
            ...
            if (event.targetPanel === "timeline" /* TargetPanel.PERFORMANCE_PANEL */) {
    ...
    async #handleStepAdded(stepOrSection, position) {
        ...
        if ('steps' in stepOrSection) {
            ...
            if (position === "after" /* AddStepPosition.AFTER */) {
                if (this.sections?.[sectionIdx].steps.length) {
                    step = this.sections?.[sectionIdx].steps[0];
                    actualPosition = "before" /* AddStepPosition.BEFORE */;
                }
                else {
                    step = this.sections?.[sectionIdx].causingStep;
                    actualPosition = "after" /* AddStepPosition.AFTER */;
                }
            }
            else {
                if (sectionIdx <= 0) {
        ...
        const indexToInsertAt = currentIndex + (actualPosition === "before" /* AddStepPosition.BEFORE */ ? 0 : 1);
    ...
    async #onRecordingStarted(data) {
        ...
        this.currentRecordingSession.addEventListener("recordingupdated" /* Models.RecordingSession.Events.RECORDING_UPDATED */, async ({ data }) => {
            ...
            this.#setCurrentRecording(await this.#storage.upsertRecording(data, this.currentRecording.storageName));
    ...
    async #onPlayRecordingByName(storageName) {
        ...
        await this.#onPlayRecording({ targetPanel: "chrome-recorder" /* TargetPanel.DEFAULT */, speed: this.#recorderSettings.speed });
    }
    ...
    handleActions(actionId) {
        ...
        switch (actionId) {
            ...
            case "chrome-recorder.replay-recording" /* Actions.RecorderActions.REPLAY_RECORDING */:
                void this.#onPlayRecording({ targetPanel: "chrome-recorder" /* TargetPanel.DEFAULT */, speed: this.#recorderSettings.speed });
                return;
    ...
    performUpdate() {
        ...
        const output = {
            ...
            set recordingView(widget) {
                that.#recordingView = widget;
            },
            set createRecordingView(widget) {
