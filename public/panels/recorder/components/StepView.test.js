...
describeWithEnvironment('StepView', () => {
    ...
    async function createStepView(opts = {}) {
    ...
    describe('Step and section actions menu', () => {
        it('should open actions menu', async () => {
            ...
            assert.notOk(view.shadowRoot?.querySelector('devtools-menu[has-open-dialog]'));
            const button = view.shadowRoot?.querySelector('.step-actions');
            assert.ok(button);
            dispatchClickEvent(button);
            await coordinator.done();
            assert.ok(view.shadowRoot?.querySelector('devtools-menu[has-open-dialog]'));
        });
        it('should dispatch "AddStep before" events on steps', async () => {
            const view = await createStepView({ step });
            const menu = view.shadowRoot?.querySelector('.step-actions + devtools-menu');
            assert.ok(menu);
            const eventPromise = getEventPromise(view, 'addstep');
            menu.dispatchEvent(new Menus.Menu.MenuItemSelectedEvent('add-step-before'));
        ...
        it('should dispatch "AddStep after" events on steps', async () => {
            ...
            assert.ok(menu);
            const eventPromise = getEventPromise(view, 'addstep');
            menu.dispatchEvent(new Menus.Menu.MenuItemSelectedEvent('add-step-after'));
            const event = await eventPromise;
        ...
        it('should dispatch "Remove steps" events on steps', async () => {
            ...
            const menu = view.shadowRoot?.querySelector('.step-actions + devtools-menu');
            assert.ok(menu);
            const eventPromise = getEventPromise(view, 'removestep');
            menu.dispatchEvent(new Menus.Menu.MenuItemSelectedEvent('remove-step'));
            const event = await eventPromise;
        ...
        it('should dispatch "Add breakpoint" event on steps', async () => {
            ...
            const menu = view.shadowRoot?.querySelector('.step-actions + devtools-menu');
            assert.ok(menu);
            const eventPromise = getEventPromise(view, 'addbreakpoint');
            menu.dispatchEvent(new Menus.Menu.MenuItemSelectedEvent('add-breakpoint'));
            const event = await eventPromise;
        ...
        it('should dispatch "Remove breakpoint" event on steps', async () => {
            ...
            const menu = view.shadowRoot?.querySelector('.step-actions + devtools-menu');
            assert.ok(menu);
            const eventPromise = getEventPromise(view, 'removebreakpoint');
            menu.dispatchEvent(new Menus.Menu.MenuItemSelectedEvent('remove-breakpoint'));
            const event = await eventPromise;
            assert.deepStrictEqual(event.index, 0);
        });
    });
    ...
    describe('Copy steps', () => {
        it('should copy a step to clipboard', async () => {
            ...
            menu.dispatchEvent(new Menus.Menu.MenuItemSelectedEvent('copy-step-as-json'));
    ...
    describe('Selection', () => {
        it('should render timeline as selected if isSelected = true', async () => {
            ...
            assert.ok(view);
            const section = view.shadowRoot?.querySelector('devtools-timeline-section');
            assert.ok(section);
