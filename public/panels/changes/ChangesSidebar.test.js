...
describeWithEnvironment('ChangesSidebar', () => {
    function getSidebar() {
        ...
        const project = new Bindings.ContentProviderBasedProject.ContentProviderBasedProject(workspace, 'project', Workspace.Workspace.projectTypes.Network, 'project', false);
        const uiSourceCodes = resourceTypes.map((type, index) => new Workspace.UISourceCode.UISourceCode(project, urlString `http://example.com/uiSourceCode${index}`, type));
        uiSourceCodes.push(new Workspace.UISourceCode.UISourceCode(project, urlString `snippet:snippetSourceCode`, Common.ResourceType.resourceTypes.Script));
    ...
    function getTreeItem(sidebar, selector) {
        return sidebar.contentElement.firstChild?.shadowRoot?.querySelector(selector);
    }
...
//# sourceMappingURL=ChangesSidebar.test.js.map