...
describeWithEnvironment('StylePropertyTreeElement', () => {
    ...
    describe('updateTitle', () => {
        ...
        it('is able to expand longhands with vars', async () => {
            ...
            connection.setHandler('CSS.getLonghandProperties', (request) => {
                if (request.shorthandName !== 'shorthand') {
                    return { error: { message: 'Invalid shorthand', code: -32000 } };
                }
                const longhands = request.value.split(' ');
                if (longhands.length !== 3) {
                    return { error: { message: 'Invalid value', code: -32000 } };
                }
                return {
                    result: {
                        longhandProperties: [
        ...
        describe('color-mix swatch', () => {
            ...
            it('shows a popover with its computed color as RGB if possible', async () => {
                ...
                assert.exists(colorMixSwatch);
                renderElementIntoDOM(stylePropertyTreeElement.valueElement);
                const tooltip = stylePropertyTreeElement.valueElement?.querySelector(':scope > devtools-tooltip');
            ...
            it('propagates updates to outer color-mixes', () => {
                ...
                const newColor = colorSwatch.color?.as("hex" /* Common.Color.Format.HEX */);
            ...
            it('supports evaluation during tracing', async () => {
                ...
                connection.setSuccessHandler('CSS.resolveValues', (request) => ({ results: request.values.map(v => v === property.value ? 'grey' : v) }));
    ...
    it('applies the new style when the color format is changed', async () => {
        ...
        const expectedColorString = swatch.color?.asString("lab" /* Common.Color.Format.LAB */);
        assert.exists(expectedColorString);
        assert.match(expectedColorString, /lab\([-.0-9]* [-.0-9]* [-.0-9]*\)/);
        const newColor = swatch.color?.as("lab" /* Common.Color.Format.LAB */);
        assert.isOk(newColor);
    ...
    describe('CSS hints', () => {
        ...
        it('should not create a hint for SVG elements', () => {
            ...
            stylePropertyTreeElement.updateAuthoringHint();
            assert.isNotOk(stylePropertyTreeElement.listItemElement.classList.contains('inactive-property'), 'CSS hint was rendered unexpectedly.');
        });
    });
    describe('Animation override hint', () => {
        ...
        it('should create a hint when property is overridden by animation and verify tooltip content', () => {
            ...
            const animationOverrideHintWrapper = stylePropertyTreeElement.listItemElement.querySelector('.animation-override-hint-wrapper');
            assert.exists(animationOverrideHintWrapper, 'Hint wrapper not found via .animation-override-hint-wrapper');
            const tooltip = stylePropertyTreeElement.listItemElement.querySelector('devtools-tooltip');
        ...
        it('should not create a hint when property is overridden by animation but the animations panel is open', () => {
            LegacyUI.ViewManager.ViewManager.instance().isViewVisible.returns(true);
            const stylePropertyTreeElement = getTreeElement('opacity', '0.5');
            sinon.stub(matchedStyles, 'isPropertyOverriddenByAnimation').returns(true);
            stylePropertyTreeElement.updateAnimationOverrideHint();
            const hintWrapper = stylePropertyTreeElement.listItemElement.querySelector('.animation-override-hint-wrapper');
    ...
    describe('ColorRenderer', () => {
        it('correctly renders children of the color swatch', () => {
            ...
            const colorSwatch = stylePropertyTreeElement.valueElement?.querySelector('devtools-color-swatch');
            assert.exists(colorSwatch);
            assert.strictEqual(colorSwatch.color?.asString("hex" /* Common.Color.Format.HEX */), '#ff0000');
        ...
        it('connects correctly with an inner angle swatch', () => {
            ...
            renderElementIntoDOM(stylePropertyTreeElement.valueElement);
            assert.exists(colorSwatch);
            assert.strictEqual(colorSwatch.color?.asString("hsl" /* Common.Color.Format.HSL */), 'hsl(120deg 50% 25%)');
            ...
            assert.strictEqual(colorSwatch.color?.asString("hsl" /* Common.Color.Format.HSL */), 'hsl(130deg 50% 25%)');
        ...
        it('renders relative colors', () => {
            ...
            assert.strictEqual(colorSwatch?.color?.asString("hsl" /* Common.Color.Format.HSL */), 'hsl(240deg 50% 50%)');
        });
    ...
    describe('RelativeColorChannelRenderer', () => {
        it('provides a tooltip for relative color channels', () => {
            ...
            stylePropertyTreeElement.updateTitle();
            const tooltips = stylePropertyTreeElement.valueElement?.querySelectorAll('devtools-tooltip:not([jslogcontext="elements.css-value-trace"])');
        ...
        it('evaluates relative color channels during tracing', async () => {
            connection.setHandler('CSS.resolveValues', null);
            connection.setSuccessHandler('CSS.resolveValues', (request) => ({ results: request.values.map(v => v === 'calc(1.000 / 2)' ? '0.5' : '') }));
    ...
    describe('AnchorFunctionRenderer', () => {
        ...
        beforeEach(() => {
            ...
            connection.setSuccessHandler('DOM.getAnchorElement', () => ({ result: undefined }));
        });
    ...
    describe('LightDarkColorRenderer', () => {
        it('renders light-dark correctly', async () => {
            ...
            async function check(colorScheme, lightText, darkText) {
                ...
                renderElementIntoDOM(stylePropertyTreeElement.valueElement);
                assert.strictEqual(swatch?.nextElementSibling?.innerText, lightDark);
    ...
    describe('CSSWideKeywordRenderer', () => {
        function mockResolvedKeyword(propertyName, _keyword, propertyValue = '') {
    ...
    describe('LengthRenderer', () => {
        it('shows a popover with pixel values for relative units', async () => {
            connection.setHandler('CSS.resolveValues', null);
            connection.setSuccessHandler('CSS.resolveValues', (request) => ({ results: request.values.map(v => v === '2em' ? '15px' : v) }));
            ...
            connection.setSuccessHandler('CSS.getComputedStyleForNode', () => ({ computedStyle: {} }));
            await stylePropertyTreeElement.onpopulate();
            stylePropertyTreeElement.updateTitle();
            renderElementIntoDOM(stylePropertyTreeElement.valueElement);
    ...
    describe('MathFunctionRenderer', () => {
        it('strikes out non-selected values', async () => {
            connection.setHandler('CSS.resolveValues', null);
            connection.setSuccessHandler('CSS.resolveValues', (request) => {
                const stripCalc = (v) => {
            ...
            const args = stylePropertyTreeElement.valueElement?.querySelectorAll(':scope > span > span:not(.tracing-anchor)');
        ...
        it('shows the original text during tracing when evaluation fails', async () => {
            connection.setHandler('CSS.resolveValues', null);
            connection.setSuccessHandler('CSS.resolveValues', (request) => ({ results: request.values.map(() => '') }));
    ...
    describe('EnvFunctionRenderer', () => {
        it('strikes out non-selected values', async () => {
            ...
            assert.deepEqual(Array.from(args.values()).map(arg => arg.classList.contains('inactive-value')), [false, true, true, false]);
        });
    ...
    it('reopens open tooltips on updates', async () => {
        ...
        const stylePropertyTreeElement = getTreeElement('color', 'color-mix(in srgb, red, blue)');
        stylePropertyTreeElement.updateTitle();
        const tooltip = stylePropertyTreeElement.valueElement?.querySelector('devtools-tooltip:not([jslogcontext="elements.css-value-trace"])');
        assert.exists(tooltip);
        ...
        stylePropertyTreeElement.updateTitle();
        const tooltip2 = stylePropertyTreeElement.valueElement?.querySelector('devtools-tooltip:not([jslogcontext="elements.css-value-trace"])');
