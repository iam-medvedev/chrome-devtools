...
describeWithMockConnection('StandaloneStylesContainer', () => {
    ...
    async function setupContainer(properties = []) {
        ...
        const updatePromise = new Promise(resolve => {
            container.addEventListener("StylesUpdateCompleted" /* Elements.StandaloneStylesContainer.Events.STYLES_UPDATE_COMPLETED */, () => resolve(), { once: true });
        });
    ...
    it('should perform full rebuild when CSS model changes without an edit', async () => {
        ...
        container.computedStyleModel().dispatchEventToListeners("CSSModelChanged" /* ComputedStyle.ComputedStyleModel.Events.CSS_MODEL_CHANGED */, undefined);
        await updatePromise;
    ...
    it('should perform full rebuild when domNode changes', async () => {
        ...
        await updatePromise;
        sinon.assert.called(discardSpy);
        assert.notStrictEqual(container.allSections(), initialSections, 'Sections should have been recreated for the new node');
    });
