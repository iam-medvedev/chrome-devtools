...
describeWithMockConnection.only('EventListenersWidget', () => {
    ...
    it('updates on filter change from both programmatic setting change and UI event', async () => {
        ...
        const dispatchFilterTypeSetting = Common.Settings.Settings.instance().settingForTest('event-listener-dispatch-filter-type');
    ...
    it('updates on framework listeners setting change', async () => {
        const view = await setup();
        const showFrameworkListenersSetting = Common.Settings.Settings.instance().settingForTest('show-frameowkr-listeners');
    ...
    it('updates on ancestor setting change', async () => {
        const view = await setup();
        const showForAncestorsSetting = Common.Settings.Settings.instance().moduleSetting('show-event-listeners-for-ancestors');
        ...
        input = await view.nextInput;
        assert.deepEqual(input.eventListenerObjects, [nodeRemoteObject, parentRemoteObject, grandParentRemoteObject, windowRemoteObject]);
    });
...
//# sourceMappingURL=EventListenersWidget.test.js.map