...
function createCssModelAndProperty() {
    ...
    header.requestContentData.resolves(new TextUtils.ContentData.ContentData(cssContent, /* isBase64=*/ false, 'text/css'));
    const target = createTarget();
    const cssModel = new SDK.CSSModel.CSSModel(target);
    sinon.stub(cssModel, 'styleSheetHeaderForId').returns(header);
    const cssProperty = new SDK.CSSProperty.CSSProperty({ styleSheetId: 'test-sheet-id' }, 0, 'color', 'red', true, false, true, false, 'color: red;', new TextUtils.TextRange.TextRange(0, 7, 0, 18));
...
describeWithEnvironment('StylesAiCodeCompletionProvider', () => {
    ...
    beforeEach(() => {
        ...
        sinon.stub(Host.AidaClient.HostConfigTracker.instance(), 'aidaAvailability')
            .get(() => "available" /* Host.AidaClient.AidaAccessPreconditions.AVAILABLE */);
    ...
    describe('Triggers code completion', () => {
        it('when property name is being edited', async () => {
            ...
            assert.deepEqual(language, "CSS" /* Host.AidaClient.AidaInferenceLanguage.CSS */);
        });
        ...
        it('caps prefix and suffix to MAX_PREFIX_SUFFIX_LENGTH', async () => {
            ...
            const cssModel = new SDK.CSSModel.CSSModel(target);
            sinon.stub(cssModel, 'styleSheetHeaderForId').returns(header);
            const cssProperty = new SDK.CSSProperty.CSSProperty({ styleSheetId: 'test-sheet-id' }, 0, 'color', 'red', true, false, true, false, 'color: red;', new TextUtils.TextRange.TextRange(0, largePrefix.length + 7, 0, largePrefix.length + 18));
    ...
    it('makes a callback when AIDA returns a suggestion', async () => {
        const completeCodeStub = sinon.stub(AiCodeCompletion.AiCodeCompletion.AiCodeCompletion.prototype, 'completeCode').resolves({
            response: {
                generatedSamples: [{
