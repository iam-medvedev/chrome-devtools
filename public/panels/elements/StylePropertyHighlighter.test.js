...
describeWithMockConnection('StylePropertyHighlighter', () => {
    ...
    it('highlights layers', async () => {
        ...
        sinon.assert.calledOnceWithExactly(highlightSpy, block.titleElement());
    });
    ...
    it('highlights longhand properties of a shorthand property', async () => {
        const { stylesSidebarPane, matchedStyles } = await setupStylesPane();
        const style = sinon.createStubInstance(SDK.CSSStyleDeclaration.CSSStyleDeclaration);
        const shorthandProperty = new SDK.CSSProperty.CSSProperty(style, 0, 'background', 'red', true, false, true, false, '', undefined);
        const longhandProperty = new SDK.CSSProperty.CSSProperty(style, 1, 'background-color', 'red', true, false, true, false, '', undefined);
        ...
        const section = new Elements.StylePropertiesSection.StylePropertiesSection(stylesSidebarPane, matchedStyles, style, 0, null, null, null);
        ...
        const shorthandTreeElement = section.propertiesTreeOutline.firstChild();
        const longhandTreeElement = shorthandTreeElement.childAt(0);
