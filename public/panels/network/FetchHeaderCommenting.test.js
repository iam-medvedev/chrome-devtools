...
describeWithEnvironment('FetchHeaderCommenting', () => {
  ...
  "method": "POST"
            ...
            for (const headerName of ['cookie2', 'date', 'expect', 'set-cookie', 'te', 'trailer', 'transfer-encoding', 'upgrade', 'via']) {
  ...
  "method": "GET"
            ...
            const customRules = [
                { pattern: /^method$/i, comment: () => 'not a header', style: 'append' },
            ];
            assert.strictEqual(commentForbiddenHeaders(input, customRules), input);
        });
        it('supports custom rules', () => {
            const customRules = [
                { pattern: /^x-internal-/i, comment: () => 'internal header', style: 'append' },
            ];
            const input = `fetch("https://example.com/", {
  "headers": {
    "accept": "text/html",
...
//# sourceMappingURL=FetchHeaderCommenting.test.js.map