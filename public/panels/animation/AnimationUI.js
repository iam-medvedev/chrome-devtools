...
export class AnimationUI {
    ...
    drawDelayLine(parentElement) {
        ...
        const leftMargin = Math.min(this.#timeline.width(), (this.delayOrStartTime() + this.duration() * this.#animation.source().iterations()) *
        ...
        this.#endDelayLine.setAttribute('x2', forwardsFill ? (this.#timeline.width() - leftMargin + margin).toFixed(2) :
    ...
    redraw() {
        ...
        for (iteration = 1; iteration < this.#animation.source().iterations() &&
            ...
            (iterationWidth > 0 || this.#animation.source().iterations() !== Infinity); iteration++) {
