                ...
                breakpointItems: [
                    {
                        ...
                        isHit: true,
                        status: "ENABLED" /* Sources.BreakpointsView.BreakpointStatus.ENABLED */,
                        type,
                        hoverText,
                    },
                ],
        ...
        groups: [
            {
                ...
                breakpointItems: [
                    ...
                    },
                    {
                        ...
                        codeSnippet: 'if (x > a) {',
                        isHit: true,
                        status: "DISABLED" /* Sources.BreakpointsView.BreakpointStatus.DISABLED */,
                    },
                ],
            },
            {
...
function checkCheckboxState(checkbox, breakpointItem) {
    ...
    if (breakpointItem.status === "INDETERMINATE" /* Sources.BreakpointsView.BreakpointStatus.INDETERMINATE */) {
        assert.isTrue(indeterminate);
    }
    else {
        assert.isFalse(indeterminate);
        assert.strictEqual((breakpointItem.status === "ENABLED" /* Sources.BreakpointsView.BreakpointStatus.ENABLED */), checked);
    }
}
...
describeWithEnvironment('BreakpointsSidebarController', () => {
    ...
    it('changes breakpoint state', async () => {
        const { groups, location } = await setUpTestWithOneBreakpointLocation();
        const breakpointItem = groups[0].breakpointItems[0];
        assert.strictEqual(breakpointItem.status, "ENABLED" /* Sources.BreakpointsView.BreakpointStatus.ENABLED */);
        const breakpoint = location.breakpoint;
    ...
    it('correctly reveals breakpoint editor', async () => {
        ...
        await Sources.BreakpointsView.BreakpointsSidebarController.instance().breakpointEdited(breakpointItem, false /* editButtonClicked */);
    ...
    describe('getUpdatedBreakpointViewData', () => {
        ...
        it('correctly extracts the enabled state', async () => {
            ...
            const breakpointItem = groups[0].breakpointItems[0];
            assert.strictEqual(breakpointItem.status, "DISABLED" /* Sources.BreakpointsView.BreakpointStatus.DISABLED */);
        });
        it('correctly extracts the enabled state', async () => {
            const testData = [
            ...
            assert.lengthOf(actualViewData.groups, 1);
            assert.lengthOf(actualViewData.groups[0].breakpointItems, 1);
            assert.strictEqual(actualViewData.groups[0].breakpointItems[0].status, "INDETERMINATE" /* Sources.BreakpointsView.BreakpointStatus.INDETERMINATE */);
        });
...
describeWithMockConnection('BreakpointsSidebarController', () => {
    ...
    it('auto-expands if a breakpoint was hit', async () => {
        ...
        const controller = Sources.BreakpointsView.BreakpointsSidebarController.instance({ forceNew: true, breakpointManager, settings: Common.Settings.Settings.instance() });
        controller.expandedStateChanged(uiSourceCode.url(), false /* expanded */);
...
describeWithMockConnection('BreakpointsView', () => {
    ...
    describe('group checkboxes', () => {
        async function waitForCheckboxToggledEventsWithCheckedUpdate(numBreakpointItems, checked) {
        ...
        it('show a checked group checkbox if at least one breakpoint in that group is enabled', async () => {
            ...
            data.groups[0].breakpointItems[0].status = "ENABLED" /* Sources.BreakpointsView.BreakpointStatus.ENABLED */;
            component.data = data;
            await RenderCoordinator.done();
            await component.updateComplete;
            await hover(component, SUMMARY_SELECTOR);
        ...
        it('show an unchecked group checkbox if no breakpoint in that group is enabled', async () => {
            ...
            for (let i = 0; i < breakpointItems.length; ++i) {
                breakpointItems[i].status = "DISABLED" /* Sources.BreakpointsView.BreakpointStatus.DISABLED */;
            }
            component.data = data;
            await RenderCoordinator.done();
            await component.updateComplete;
        ...
        it('disable all breakpoints on unchecking', async () => {
            ...
            for (let i = 0; i < numBreakpointItems; ++i) {
                data.groups[0].breakpointItems[i].status = "ENABLED" /* Sources.BreakpointsView.BreakpointStatus.ENABLED */;
            }
            component.data = data;
            await RenderCoordinator.done();
            await component.updateComplete;
        ...
        it('enable all breakpoints on unchecking', async () => {
            ...
            // Make sure that all breakpoints are disabled.
            for (let i = 0; i < numBreakpointItems; ++i) {
                data.groups[0].breakpointItems[i].status = "DISABLED" /* Sources.BreakpointsView.BreakpointStatus.DISABLED */;
            }
            component.data = data;
            await RenderCoordinator.done();
            await component.updateComplete;
    ...
    describe('pause on exceptions', () => {
        it('state is rendered correctly when disabled', async () => {
            ...
            assert.isNotNull(component.contentElement);
            const pauseOnUncaughtExceptionsItem = component.contentElement.querySelector(PAUSE_ON_UNCAUGHT_EXCEPTIONS_SELECTOR);
    ...
    describe('navigating with keyboard', () => {
        ...
        describe('pressing the HOME key', () => {
            it('takes the user to the pause-on-exceptions line', async () => {
                ...
                assert.isNotNull(component.contentElement);
                const secondGroupsSummary = component.contentElement.querySelector(`${DETAILS_SELECTOR}:nth-of-type(2) > ${SUMMARY_SELECTOR}`);
        ...
        describe('pressing the END key', () => {
            it('takes the user to the summary node of the last group (if last group is collapsed)', async () => {
                ...
                const selected = component.contentElement.querySelector(TABBABLE_SELECTOR);
                assert.instanceOf(selected, HTMLElement);
                const lastGroupSummary = component.contentElement.querySelector(`${DETAILS_SELECTOR}:nth-of-type(2) > ${SUMMARY_SELECTOR}`);
        ...
        describe('pressing the ArrowDown key', () => {
            ...
            it('on the summary node of a collapsed group takes the user to the summary node of the next group', async () => {
                const { component } = await renderBreakpointsForKeyboardNavigation();
                assert.isNotNull(component.contentElement);
                const firstGroupSummary = component.contentElement.querySelector(`${DETAILS_SELECTOR}:nth-of-type(1) > ${SUMMARY_SELECTOR}`);
                assert.instanceOf(firstGroupSummary, HTMLElement);
                ...
                const selected = component.contentElement.querySelector(TABBABLE_SELECTOR);
                assert.instanceOf(selected, HTMLElement);
                const secondGroupSummary = component.contentElement.querySelector(`${DETAILS_SELECTOR}:nth-of-type(2) > ${SUMMARY_SELECTOR}`);
