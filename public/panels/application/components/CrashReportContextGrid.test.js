...
describeWithEnvironment('CrashReportContextGrid', () => {
    ...
    it('fires select event when a row is selected', async () => {
        ...
        const input = await inputPromise;
        let selectedKey = '';
        component.element.addEventListener('select', (e) => {
    ...
    it('copies key and value correctly from context menu', async () => {
        ...
        const copyValueItem = contextMenu.defaultSection().items.find(item => item.buildDescriptor().label === 'Copy value');
...
//# sourceMappingURL=CrashReportContextGrid.test.js.map