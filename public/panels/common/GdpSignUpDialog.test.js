...
function stubCreateProfileWithResolvers() {
    const { promise: createProfilePromise, resolve: resolveCreateProfile } = Promise.withResolvers();
    const createProfileStub = sinon.stub(Host.GdpClient.GdpClient.instance(), 'createProfile').returns(createProfilePromise);
...
describeWithEnvironment('GdpSignUpDialog', () => {
    ...
    describe('onSignUpClick', () => {
        let userBadgesInitializeStub;
        let userBadgesRecordActionStub;
        let snackbarShowStub;
        beforeEach(() => {
        ...
        it('handles successful sign up', async () => {
            ...
            sinon.assert.calledOnceWithExactly(createProfileStub, { user: 'Test User', emailPreference: Host.GdpClient.EmailPreference.DISABLED });
        ...
        it('handles successful sign up with email preference', async () => {
            ...
            await new Promise(resolve => setTimeout(resolve, 0));
            assert.isTrue(Common.Settings.settingForTest('receive-gdp-badges').get());
            sinon.assert.calledOnceWithExactly(createProfileStub, { user: 'Test User', emailPreference: Host.GdpClient.EmailPreference.ENABLED });
            sinon.assert.calledOnce(userBadgesInitializeStub);
            sinon.assert.calledOnceWithExactly(userBadgesRecordActionStub, Badges.BadgeAction.GDP_SIGN_UP_COMPLETE);
            sinon.assert.calledOnce(hideSpy);
        });
...
//# sourceMappingURL=GdpSignUpDialog.test.js.map