    ...
    describe('Resource', () => {
        ...
        describe('setFunctionRangesForScript', () => {
            ...
            it('correctly calls DebuggerWorkspaceBindings.setFunctionRanges via Resource.setFunctionRangesForScript API', async () => {
                ...
                project.addUISourceCode(new Workspace.UISourceCode.UISourceCode(project, scriptUrl, Common.ResourceType.resourceTypes.SourceMapScript));
                // create a mock uiSourceCode for the non-sourceMap script
                const normalScriptUrl = urlString `https://example.com/normal.js`;
                project.addUISourceCode(new Workspace.UISourceCode.UISourceCode(project, normalScriptUrl, Common.ResourceType.resourceTypes.Script));
                ...
                const workspaceBindingSetFunctionRangesStub = sinon.stub(Bindings.DebuggerWorkspaceBinding.DebuggerWorkspaceBinding.instance(), 'setFunctionRanges');
...
describe('Runtime hosts policy', () => {
    ...
    it('blocks getting resource contents on allowed urls if target is blocked', async () => {
        const parentTarget = getBackend(context).createTarget({ id: 'parent' });
        parentTarget.setInspectedURL(allowedUrl);
        const blockedTarget = getBackend(context).createTarget({ id: 'blocked-target' });
    ...
    it('does not include requests from blocked targets in onRequestFinished event listener even if request URL is allowed', async () => {
        const frameId = 'frame-id';
        const allowedTarget = getBackend(context).createTarget({ id: 'allowed-target' });
...
describe('ExtensionServer', () => {
    it('can correctly expand resource paths', async () => {
        ...
        assert.isUndefined(PanelCommon.ExtensionServer.ExtensionServer.expandResourcePath(extensionOrigin, 'http://example.com/foo'));
        assert.strictEqual(expectation, PanelCommon.ExtensionServer.ExtensionServer.expandResourcePath(extensionOrigin, expectation));
        assert.strictEqual(expectation, PanelCommon.ExtensionServer.ExtensionServer.expandResourcePath(extensionOrigin, '/foo'));
        assert.strictEqual(expectation, PanelCommon.ExtensionServer.ExtensionServer.expandResourcePath(extensionOrigin, 'foo'));
        assert.isUndefined(PanelCommon.ExtensionServer.ExtensionServer.expandResourcePath(almostOrigin, 'http://example.com/foo'));
        assert.strictEqual(expectation, PanelCommon.ExtensionServer.ExtensionServer.expandResourcePath(almostOrigin, expectation));
        assert.strictEqual(expectation, PanelCommon.ExtensionServer.ExtensionServer.expandResourcePath(almostOrigin, '/foo'));
        assert.strictEqual(expectation, PanelCommon.ExtensionServer.ExtensionServer.expandResourcePath(almostOrigin, 'foo'));
    });
