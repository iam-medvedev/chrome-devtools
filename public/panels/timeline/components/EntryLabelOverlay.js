...
export class EntryLabelOverlay extends HTMLElement {
    ...
    static LABEL_AND_CONNECTOR_SHIFT_LENGTH = 4;
    // Length of the line that connects the label to the entry.
    static LABEL_CONNECTOR_HEIGHT = 6;
  ...
  */
    ...
    #drawConnector() {
        const labelPartsWrapper = this.#shadow.querySelector('.label-parts-wrapper');
        const connectorLineContainer = labelPartsWrapper?.querySelector('#connectorContainer');
        const connector = connectorLineContainer.querySelector('line');
        const entryHighlightWrapper = labelPartsWrapper?.querySelector('.entry-highlight-wrapper');
    ...
    #drawLabel() {
        const labelPartsWrapper = this.#shadow.querySelector('.label-parts-wrapper');
        const labelBox = labelPartsWrapper?.querySelector('.label-box');
        const connectorLineContainer = labelPartsWrapper?.querySelector('#connectorContainer');
        const connector = connectorLineContainer.querySelector('line');
        const entryHighlightWrapper = labelPartsWrapper?.querySelector('.entry-highlight-wrapper');
