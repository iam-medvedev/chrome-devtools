...
function getCancelButton(view) {
    const buttons = view.querySelector('.vbox').shadowRoot.querySelectorAll('devtools-button');
    return Array.from(buttons).find(b => b.textContent === 'Cancel') || null;
}
function getValidationErrors(view) {
    const errors = view.querySelector('.vbox').shadowRoot.querySelector('.list-widget-input-validation-error');
