...
describeWithMockConnection('TimelineUIUtils', function () {
    ...
    it('creates top frame location text for function calls', async function () {
        ...
        const functionCallEvent = traceData.Renderer.allTraceEntries.find(TraceEngine.Types.TraceEvents.isTraceEventFunctionCall);
        assert.isOk(functionCallEvent);
        assert.strictEqual('chrome-extension://blijaeebfebmkmekmdnehcmmcjnblkeo/lib/utils.js:11:43', await Timeline.TimelineUIUtils.TimelineUIUtils.buildDetailsTextForTraceEvent(functionCallEvent, traceData));
    });
    it('creates top frame location text as a fallback', async function () {
        ...
        const timerInstallEvent = traceData.Renderer.allTraceEntries.find(TraceEngine.Types.TraceEvents.isTraceEventTimerInstall);
        assert.isOk(timerInstallEvent);
        assert.strictEqual('https://web.dev/js/index-7b6f3de4.js:96:533', await Timeline.TimelineUIUtils.TimelineUIUtils.buildDetailsTextForTraceEvent(timerInstallEvent, traceData));
    });
    describe('script location as an URL', function () {
        it('makes the script location of a call frame a full URL when the inspected target is not the same the call frame was taken from (e.g. a loaded file)', async function () {
            ...
            const node = await Timeline.TimelineUIUtils.TimelineUIUtils.buildDetailsNodeForTraceEvent(fakeFunctionCall, target, new Components.Linkifier.Linkifier(), false, traceData);
            if (!node) {
    ...
    describe('mapping to authored function name when recording is fresh', function () {
        it('maps to the authored name and script of a profile call', async function () {
            ...
            const linkifier = new Components.Linkifier.Linkifier();
            const node = await Timeline.TimelineUIUtils.TimelineUIUtils.buildDetailsNodeForTraceEvent(profileCall, target, linkifier, true, traceData);
            if (!node) {
                throw new Error('Node was unexpectedly null');
            }
    ...
    describe('adjusting timestamps for events and navigations', function () {
        it('adjusts the time for a DCL event after a navigation', async function () {
            ...
            const adjustedTime = Timeline.TimelineUIUtils.timeStampForEventAdjustedForClosestNavigationIfPossible(dclEvent, traceData);
        ...
        it('can adjust the times for events that are not PageLoad markers', async function () {
            ...
            const adjustedMarkTime = Timeline.TimelineUIUtils.timeStampForEventAdjustedForClosestNavigationIfPossible(userMark, traceData);
...
`;
        ...
        it('assigns the correct color to the swatch of an event\'s title', async function () {
            ...
            const details = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(traceData, task, new Components.Linkifier.Linkifier(), false);
    ...
    describe('testContentMatching', () => {
        it('matches call frame events based on a regular expression and the contents of the event', async function () {
            ...
            assert.isTrue(Timeline.TimelineUIUtils.TimelineUIUtils.testContentMatching(performConcurrentWorkEvent, /perfo/, traceData));
        });
    });
    describe('traceEventDetails', function () {
        it('shows the interaction ID and INP breakdown metrics for a given interaction', async function () {
            ...
            const details = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(traceData, interactionEvent, new Components.Linkifier.Linkifier(), false);
        ...
        it('renders all event data for a generic trace', async function () {
            ...
            const details = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(traceData, event, new Components.Linkifier.Linkifier(), false);
            const rowData = getRowDataForDetailsElement(details);
            assert.deepEqual(rowData, [
                {
                    title: '',
        ...
        it('renders invalidations correctly', async function () {
            ...
            const details = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(traceData, updateLayoutTreeEvent, new Components.Linkifier.Linkifier(), false);
            const rowData = getRowDataForDetailsElement(details);
            assert.deepEqual(rowData, [
                {
        ...
        it('renders details for performance.mark', async function () {
            ...
            const details = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(traceData, mark, new Components.Linkifier.Linkifier(), false);
            const rowData = getRowDataForDetailsElement(details);
            assert.deepEqual(rowData, [
                {
                    title: 'Timestamp',
        ...
        it('renders details for performance.measure', async function () {
            ...
            const details = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(traceData, measure, new Components.Linkifier.Linkifier(), false);
            const rowData = getRowDataForDetailsElement(details);
            assert.deepEqual(rowData, [
                {
                    title: 'Timestamp',
        ...
        it('renders details for a v8.compile ("Compile Script") event', async function () {
            ...
            const details = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(traceData, compileEvent, new Components.Linkifier.Linkifier(), false);
            const rowData = getRowDataForDetailsElement(details);
            assert.deepEqual(rowData, [
                {
                    title: 'Script',
        ...
        it('renders the details for a layout shift properly', async function () {
            ...
            const details = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(traceData, layoutShift, new Components.Linkifier.Linkifier(), false);
            const rowData = getRowDataForDetailsElement(details);
            assert.deepEqual(rowData, [
                {
                    title: 'Warning',
        ...
        it('renders the details for an extension entry properly', async function () {
            ...
            const details = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(traceData, extensionEntry, new Components.Linkifier.Linkifier(), false);
            const rowData = getRowDataForDetailsElement(details);
            assert.deepEqual(rowData, [
                {
                    title: 'Description',
        ...
        it('renders the details for an extension marker properly', async function () {
            ...
            const details = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(traceData, extensionMark, new Components.Linkifier.Linkifier(), false);
            const rowData = getRowDataForDetailsElement(details);
            assert.deepEqual(rowData, [
                {
                    title: 'Description',
        ...
        it('renders the details for a profile call properly', async function () {
            ...
            const details = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(traceData, profileCalls[0], new Components.Linkifier.Linkifier(), false);
            const rowData = getRowDataForDetailsElement(details);
            assert.deepEqual(rowData, [
                {
                    title: 'Function',
        ...
        it('renders the stack trace of a ScheduleStyleRecalculation properly', async function () {
            ...
            const details = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(traceData, scheduleStyleRecalcs[1], new Components.Linkifier.Linkifier(), false);
        ...
        it('renders the stack trace of extension entries properly', async function () {
            ...
            const markerDetails = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(traceData, extensionMarker, new Components.Linkifier.Linkifier(), false);
            ...
            const trackEntryDetails = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(traceData, extensionTrackEntry, new Components.Linkifier.Linkifier(), false);
        ...
        it('renders the stack trace of user timings properly', async function () {
            ...
            const markDetails = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(traceData, performanceMark, new Components.Linkifier.Linkifier(), false);
            ...
            const measureDetails = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(traceData, performanceMeasure, new Components.Linkifier.Linkifier(), false);
        ...
        it('renders the warning for a trace event in its details', async function () {
            ...
            const details = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(traceData, longTask, new Components.Linkifier.Linkifier(), false);
            const rowData = getRowDataForDetailsElement(details);
            assert.deepEqual(rowData, [
                {
                    title: 'Warning',
        ...
        it('shows information for the WebSocketCreate initiator when viewing a WebSocketSendHandshakeRequest event', async function () {
            ...
            TraceLoader.initTraceBoundsManager(traceData);
            const sendHandshake = traceData.Renderer.allTraceEntries.find(TraceEngine.Types.TraceEvents.isTraceEventWebSocketSendHandshakeRequest);
            if (!sendHandshake) {
                throw new Error('Could not find handshake event.');
            }
            const details = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(traceData, sendHandshake, new Components.Linkifier.Linkifier(), false);
            const rowData = getRowDataForDetailsElement(details);
        ...
        it('shows information for the events initiated by WebSocketCreate when viewing a WebSocketCreate event', async function () {
            const { traceData } = await TraceLoader.traceEngine(this, 'web-sockets.json.gz');
            TraceLoader.initTraceBoundsManager(traceData);
            const sendHandshake = traceData.Renderer.allTraceEntries.find(TraceEngine.Types.TraceEvents.isTraceEventWebSocketCreate);
            if (!sendHandshake) {
                throw new Error('Could not find handshake event.');
            }
            const details = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(traceData, sendHandshake, new Components.Linkifier.Linkifier(), false);
        ...
        it('shows the aggregated time information for an event', async function () {
            ...
            const details = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(traceData, event, new Components.Linkifier.Linkifier(), true);
    ...
    describe('isMarkerEvent', () => {
        ...
        it('is true for a Mark First Paint event', async function () {
            const { traceData } = await TraceLoader.traceEngine(this, 'web-dev-initial-url.json.gz');
            const markFirstPaint = traceData.PageLoadMetrics.allMarkerEvents.find(TraceEngine.Types.TraceEvents.isTraceEventFirstPaint);
        ...
        it('is true for a Mark FCP event', async function () {
            const { traceData } = await TraceLoader.traceEngine(this, 'web-dev-initial-url.json.gz');
            const markFCPEvent = traceData.PageLoadMetrics.allMarkerEvents.find(TraceEngine.Types.TraceEvents.isTraceEventFirstContentfulPaint);
        ...
        it('is true for a MarkDOMContent event', async function () {
            const { traceData } = await TraceLoader.traceEngine(this, 'web-dev-initial-url.json.gz');
            const markDOMContentEvent = traceData.PageLoadMetrics.allMarkerEvents.find(TraceEngine.Types.TraceEvents.isTraceEventMarkDOMContent);
        ...
        it('is true for a MarkLoad event', async function () {
            const { traceData } = await TraceLoader.traceEngine(this, 'web-dev-initial-url.json.gz');
            const markLoadEvent = traceData.PageLoadMetrics.allMarkerEvents.find(TraceEngine.Types.TraceEvents.isTraceEventMarkLoad);
        ...
        it('is true for a LCP candiadate event', async function () {
            const { traceData } = await TraceLoader.traceEngine(this, 'web-dev-initial-url.json.gz');
            const markLCPCandidate = traceData.PageLoadMetrics.allMarkerEvents.find(TraceEngine.Types.TraceEvents.isTraceEventLargestContentfulPaintCandidate);
    ...
    describe('buildDetailsNodeForMarkerEvents', () => {
        it('builds the right link for an LCP Event', async function () {
            const { traceData } = await TraceLoader.traceEngine(this, 'web-dev.json.gz');
            const markLCPEvent = getEventOfType(traceData.PageLoadMetrics.allMarkerEvents, TraceEngine.Types.TraceEvents.isTraceEventLargestContentfulPaintCandidate);
        ...
        it('builds the right link for an FCP Event', async function () {
            const { traceData } = await TraceLoader.traceEngine(this, 'web-dev.json.gz');
            const markFCPEvent = getEventOfType(traceData.PageLoadMetrics.allMarkerEvents, TraceEngine.Types.TraceEvents.isTraceEventFirstContentfulPaint);
        ...
        it('builds a generic event for other marker events', async function () {
            const { traceData } = await TraceLoader.traceEngine(this, 'web-dev.json.gz');
            const markLoadEvent = getEventOfType(traceData.PageLoadMetrics.allMarkerEvents, TraceEngine.Types.TraceEvents.isTraceEventMarkLoad);
