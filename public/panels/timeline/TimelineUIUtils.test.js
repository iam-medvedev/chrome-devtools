...
describeWithMockConnection('TimelineUIUtils', function () {
    ...
    describe('mapping to authored script when recording is fresh', function () {
        beforeEach(async () => {
            ...
            const script = debuggerModel.parsedScriptSource(SCRIPT_ID_STRING, scriptUrl, 0, 0, 0, 0, 0, '', undefined, false, sourceMapUrl, true, false, length, false, null, null, null, null, null, null);
    ...
    describe('adjusting timestamps for events and navigations', function () {
        ...
        it('can adjust the times for events that are not PageLoad markers', async function () {
            ...
            const adjustedMarkTime = Timeline.TimelineUIUtils.timeStampForEventAdjustedForClosestNavigationIfPossible(userMark, parsedTrace);
...
`;
        ...
        it('treats the v8.parseOnBackgroundWaiting as scripting even though it would usually be idle', function () {
            const event = makeCompleteEvent("v8.parseOnBackgroundWaiting" /* Trace.Types.Events.Name.STREAMING_COMPILE_SCRIPT_WAITING */, 1, 1, 'v8,devtools.timeline,disabled-by-default-v8.compile');
        ...
        it('assigns the correct color to the swatch of an event\'s title', async function () {
            ...
            const details = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(parsedTrace, task, new Components.Linkifier.Linkifier(), false, null);
    ...
    describe('traceEventDetails', function () {
        it('shows the interaction ID and INP breakdown metrics for a given interaction', async function () {
            ...
            const details = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(parsedTrace, interactionEvent, new Components.Linkifier.Linkifier(), false, null);
        ...
        it('renders invalidations correctly', async function () {
            ...
            const details = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(parsedTrace, recalcStyleEvent, new Components.Linkifier.Linkifier(), false, null);
            const rowData = getRowDataForDetailsElement(details);
            assert.deepEqual(rowData, [
        ...
        it('renders details for performance.mark', async function () {
            ...
            const details = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(parsedTrace, mark, new Components.Linkifier.Linkifier(), false, null);
            const rowData = getRowDataForDetailsElement(details);
            assert.deepEqual(rowData, [
                {
                    title: 'Timestamp',
        ...
        it('renders details for performance.measure', async function () {
            ...
            const details = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(parsedTrace, measure, new Components.Linkifier.Linkifier(), false, null);
            const rowData = getRowDataForDetailsElement(details);
            assert.deepEqual(rowData, [
                {
                    title: 'Timestamp',
        ...
        it('renders the details for a profile call properly', async function () {
            ...
            const details = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(parsedTrace, profileCalls[0], new Components.Linkifier.Linkifier(), false, null);
            const stackTraceData = getStackTraceForDetailsElement(details);
            assert.exists(stackTraceData);
        ...
        it('renders the stack trace of a ScheduleStyleRecalculation properly', async function () {
            ...
            const details = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(parsedTrace, scheduleStyleRecalcs[1], new Components.Linkifier.Linkifier(), false, null);
            const stackTraceData = getStackTraceForDetailsElement(details);
        ...
        it('renders the stack trace of a RecalculateStyles properly', async function () {
            ...
            const [process] = parsedTrace.data.Renderer.processes.values();
            const [thread] = process.threads.values();
            const stylesRecalc = thread.entries.filter(entry => entry.name === "UpdateLayoutTree" /* Trace.Types.Events.Name.RECALC_STYLE */);
            const details = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(parsedTrace, stylesRecalc[3], new Components.Linkifier.Linkifier(), false, null);
            const stackTraceData = getStackTraceForDetailsElement(details);
            assert.deepEqual(stackTraceData, ['(anonymous) @ web.dev/js/app.js?v=1423cda3:1:183']);
        });
        ...
        it('renders the stack trace of extension entries properly', async function () {
            ...
            const [function1, function2, function3] = allThreadEntriesInTrace(parsedTrace).filter(Trace.Types.Events.isProfileCall);
            const mark = allThreadEntriesInTrace(parsedTrace).find(event => event.name === 'Mark');
            const measure = allThreadEntriesInTrace(parsedTrace).find(event => event.name === "UserTiming" /* Trace.Types.Events.Name.USER_TIMING */);
            assert.exists(mark);
            ...
            const markerDetails = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(parsedTrace, markerExtensionEntry, new Components.Linkifier.Linkifier(), false, null);
            ...
            const trackEntryDetails = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(parsedTrace, mockExtensionTrackEntry, new Components.Linkifier.Linkifier(), false, null);
        ...
        it('renders the stack trace of user timings properly', async function () {
            ...
            assert.exists(mark);
            assert.exists(measure);
            const markerDetails = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(parsedTrace, mark, new Components.Linkifier.Linkifier(), false, null);
            const markerStackTraceData = getStackTraceForDetailsElement(markerDetails);
            assert.exists(markerStackTraceData);
            assert.deepEqual(markerStackTraceData, [
                `${function3.callFrame.functionName} @ unknown`,
        ...
        it('renders the warning for a trace event in its details', async function () {
            ...
            const details = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(parsedTrace, longTask, new Components.Linkifier.Linkifier(), false, null);
            const rowData = getRowDataForDetailsElement(details);
            assert.deepEqual(rowData, [
                {
                    title: 'Warning',
        ...
        it('includes the timeout for a RequestIdleCallback event', async function () {
            ...
            const details = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(parsedTrace, requestIdleCallback, new Components.Linkifier.Linkifier(), false, null);
            const rowData = getRowDataForDetailsElement(details);
            assert.deepEqual(rowData.slice(0, 2), // Don't care about the stack trace or initiator
            [
                {
                    title: 'Callback ID',
                    value: '1',
        ...
        it('shows information for the WebSocketCreate initiator when viewing a WebSocketSendHandshakeRequest event', async function () {
            ...
            TraceLoader.initTraceBoundsManager(parsedTrace);
            const sendHandshake = allThreadEntriesInTrace(parsedTrace).find(Trace.Types.Events.isWebSocketSendHandshakeRequest);
            if (!sendHandshake) {
                throw new Error('Could not find handshake event.');
            }
            const details = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(parsedTrace, sendHandshake, new Components.Linkifier.Linkifier(), false, null);
            const rowData = getRowDataForDetailsElement(details);
        ...
        it('shows the aggregated time information for an event', async function () {
            ...
            const details = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(parsedTrace, event, new Components.Linkifier.Linkifier(), true, null);
        ...
        it('renders details for SchedulePostTaskCallback events', async function () {
            ...
            const scheduleDetails = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(parsedTrace, scheduleEvent, new Components.Linkifier.Linkifier(), false, null);
        ...
        it('lets the initiator be clicked on to select it', async function () {
            ...
            sinon.assert.calledOnceWithExactly(timelinePanel.select, Timeline.TimelineSelection.selectionFromEvent(postTaskEvent));
        });
        it('lets the user click the title of an event to zoom into it', async function () {
            ...
            await raf();
            const title = container.querySelector('.timeline-details-chip-title-reveal-entry');
            assert.isOk(title);
        ...
        it('renders the stack trace of a profile call event', async function () {
            ...
            const details = await Timeline.TimelineUIUtils.TimelineUIUtils.buildTraceEventDetails(parsedTrace, jsCall, new Components.Linkifier.Linkifier(), false, null);
            const container = document.createElement('div');
            renderElementIntoDOM(container);
            container.appendChild(details);
            const stackTraceContainer = container.querySelector('.stack-preview-container');
            const stackTracesElements = stackTraceContainer?.shadowRoot?.querySelector('.stack-preview-container')?.querySelectorAll('tbody');
            assert.exists(stackTracesElements);
            const testData = [...stackTracesElements].map(stackTraceElement => {
                const description = stackTraceElement.querySelector('.stack-preview-async-row');
                const stackFrameElements = stackTraceElement.querySelectorAll('tr:not(.stack-preview-async-row)');
    ...
    describe('eventTitle', function () {
        ...
        it('will use the resolved function name for a profile node that has a sourcemap', async function () {
            ...
            SourceMapsResolver.SourceMapsResolver.storeResolvedCodeDataForCallFrame(profileEntry.callFrame, { name: 'resolved-function-test', devtoolsLocation: null, script: null });
    ...
    describe('isMarkerEvent', () => {
        ...
        it('is true for a LCP candiadate event', async function () {
            const parsedTrace = await TraceLoader.traceEngine(this, 'web-dev-initial-url.json.gz');
            const markLCPCandidate = parsedTrace.data.PageLoadMetrics.allMarkerEvents.find(Trace.Types.Events.isLargestContentfulPaintCandidate);
    ...
    describe('buildDetailsNodeForMarkerEvents', () => {
        it('builds the right link for an LCP Event', async function () {
            const parsedTrace = await TraceLoader.traceEngine(this, 'web-dev.json.gz');
            const markLCPEvent = getEventOfType(parsedTrace.data.PageLoadMetrics.allMarkerEvents, Trace.Types.Events.isLargestContentfulPaintCandidate);
