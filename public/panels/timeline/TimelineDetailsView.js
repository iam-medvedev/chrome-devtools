...
export class TimelineDetailsView extends UI.Widget.VBox {
    ...
    async setModel(model, traceEngineData, selectedEvents) {
        if (this.model !== model) {
            this.model = model;
        }
    ...
    async setSelection(selection) {
        ...
        if (TimelineSelection.isSyntheticNetworkRequestDetailsEventSelection(selectionObject)) {
            ...
            const event = selectionObject;
            const traceEventDetails = await TimelineUIUtils.buildTraceEventDetails(event, this.model.timelineModel(), this.detailsLinkifier, true, this.#traceEngineData);
    ...
    updateSelectedRangeStats(startTime, endTime) {
        ...
        if (this.#selectedEvents && isSelectorStatsEnabled) {
            const eventsInRange = TimelineModel.TimelineModel.TimelineModelImpl.findRecalculateStyleEvents(this.#selectedEvents, startTime, endTime);
