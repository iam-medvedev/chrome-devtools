...
describeWithEnvironment('Initiators', () => {
    it('returns the initiator data', async function () {
        ...
        const timerFireEvent = Array.from(parsedTrace.Initiators.eventToInitiator.keys()).find(Trace.Types.Events.isTimerFire);
    ...
    it('will walk back through the initiators to find the entire chain', async function () {
        ...
        for (const initiatorData of initiatorsData) {
            ...
            assert.strictEqual(initiatorData.event.name, "TimerFire" /* Trace.Types.Events.Name.TIMER_FIRE */);
            assert.strictEqual(initiatorData.initiator.name, "TimerInstall" /* Trace.Types.Events.Name.TIMER_INSTALL */);
        }
    });
    it('will walk forward to find the events initiated by the selected entry', async function () {
        ...
        const timerInstall = parsedTrace.Renderer.allTraceEntries.find(entry => {
            return entry.name === "TimerInstall" /* Trace.Types.Events.Name.TIMER_INSTALL */;
        });
        ...
        for (const initatorData of initatorsData) {
            // Ensure each initiatorData object has TimerInstall>TimerFire event to initiator.
            assert.strictEqual(initatorData.event.name, "TimerFire" /* Trace.Types.Events.Name.TIMER_FIRE */);
            assert.strictEqual(initatorData.initiator.name, "TimerInstall" /* Trace.Types.Events.Name.TIMER_INSTALL */);
        }
    });
    it('will return the closest expandable ancestor as an initiator in a pair if the initiator itself is hidden', async function () {
        ...
        const initiatorsData = Timeline.Initiators.initiatorsDataToDraw(parsedTrace, timerInstall, [timerInstall], [timerInstallParent?.entry]);
