...
describeWithEnvironment('AIQueries', () => {
    it('can query for the longest tasks', async function () {
        ...
        const tasks = Utils.InsightAIContext.AIQueries.longestTasks(firstNav.args.data?.navigationId, insightSet.bounds, parsedTrace);
        assert.isOk(tasks);
        const expected = [33, 21, 16];
        assert.deepEqual(tasks.map(task => Math.round(task.rootNode.totalTime)), expected);
    });
});
//# sourceMappingURL=InsightAIContext.test.js.map