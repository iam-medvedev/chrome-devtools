...
describeWithEnvironment('CompatibilityTracksAppender', function () {
    ...
    async function initTrackAppender(context, fixture = 'timings-track.json.gz') {
        ...
        tracksAppender = new Timeline.CompatibilityTracksAppender.CompatibilityTracksAppender(flameChartData, parsedTrace, entryData, entryTypeByLevel, entityMapper);
    ...
    describe('Tree view data', () => {
        ...
        describe('eventsForTreeView', () => {
            ...
            it('returns events for tree view for nested tracks', async () => {
                ...
                const rasterTracks = tracksAppender.threadAppenders().filter(threadAppender => threadAppender.threadType === "RASTERIZER" /* Trace.Handlers.Threads.ThreadType.RASTERIZER */);
    ...
    describe('popoverInfo', () => {
        ...
        it('includes the timestamp when hovering over a perf extensibility marker', async () => {
            ...
            assert.deepEqual(info, { title: 'A mark', formattedTime: '1.61 s', warningElements: [], additionalElements: [], url: null });
        });
    });
