...
import { allThreadEntriesInTrace, makeMockRendererHandlerData as makeRendererHandlerData, makeProfileCall, setupIgnoreListManagerEnvironment, } from '../../../testing/TraceHelpers.js';
...
describeWithEnvironment('ThreadAppender', function () {
    ...
    it('candy-stripes long tasks', async function () {
        const { parsedTrace, flameChartData, entryData } = await renderThreadAppendersFromTrace(this, 'simple-js-program.json.gz');
        const events = allThreadEntriesInTrace(parsedTrace);
        if (!events) {
            throw new Error('Could not find renderer events');
        }
    ...
    describe('showAllEvents', () => {
        ...
        it('appends WASM profile calls to the flame chart data', async function () {
            ...
            const wasmCall = entryData.find(entry => Trace.Types.Events.isProfileCall(entry) && entry.callFrame.functionName === 'mainWasm');
