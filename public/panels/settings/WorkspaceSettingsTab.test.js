...
describeWithEnvironment('WorkspaceSettingsTab widget', () => {
    function createStubFileSystem(path) {
        const fileSystem = sinon.createStubInstance(Persistence.IsolatedFileSystem.IsolatedFileSystem);
        fileSystem.path.returns(urlString(path));
        return fileSystem;
    }
    async function setup(initialFileSystemPaths = []) {
        const fileSystemManager = Persistence.IsolatedFileSystemManager.IsolatedFileSystemManager.instance({ forceNew: true });
        ...
        const networkPersistenceManager = sinon.createStubInstance(Persistence.NetworkPersistenceManager.NetworkPersistenceManager);
    ...
    it('listens to FileSystemAdded events', async () => {
        ...
        fileSystemManager.dispatchEventToListeners(Persistence.IsolatedFileSystemManager.Events.FileSystemAdded, fileSystem);
        await view.nextInput;
        assert.lengthOf(view.input.fileSystems, 1);
    ...
    it('listens to FileSystemRemoved events', async () => {
        ...
        fileSystemManager.dispatchEventToListeners(Persistence.IsolatedFileSystemManager.Events.FileSystemRemoved, fileSystem);
        await view.nextInput;
    ...
    it('wires up removeFileSystem', async () => {
        ...
        view.input.onRemoveClicked(fileSystem);
        sinon.assert.calledOnceWithExactly(removeFileSystemStub, fileSystem);
    });
});
//# sourceMappingURL=WorkspaceSettingsTab.test.js.map