...
describeWithEnvironment('SyncSection', () => {
    const MOCK_SYNC_INFO_NOT_SIGNED_IN = {
        isSyncActive: false,
    };
    const MOCK_SYNC_INFO_SIGNED_IN_SYNC_ON = {
        isSyncActive: true,
        arePreferencesSynced: true,
        accountEmail: 'test@google.com',
        accountImage: 'data:image/png;base64,test',
    };
    const MOCK_SYNC_INFO_SIGNED_IN_SYNC_OFF = {
        ...MOCK_SYNC_INFO_SIGNED_IN_SYNC_ON,
        isSyncActive: false,
    };
    const MOCK_SYNC_INFO_SIGNED_IN_PREFERENCES_OFF = {
        ...MOCK_SYNC_INFO_SIGNED_IN_SYNC_ON,
        arePreferencesSynced: false,
    };
    const MOCK_GDP_PROFILE = {
        name: 'test',
        activeSubscription: {
    ...
    describe('rendering', () => {
        ...
        it('passes SYNC_DISABLED warning type when sync is off', async () => {
            ...
            widget.syncInfo = MOCK_SYNC_INFO_SIGNED_IN_SYNC_OFF;
            const input = await view.nextInput;
            assert.strictEqual(input.warningType, "SYNC_DISABLED" /* Components.SyncSection.WarningType.SYNC_DISABLED */);
        });
        it('passes PREFERENCES_SYNC_DISABLED warning type when preferences sync is off', async () => {
            const { view, widget } = await createWidget();
            widget.syncInfo = MOCK_SYNC_INFO_SIGNED_IN_PREFERENCES_OFF;
            const input = await view.nextInput;
            assert.strictEqual(input.warningType, "PREFERENCES_SYNC_DISABLED" /* Components.SyncSection.WarningType.PREFERENCES_SYNC_DISABLED */);
        });
