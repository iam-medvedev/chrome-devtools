...
const context1 = {
    ...
    contextType: "realtime" /* Protocol.WebAudio.ContextType.Realtime */,
    contextState: "running" /* Protocol.WebAudio.ContextState.Running */,
    sampleRate: 44100,
...
describeWithMockConnection('WebAudioView', () => {
    ...
    it('selects created context', async () => {
        ...
        assert.exists(model);
        model.dispatchEventToListeners("ContextCreated" /* WebAudio.WebAudioModel.Events.CONTEXT_CREATED */, context1);
    ...
    it('clears list on reset', async () => {
        ...
        model.dispatchEventToListeners("ModelReset" /* WebAudio.WebAudioModel.Events.MODEL_RESET */);
        const input = await view.nextInput;
    ...
    it('re-selects created context after change', async () => {
        ...
        model.dispatchEventToListeners("ContextCreated" /* WebAudio.WebAudioModel.Events.CONTEXT_CREATED */, context1);
        await view.nextInput;
        model.dispatchEventToListeners("ContextChanged" /* WebAudio.WebAudioModel.Events.CONTEXT_CHANGED */, context1);
        const input = await view.nextInput;
        assert.deepEqual(input.contexts, [context1]);
        assert.strictEqual(input.selectedContextIndex, 0);
    });
    it('keeps first context selected when another is added', async () => {
        ...
        model.dispatchEventToListeners("ContextCreated" /* WebAudio.WebAudioModel.Events.CONTEXT_CREATED */, context1);
        await view.nextInput;
        model.dispatchEventToListeners("ContextCreated" /* WebAudio.WebAudioModel.Events.CONTEXT_CREATED */, context2);
        const input = await view.nextInput;
    ...
    it('does not change selection when a different context is changed', async () => {
        ...
        model.dispatchEventToListeners("ContextCreated" /* WebAudio.WebAudioModel.Events.CONTEXT_CREATED */, context2);
        await view.nextInput;
        model.dispatchEventToListeners("ContextChanged" /* WebAudio.WebAudioModel.Events.CONTEXT_CHANGED */, context2);
        const input = await view.nextInput;
        assert.deepEqual(input.contexts, [context1, context2]);
        assert.strictEqual(input.selectedContextIndex, 0);
    });
    ...
    it('keeps context selected when another is destroyed', async () => {
        ...
        model.dispatchEventToListeners("ContextCreated" /* WebAudio.WebAudioModel.Events.CONTEXT_CREATED */, context2);
        await view.nextInput;
        model.dispatchEventToListeners("ContextDestroyed" /* WebAudio.WebAudioModel.Events.CONTEXT_DESTROYED */, context2.contextId);
        const input = await view.nextInput;
        assert.deepEqual(input.contexts, [context1]);
        assert.strictEqual(input.selectedContextIndex, 0);
    });
    it('selects another context when the selected one is destroyed', async () => {
        ...
        model.dispatchEventToListeners("ContextCreated" /* WebAudio.WebAudioModel.Events.CONTEXT_CREATED */, context2);
        await view.nextInput;
        model.dispatchEventToListeners("ContextDestroyed" /* WebAudio.WebAudioModel.Events.CONTEXT_DESTROYED */, context1.contextId);
        const input = await view.nextInput;
