...
describeWithMockConnection('AI Assistance Panel', () => {
    ...
    describe('consent view', () => {
        ...
        it('should switch from consent view to chat view when enabling setting', async () => {
            ...
            assert.strictEqual((await view.nextInput).state, "chat-view" /* AiAssistancePanel.State.CHAT_VIEW */);
        });
    ...
    describe('history interactions', () => {
        ...
        it('should select default agent after new chat', async () => {
            ...
            view.input.onNewChatClick();
            assert.deepEqual((await view.nextInput).messages, []);
            assert.deepEqual(view.input.conversationType, "freestyler" /* AiAssistanceModel.ConversationType.STYLING */);
        });
        it('should select the performance insights agent if it is enabled and the user has expanded an insight', async () => {
            ...
            view.input.onNewChatClick();
            assert.deepEqual((await view.nextInput).messages, []);
            assert.deepEqual(view.input.conversationType, "performance-insight" /* AiAssistanceModel.ConversationType.PERFORMANCE_INSIGHT */);
        });
        ...
        it('should switch agents and restore history', async () => {
            ...
            (await view.nextInput)
                .onTextSubmit('User question to Freestyler?', imageInput, "screenshot" /* AiAssistanceModel.MultimodalInputType.SCREENSHOT */);
            assert.deepEqual((await view.nextInput).messages, [
                {
                    entity: "user" /* AiAssistancePanel.ChatMessageEntity.USER */,
    ...
    describe('auto agent selection for panels', () => {
        const tests = [
            {
                ...
                expectedConversationType: "freestyler" /* AiAssistanceModel.ConversationType.STYLING */,
                featureFlagName: 'devToolsFreestyler',
            },
            {
                panel: Network.NetworkPanel.NetworkPanel,
                expectedConversationType: "drjones-network-request" /* AiAssistanceModel.ConversationType.NETWORK */,
                featureFlagName: 'devToolsAiAssistanceNetworkAgent',
            },
            {
                panel: Sources.SourcesPanel.SourcesPanel,
                expectedConversationType: "drjones-file" /* AiAssistanceModel.ConversationType.FILE */,
        ...
        describe('Performance Insight agent', () => {
            it('should select the PERFORMANCE_INSIGHT agent when the performance panel is open and insights are enabled and an insight is expanded', async () => {
                ...
                UI.Context.Context.instance().setFlavor(Timeline.TimelinePanel.SelectedInsight, new Timeline.TimelinePanel.SelectedInsight({}));
                const { view } = await createAiAssistancePanel();
                assert.strictEqual(view.input.conversationType, "performance-insight" /* AiAssistanceModel.ConversationType.PERFORMANCE_INSIGHT */);
            });
    ...
    it('erases previous partial response on blocked error', async () => {
        ...
        assert.deepEqual((await view.nextInput).messages, [
            ...
            },
            {
                ...
                entity: "model" /* AiAssistancePanel.ChatMessageEntity.MODEL */,
                rpcId: undefined,
                error: "block" /* AiAssistanceModel.ErrorType.BLOCK */,
                steps: [],
            },
        ]);
    });
    ...
    describe('multimodal input', () => {
        ...
        it('sends image as input', async () => {
            ...
            assert.isTrue(view.input.multimodalInputEnabled);
            view.input.onTextSubmit('test', { inlineData: { data: 'imageInput', mimeType: 'image/jpeg' } }, "screenshot" /* AiAssistanceModel.MultimodalInputType.SCREENSHOT */);
            assert.deepEqual((await view.nextInput).messages, [
                {
                    entity: "user" /* AiAssistancePanel.ChatMessageEntity.USER */,
                    text: 'test',
    ...
    describe('handleMcpRequest', () => {
        ...
        const notAvailable = 'For AI features to be available, you need to log into Chrome and enable AI assistance in DevTools settings';
        beforeEach(() => {
            Common.Settings.moduleSetting('ai-assistance-enabled').set(true);
            updateHostConfig({
                devToolsFreestyler: {
        ...
        it('can be blocked by a setting', async () => {
            ...
            try {
                await panel.handleMcpRequest('Please help me debug this problem', "freestyler" /* AiAssistanceModel.ConversationType.STYLING */);
        ...
        it('returns an explanation for styling assistance requests', async () => {
            ...
            const response = await panel.handleMcpRequest('Please help me debug this problem', "freestyler" /* AiAssistanceModel.ConversationType.STYLING */);
            assert.strictEqual(response, explanation);
            assert.strictEqual(document.body.querySelector('devtools-snackbar')?.shadowRoot?.querySelector('.message')?.textContent?.trim(), 'DevTools received an MCP request');
        });
        it('handles styling assistance requests which contain a selector', async () => {
            ...
            const response = await panel.handleMcpRequest('Please help me debug this problem', "freestyler" /* AiAssistanceModel.ConversationType.STYLING */, 'h1');
            assert.strictEqual(response, explanation);
...
STOP`,
        ...
        it('throws an error for file assistance requests', async () => {
            ...
            try {
                await panel.handleMcpRequest('Please help me debug this problem', "drjones-file" /* AiAssistanceModel.ConversationType.FILE */);
                assert.fail('Expected `handleMcpRequest` to throw');
            }
            catch (err) {
                assert.strictEqual(err.message, `Debugging with an agent of type '${"drjones-file" /* AiAssistanceModel.ConversationType.FILE */}' is not implemented yet.`);
            }
        });
        it('throws an error for network assistance requests', async () => {
            ...
            try {
                await panel.handleMcpRequest('Please help me debug this problem', "drjones-network-request" /* AiAssistanceModel.ConversationType.NETWORK */);
                assert.fail('Expected `handleMcpRequest` to throw');
            }
            catch (err) {
                assert.strictEqual(err.message, `Debugging with an agent of type '${"drjones-network-request" /* AiAssistanceModel.ConversationType.NETWORK */}' is not implemented yet.`);
            }
        });
        it('throws an error for performance assistance requests', async () => {
            ...
            try {
                await panel.handleMcpRequest('Please help me debug this problem', "drjones-performance" /* AiAssistanceModel.ConversationType.PERFORMANCE */);
                assert.fail('Expected `handleMcpRequest` to throw');
            }
            catch (err) {
        ...
        it('throws an error for performance-insight assistance requests', async () => {
            ...
            try {
                await panel.handleMcpRequest('Please help me debug this problem', "performance-insight" /* AiAssistanceModel.ConversationType.PERFORMANCE_INSIGHT */);
                assert.fail('Expected `handleMcpRequest` to throw');
            }
            catch (err) {
