...
describe('ExportForAgentsDialog', () => {
    ...
    it('hides the loading spinner and shows markdown text when switching to markdown view while prompt is loading', async () => {
        let resolvePrompt = () => { };
        const promptTextPromise = new Promise(resolve => {
            resolvePrompt = resolve;
        });
        ...
        assert.isNotNull(component.contentElement.querySelector('devtools-spinner'), 'Spinner should be present in prompt view while loading');
        const textarea = querySelectorErrorOnMissing(component.contentElement, 'textarea');
    ...
    it('copies prompt text to clipboard when in prompt state', async () => {
        ...
        snackbarStub.returns(mockSnackbar);
        const component = new AiAssistance.ExportForAgentsDialog.ExportForAgentsDialog({
            dialog,
            promptText,
            markdownText,
        ...
        renderElementIntoDOM(component);
        await component.updateComplete;
        const primaryButton = component.contentElement?.querySelector('devtools-button');
    ...
    it('persists the selected state across dialog loads', async () => {
        ...
        const markdownRadioButton = querySelectorErrorOnMissing(component1.contentElement, 'input[value="conversation"]');
        markdownRadioButton.click();
        await component1.updateComplete;
        assert.isTrue(markdownRadioButton.checked);
        component1.detach();
        ...
        const markdownRadioButton2 = querySelectorErrorOnMissing(component2.contentElement, 'input[value="conversation"]');
