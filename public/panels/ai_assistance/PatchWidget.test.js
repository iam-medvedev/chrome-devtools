...
import { cleanup, createPatchWidget, createPatchWidgetWithDiffView, initializePersistenceImplForTests, MockAidaAbortError, mockAidaClient, MockAidaFetchError, } from '../../testing/AiAssistanceHelpers.js';
    ...
    describe('applyToPageTree', () => {
        ...
        describe('enterprise text cases', () => {
            it('should tooltip text include no logging case when the enterprise policy value is ALLOW_WITHOUT_LOGGING', async () => {
                ...
                assert.include(view.input.disclaimerTooltipText, 'This data will not be used to improve Google’s AI models.');
            });
            it('should tooltip text not include no logging case when the enterprise policy value is ALLOW', async () => {
                ...
                const { view } = await createPatchWidget();
                assert.notInclude(view.input.disclaimerTooltipText, 'This data will not be used to improve Google’s AI models.');
            });
        });
...
* /index.html`);
    ...
    describe('diff view', () => {
        ...
        it('on apply should call handle function and stash changes', async () => {
            const { view, widget, } = await createPatchWidget({ aidaClient: mockAidaClient([[{ explanation: 'patch applied' }]]) });
            widget.changeSummary = 'body { background-color: red; }';
