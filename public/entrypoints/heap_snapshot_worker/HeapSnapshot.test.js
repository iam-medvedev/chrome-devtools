...
describe('HeapSnapshot', () => {
    ...
    function postprocessHeapSnapshotMock(mock) {
        mock.nodes = new MockArray(mock.nodes);
        mock.edges = new MockArray(mock.edges);
        if (mock.trace_function_infos) {
            mock.trace_function_infos = new MockArray(mock.trace_function_infos);
        }
        return mock;
    }
    ...
    class HeapNode {
        ...
        id;
        ordinal;
        constructor(name, selfSize, type, id) {
        ...
        linkNode(node, type, nameOrIndex) {
            if (!this.builder) {
            ...
            if (key in this.edges) {
                throw new Error('Can\'t add edge with the same nameOrIndex. nameOrIndex: ' + nameOrIndex +
    ...
    class HeapEdge {
        ...
        type;
        nameOrIndex;
        constructor(targetNode, type, nameOrIndex) {
    ...
    it('heapSnapshotLargeRetainedSize', async () => {
        ...
        assert.strictEqual(snapshot.rootNode().retainedSize(), iterations * nodeSize, 'Ensure that root node retained size supports values exceeding 2^32 bytes.');
    });
    it('heapSnapshotDominatedNodes', async () => {
        ...
        assert.strictEqual(actualDominatedNodeIndex.length, expectedDominatedNodeIndex.length, 'Dominated Nodes Index length');
    ...
    it('heapSnapshotPageOwned', async () => {
        ...
        assert.strictEqual(JSON.stringify(actualPageObjects), JSON.stringify(expectedPageObjects), 'We are expecting that only window(third element) and PageOwnedNode(forth element) have the pageObject flag.');
    });
    it('heapSnapshotRetainers', async () => {
        const snapshot = await HeapSnapshotWorker.HeapSnapshot.createJSHeapSnapshotForTesting(createHeapSnapshotMock());
        const expectedRetainers = { '': [], A: [''], B: ['', 'A'], C: ['A', 'B'], D: ['B'], E: ['C'] };
        ...
        for (; nodeIterator.hasNext(); nodeIterator.next()) {
            ...
            names.sort();
            assert.strictEqual(names.join(','), expectedRetainers[node.name()].join(','), 'retainers of "' + node.name() + '"');
        }
    });
    ...
    function findNodeIndexByName(snapshot, name) {
        const nodeIterator = new HeapSnapshotWorker.HeapSnapshot.HeapSnapshotNodeIterator(snapshot.createNode(0));
        for (; nodeIterator.hasNext(); nodeIterator.next()) {
            const node = nodeIterator.item();
            if (node.name() === name) {
    ...
    it('heapSnapshotAggregates', async () => {
        ...
        for (const key in aggregates) {
            ...
            for (const parameter in expectedAggregate) {
                assert.strictEqual(aggregate[parameter], expectedAggregate[parameter], 'parameter ' + parameter + ' of "' + aggregate.name + '"');
            }
        }
        const expectedIndexes = { A: [7], B: [14], C: [21], D: [28], E: [35] };
    ...
    it('heapSnapshotFlags', async () => {
        const snapshot = await HeapSnapshotWorker.HeapSnapshot.createJSHeapSnapshotForTesting(createHeapSnapshotMockWithDOM());
        const expectedCanBeQueried = {
            '': false,
            A: true,
            B: true,
            C: true,
        ...
        for (; nodeIterator.hasNext(); nodeIterator.next()) {
            const node = nodeIterator.item();
            assert.strictEqual(node.canBeQueried(), expectedCanBeQueried[node.name()], 'canBeQueried of "' + node.name() + '"');
        }
    });
    it('heapSnapshotNodesProvider', async () => {
        const snapshot = await HeapSnapshotWorker.HeapSnapshot.createJSHeapSnapshotForTesting(createHeapSnapshotMock());
        const allNodeIndexes = [];
    ...
    describe('heapSnapshotEdgesProvider query options', () => {
        ...
        it('sorts edges when sortBy option is specified', async () => {
            ...
            assert.deepEqual(sortedEdgeNames, ['objAEdge', 'realObjEdge', 'null', 'numEdge', 'true', 'undefined'], 'Sorted by retainedSize descending');
        });
    });
    it('heapSnapshotLoader', async () => {
        ...
        const referenceToCompare = {
            nodes: Array.from(reference.nodes),
            containmentEdges: Array.from(reference.containmentEdges),
        };
    ...
    it('heapSnapshotLoaderThrowsOnMalformedSnapshot', async () => {
        ...
        try {
            ...
            assert.include(error.message, 'Token "snapshot" not found');
        }
        finally {
            channel.port1.close();
            channel.port2.close();
    ...
    it('heapSnapshotLoaderThrowsOnTruncatedData', async () => {
        ...
        try {
            ...
            assert.instanceOf(error, Error);
            assert.include(error.message, 'unexpected end of input');
        }
        finally {
            channel.port1.close();
            channel.port2.close();
    ...
    it('can filter objects by native context', async () => {
        ...
        assert.isTrue(indexesNoNc.has(snapshot.nodeIndexForId(300)), 'UnattributedNode should be in noNativeContext aggregates');
        assert.isFalse(indexesNoNc.has(snapshot.nodeIndexForId(100)), 'JSObject1 should NOT be in noNativeContext aggregates');
        ...
        assert.isTrue(indexesShared.has(snapshot.nodeIndexForId(400)), 'SharedObject should be in sharedNativeContext aggregates');
        assert.isFalse(indexesShared.has(snapshot.nodeIndexForId(100)), 'JSObject1 should NOT be in sharedNativeContext aggregates');
    });
    it('heapSnapshotGetObjectInfo', async () => {
        const snapshot = await HeapSnapshotWorker.HeapSnapshot.createJSHeapSnapshotForTesting(createHeapSnapshotMockWithDetachedness());
        // Root node
        ...
        assert.deepEqual(rootInfo, {
            ...
            type: 'hidden',
            nodeIndex: 0,
            detachedness: 0 /* HeapSnapshotModel.HeapSnapshotModel.DOMLinkState.UNKNOWN */,
            selfSize: 0,
            retainedSize: 8,
        ...
        assert.deepEqual(windowInfo, {
            ...
            type: 'object',
            nodeIndex: 8,
            detachedness: 2 /* HeapSnapshotModel.HeapSnapshotModel.DOMLinkState.DETACHED */,
            selfSize: 8,
            retainedSize: 8,
            distance: 1,
            edgeCount: 0,
    ...
    describe('queryObjects', () => {
        async function createTestSnapshotForQueryObjects() {
            const parsedSnapshot = postprocessHeapSnapshotMock({
                ...
                trace_tree: [],
                locations: [],
                strings: ['', 'root', 'HTMLDivElement', 'MyClass', 'OtherClass', 'AttachedNode', 'edge0', 'divProp', 'foo', 'bar'],
            });
            return await HeapSnapshotWorker.HeapSnapshot.createJSHeapSnapshotForTesting(parsedSnapshot);
        }
        ...
        it('sorts by retainedSize, selfSize, and id', async () => {
            ...
            const itemsRetained = providerRetained.serializeItemsRange(0, 100).items;
            ...
            const itemsSelf = providerSelf.serializeItemsRange(0, 100).items;
            assert.deepEqual(itemsSelf.map(i => i.selfSize), [100, 50, 10, 5, 0]);
            const providerId = snapshot.queryObjects({ sortBy: 'id' });
            const itemsId = providerId.serializeItemsRange(0, 100).items;
